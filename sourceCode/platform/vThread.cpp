...
#ifdef WIN_SIM
    return (threadA ==
            threadB); // normally this is the only thread ID comparison place!! Except for another one in VMutex
#else
    return (pthread_equal(threadA, threadB) != 0); // normally this is the only thread ID comparison place!! Except for
                                                   // another one in VMutex (THREAD_ID_COMPARISON)
#endif
}
