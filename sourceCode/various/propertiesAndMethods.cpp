...
const std::vector<SProperty*> allProps = []{
    ...
    for (const auto& prop : allProps_obj) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_app) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_customObjectClass) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_customObject) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_detachedScript) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_stack) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_collCont) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_collection) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_drawCont) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_drawingObj) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_col) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_volume) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_dyn) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_scene) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_sim) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_meshWrap) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_mesh) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_viewable) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_objCont) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_material) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_customSceneObjectClass) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_sceneObject) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_shape) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_camera) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_dummy) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_forceSensor) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_graph) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_joint) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_light) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_ocTree) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_pointCloud) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_proximitySensor) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_visionSensor) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_script) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_marker) result.push_back(const_cast<SProperty*>(&prop));
    for (const auto& prop : allProps_customSceneObject) result.push_back(const_cast<SProperty*>(&prop));
...
const std::vector<SDepr>& getDeprecatedList()
{
    static const std::vector<SDepr> tmpDeprecated =
    {
        ...
        {"joint", prop(PropJoint::DEPRECATED_dynVelMode),                   std::string(prop(PropJoint::dynSmoothMotionProfile).name) + DEPRECATION_NO_REPLACE},
        {"joint", prop(PropJoint::DEPRECATED_dynPosMode),                   std::string(prop(PropJoint::dynSmoothMotionProfile).name) + DEPRECATION_NO_REPLACE},
