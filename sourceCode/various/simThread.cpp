...
void CSimThread::executeMessages()
{
    ...
    { // we always enter here, unless we skip display frames during simulation. In idle mode, _idleSleepAndCheckIfRenderRequired will sleep appropriately
        ...
        if (App::scene->simulation->isSimulationStopped())
        {
            ...
            {
                CPath_old* it = App::scene->sceneObjects->getPathFromIndex(i);
                it->resetPath();
            }
        }

        App::scenes->pluginContainer->sendEventCallbackMessageToAllPlugins_old(sim_message_eventcallback_beforerendering);
...
{ // CALLED FROM ANY THREAD
    cmd.sceneUniqueId = App::scene->environment->getSceneUniqueID();
...
void CSimThread::_executeSimulationThreadCommand(SSimulationThreadCommand cmd)
{
    ...
    {
        App::scene->undoBufferContainer->memorizeStateIfNeeded();
    ...
    if (cmd.sceneUniqueId != App::scene->environment->getSceneUniqueID())
        return;
    ...
    {
        CShape* shape = App::scene->sceneObjects->getShapeFromHandle(cmd.intParams[0]);
        if (shape != nullptr)
        {
            if (!shape->isCompound())
            {
    ...
    {
        CSceneObject* child = App::scene->sceneObjects->getObjectFromHandle(cmd.intParams[0]);
        CSceneObject* parent = App::scene->sceneObjects->getObjectFromHandle(cmd.intParams[1]);
        if (child != nullptr)
        {
            App::scene->sceneObjects->setObjectParent(child, parent, true);
    ...
    if ((cmd.cmdId > SIMULATION_COMMANDS_START_SCCMD) && (cmd.cmdId < SIMULATION_COMMANDS_END_SCCMD))
        App::scene->simulation->processCommand(cmd.cmdId);
    ...
    {
        CSceneObject* obj = App::scene->sceneObjects->getObjectFromHandle(cmd.intParams[0]);
        if (obj != nullptr)
        {
    ...
    if (cmd.cmdId == ADD_OR_REMOVE_TO_FROM_OBJECT_SELECTION_CMD)
    {
        ...
        {
            CSceneObject* obj = App::scene->sceneObjects->getObjectFromHandle(cmd.intParams[i]);
            if (obj != nullptr)
            {
                if (cmd.boolParams[i])
                    App::scene->sceneObjects->addObjectToSelection(cmd.intParams[i]);
                else
                    App::scene->sceneObjects->removeObjectFromSelection(cmd.intParams[i]);
                App::undoRedo_sceneChanged("");
            }
        }
    }
    ...
    { // cmd.intParams[0] is an object handle
        ...
        else
        {
            ...
            CSceneObject* object = App::scene->sceneObjects->getObjectFromHandle(cmd.intParams[0]);
        ...
        (cmd.cmdId < EDIT_MODE_COMMANDS_END_EMCMD))
    {
        ...
        if (cmd.cmdId == PATH_EDIT_MODE_OLD_APPEND_NEW_PATH_POINT_FROM_CAMERA_EMCMD)
        {
            ...
            { // make sure that object is still valid:
                if (!App::scene->sceneObjects->doesObjectExist(additionalSceneObject))
    ...
    if (cmd.cmdId == MODEL_BROWSER_DRAG_AND_DROP_CMD)
    {
        ...
        if (CFileOperations::loadModel(cmd.stringParams[0].c_str(), false, false, nullptr, false, false, &infoStr, &errorStr))
        {
            ...
            if (App::scene->sceneObjects->getSelectionCount() == 1)
            { // we could have several model bases (in the old fileformat)
                CSceneObject* obj = App::scene->sceneObjects->getLastSelectionObject();
                if (obj != nullptr)
                {
                    if (cmd.intParams[0] >= 0)
                    {
                        CSceneObject* parent(App::scene->sceneObjects->getObjectFromHandle(cmd.intParams[0]));
                        if (parent != nullptr)
                        {
    ...
    if (cmd.cmdId == DRAGENTER_GUITRIGGEREDCMD)
    {
        ...
        App::scenes->broadcastMsg(stack, -1, 0);
    ...
    {
        App::scenes->pluginContainer->selectExtRenderer(cmd.intParams[0]);
    }

    if (cmd.cmdId == SET_VIEW_ANGLE_CAMERAGUITRIGGEREDCMD)
    {
        CCamera* cam = App::scene->sceneObjects->getCameraFromHandle(cmd.intParams[0]);
        if (cam != nullptr)
    ...
    if (cmd.cmdId == SET_TRACKED_OBJECT_CAMERAGUITRIGGEREDCMD)
    {
        ...
        if (cam != nullptr)
        {
            ...
            {
                CSceneObject* obj = App::scene->sceneObjects->getObjectFromHandle(cmd.intParams[1]);
                if (obj != nullptr)
    ...
    if (cmd.cmdId == NEW_COLLECTION_COLLECTIONGUITRIGGEREDCMD)
    {
        ...
        App::scene->collections->addCollection(newGroup, false);
    ...
    {
        CCollection* theGroup = App::scene->collections->getObjectFromHandle(cmd.intParams[0]);
        if (theGroup != nullptr)
    ...
    {
        App::scene->collections->removeCollection(cmd.intParams[0]);
    }
    if (cmd.cmdId == REMOVE_COLLECTION_ITEM_COLLECTIONGUITRIGGEREDCMD)
    {
        ...
        if (theGroup != nullptr)
        {
            ...
            { // The group is empty and we have to remove it
                App::scene->collections->removeCollection(theGroup->getObjectHandle());
            }
        }
    }
    if (cmd.cmdId == RENAME_COLLECTION_COLLECTIONGUITRIGGEREDCMD)
    {
        CCollection* it = App::scene->collections->getObjectFromHandle(cmd.intParams[0]);
        if (it != nullptr)
    ...
    if (cmd.cmdId == ADD_COLLECTION_ITEM_FROMBASE_COLLECTIONGUITRIGGEREDCMD)
    {
        ...
        if (it != nullptr)
        {
            CSceneObject* lastSel = App::scene->sceneObjects->getObjectFromHandle(cmd.intParams[1]);
            if (lastSel != nullptr)
            {
    ...
    {
        CVisionSensor* it = App::scene->sceneObjects->getVisionSensorFromHandle(cmd.intParams[0]);
        if (it != nullptr)
    ...
    {
        CVisionSensor* last = App::scene->sceneObjects->getVisionSensorFromHandle(cmd.intParams[0]);
        if (last != nullptr)
        {
            int r[2];
            ...
            for (size_t i = 1; i < cmd.intParams.size(); i++)
            {
                CVisionSensor* it = App::scene->sceneObjects->getVisionSensorFromHandle(cmd.intParams[i]);
                if (it != nullptr)
                {
    ...
    {
        int h = App::scene->collisions_old->addNewObject(cmd.intParams[0], cmd.intParams[1], "");
        // Now select the new collection in the UI. We need to post it so that it arrives after the dialog refresh!:
        SSimulationThreadCommand cmd2;
        cmd2.cmdId = CALL_DIALOG_FUNCTION_GUITRIGGEREDCMD;
    ...
    {
        CCollisionObject_old* it = App::scene->collisions_old->getObjectFromHandle(cmd.intParams[0]);
        if (it != nullptr)
    ...
    if (cmd.cmdId == DELETE_OBJECT_COLLISIONGUITRIGGEREDCMD)
    {
        CCollisionObject_old* it = App::scene->collisions_old->getObjectFromHandle(cmd.intParams[0]);
        if (it != nullptr)
            App::scene->collisions_old->removeObject(it->getObjectHandle());
    }
    if (cmd.cmdId == TOGGLE_ENABLE_ALL_COLLISIONGUITRIGGEREDCMD)
    {
        App::scene->mainSettings_old->collisionDetectionEnabled =
            !App::scene->mainSettings_old->collisionDetectionEnabled;
    }
    ...
    {
        int h = App::scene->distances_old->addNewObject(cmd.intParams[0], cmd.intParams[1], "");
        // Now select the new collection in the UI. We need to post it so that it arrives after the dialog refresh!:
        SSimulationThreadCommand cmd2;
        cmd2.cmdId = CALL_DIALOG_FUNCTION_GUITRIGGEREDCMD;
    ...
    {
        CDistanceObject_old* it = App::scene->distances_old->getObjectFromHandle(cmd.intParams[0]);
        if (it != nullptr)
            it->setObjectName(cmd.stringParams[0].c_str(), true);
    }
    if (cmd.cmdId == DELETE_OBJECT_DISTANCEGUITRIGGEREDCMD)
    {
        CDistanceObject_old* it = App::scene->distances_old->getObjectFromHandle(cmd.intParams[0]);
        if (it != nullptr)
            App::scene->distances_old->removeObject(it->getObjectHandle());
    }
    if (cmd.cmdId == TOGGLE_ENABLE_ALL_DISTANCEGUITRIGGEREDCMD)
    {
        App::scene->mainSettings_old->distanceCalculationEnabled =
            !App::scene->mainSettings_old->distanceCalculationEnabled;
    }
    ...
    {
        CSceneObject* last = App::scene->sceneObjects->getObjectFromHandle(cmd.intParams[0]);
        if (last != nullptr)
        {
    ...
    {
        if (App::scene->simulation->isSimulationStopped())
            App::scene->simulation->setTimeStep(cmd.doubleParams[0]);
    }
    if (cmd.cmdId == SET_PPF_SIMULATIONGUITRIGGEREDCMD)
    {
        if (App::scene->simulation->isSimulationStopped())
            App::scene->simulation->setPassesPerRendering(cmd.intParams[0]);
    }
    ...
    {
        if (App::scene->simulation->isSimulationStopped())
            App::scene->simulation->setRealTimeCoeff(cmd.doubleParams[0]);
    }
    if (cmd.cmdId == TOGGLE_PAUSEATTIME_SIMULATIONGUITRIGGEREDCMD)
    {
        if (App::scene->simulation->isSimulationStopped())
            App::scene->simulation->setPauseAtSpecificTime(
                !App::scene->simulation->getPauseAtSpecificTime());
    }
    if (cmd.cmdId == SET_PAUSETIME_SIMULATIONGUITRIGGEREDCMD)
    {
        if (App::scene->simulation->isSimulationStopped())
            App::scene->simulation->setPauseTime(cmd.doubleParams[0]);
    }
    if (cmd.cmdId == TOGGLE_PAUSEATERROR_SIMULATIONGUITRIGGEREDCMD)
    {
        if (App::scene->simulation->isSimulationStopped())
            App::scene->simulation->setPauseAtError(!App::scene->simulation->getPauseAtError());
    }
    if (cmd.cmdId == TOGGLE_REMOVENEWOBJECTS_SIMULATIONGUITRIGGEREDCMD)
    {
        App::scene->simulation->setRemoveNewObjectsAtSimulationEnd(
            !App::scene->simulation->getRemoveNewObjectsAtSimulationEnd());
    }
    if (cmd.cmdId == TOGGLE_FULLSCREENATSTART_SIMULATIONGUITRIGGEREDCMD)
    {
        if (App::scene->simulation->isSimulationStopped())
            App::scene->simulation->setFullscreenAtSimulationStart(
                !App::scene->simulation->getFullscreenAtSimulationStart());
    }
    ...
    {
        CProxSensor* it = App::scene->sceneObjects->getProximitySensorFromHandle(cmd.intParams[0]);
        if (it != nullptr)
            it->setExplicitHandling(!it->getExplicitHandling());
    }
    ...
    {
        CProxSensor* last = App::scene->sceneObjects->getProximitySensorFromHandle(cmd.intParams[0]);
        if (last != nullptr)
        {
            for (size_t i = 1; i < cmd.intParams.size(); i++)
            {
                CProxSensor* it = App::scene->sceneObjects->getProximitySensorFromHandle(cmd.intParams[i]);
                if (it != nullptr)
                {
    ...
    if (cmd.cmdId == APPLY_VISUALPROP_PROXSENSORGUITRIGGEREDCMD)
    {
        ...
        if (it != nullptr)
        {
            for (size_t i = 1; i < cmd.intParams.size(); i++)
            {
                CProxSensor* it2 = App::scene->sceneObjects->getProximitySensorFromHandle(cmd.intParams[i]);
                if (it2 != nullptr)
                {
    ...
    {
        CProxSensor* prox = App::scene->sceneObjects->getProximitySensorFromHandle(cmd.intParams[0]);
        CMill* mill = App::scene->sceneObjects->getMillFromHandle(cmd.intParams[0]);
        if (prox != nullptr)
    ...
    if (cmd.cmdId == APPLY_DETECTIONVOLUMEGUITRIGGEREDCMD)
    {
        ...
        if (mill != nullptr)
        {
            for (size_t i = 1; i < cmd.intParams.size(); i++)
            {
                CMill* it = App::scene->sceneObjects->getMillFromHandle(cmd.intParams[i]);
                if (it != nullptr)
                {
                    it->convexVolume->disableVolumeComputation(true);
                    // Volume parameters:
    ...
    {
        App::scene->mainSettings_old->millsEnabled = !App::scene->mainSettings_old->millsEnabled;
    }
    if (cmd.cmdId == TOGGLE_EXPLICITHANDLING_MILLGUITRIGGEREDCMD)
    {
        CMill* it = App::scene->sceneObjects->getMillFromHandle(cmd.intParams[0]);
        if (it != nullptr)
            it->setExplicitHandling(!it->getExplicitHandling());
    }
    ...
    {
        CLight* it = App::scene->sceneObjects->getLightFromHandle(cmd.intParams[0]);
        if (it != nullptr)
    ...
    if (cmd.cmdId == APPLY_VISUALPROP_DUMMYGUITRIGGEREDCMD)
    {
        ...
        if (it != nullptr)
        {
            for (size_t i = 1; i < cmd.intParams.size(); i++)
            {
                CDummy* it2 = App::scene->sceneObjects->getDummyFromHandle(cmd.intParams[i]);
                if (it2 != nullptr)
                {
    ...
    if (cmd.cmdId == SET_LINKEDDUMMY_DUMMYGUITRIGGEREDCMD)
    {
        ...
        if (it != nullptr)
        {
            CDummy* it2 = App::scene->sceneObjects->getDummyFromHandle(cmd.intParams[1]);
            if (it2 != nullptr)
            {
    ...
    {
        CMirror* it = App::scene->sceneObjects->getMirrorFromHandle(cmd.intParams[0]);
        if (it != nullptr)
    ...
    {
        App::scene->mainSettings_old->clippingPlanesDisabled =
            !App::scene->mainSettings_old->clippingPlanesDisabled;
    }
    if (cmd.cmdId == TOGGLE_DISABLEALLMIRRORS_MIRRORGUITRIGGEREDCMD)
    {
        App::scene->mainSettings_old->mirrorsDisabled = !App::scene->mainSettings_old->mirrorsDisabled;
    }
    if (cmd.cmdId == TOGGLE_SAVECALCSTRUCT_ENVIRONMENTGUITRIGGEREDCMD)
    {
        App::scene->environment->setSaveExistingCalculationStructures(
            !App::scene->environment->getSaveExistingCalculationStructures());
    }
    if (cmd.cmdId == TOGGLE_SHAPETEXTURES_ENVIRONMENTGUITRIGGEREDCMD)
    {
        App::scene->environment->setShapeTexturesEnabled(
            !App::scene->environment->getShapeTexturesEnabled());
    }
    if (cmd.cmdId == TOGGLE_GLUITEXTURES_ENVIRONMENTGUITRIGGEREDCMD)
    {
        App::scene->environment->set2DElementTexturesEnabled(
            !App::scene->environment->get2DElementTexturesEnabled());
    }
    if (cmd.cmdId == TOGGLE_LOCKSCENE_ENVIRONMENTGUITRIGGEREDCMD)
    {
        App::scene->environment->setRequestFinalSave(!App::scene->environment->getRequestFinalSave());
    }
    if (cmd.cmdId == SET_ACKNOWLEDGMENT_ENVIRONMENTGUITRIGGEREDCMD)
    {
        App::scene->environment->setAcknowledgement(cmd.stringParams[0].c_str());
    }
    if (cmd.cmdId == CLEANUP_OBJNAMES_ENVIRONMENTGUITRIGGEREDCMD)
    {
        if (App::scene->simulation->isSimulationStopped())
            App::scene->cleanupHashNames_allObjects(-1);
    }
    if (cmd.cmdId == CLEANUP_GHOSTS_ENVIRONMENTGUITRIGGEREDCMD)
    {
        App::scene->ghostObjectCont_old->removeGhost(-1, -1);
    }
    if (cmd.cmdId == TOGGLE_ENABLED_FOGGUITRIGGEREDCMD)
    {
        App::scene->environment->setFogEnabled(!App::scene->environment->getFogEnabled());
    }
    if (cmd.cmdId == SET_TYPE_FOGGUITRIGGEREDCMD)
    {
        App::scene->environment->setFogType(cmd.intParams[0]);
    }
    if (cmd.cmdId == SET_START_FOGGUITRIGGEREDCMD)
    {
        App::scene->environment->setFogStart(cmd.doubleParams[0]);
    }
    if (cmd.cmdId == SET_END_FOGGUITRIGGEREDCMD)
    {
        App::scene->environment->setFogEnd(cmd.doubleParams[0]);
    }
    if (cmd.cmdId == SET_DENSITY_FOGGUITRIGGEREDCMD)
    {
        App::scene->environment->setFogDensity(cmd.doubleParams[0]);
    }
    ...
    if (cmd.cmdId == TOGGLE_TRANSPARENCY_MATERIALGUITRIGGEREDCMD)
    {
        ...
        CColorObject* it = GuiApp::getVisualParamPointerFromItem(cmd.intParams[0], cmd.intParams[1], cmd.intParams[2],
                                                                 nullptr, &allowedParts);
        CShape* shape = App::scene->sceneObjects->getShapeFromHandle(cmd.intParams[1]);
    ...
    {
        COcTree* it = App::scene->sceneObjects->getOctreeFromHandle(cmd.intParams[0]);
        if (it != nullptr)
    ...
    {
        COcTree* it = App::scene->sceneObjects->getLastSelectionOctree();
        App::scene->sceneObjects->popLastSelection();
        std::vector<int> sel;
        App::scene->sceneObjects->getSelectedObjectHandles(sel, -1, true, true);
        App::scene->sceneObjects->deselectObjects();
        App::logMsg(sim_verbosity_msgs, "Inserting objects into OC tree...");
        it->insertObjects(sel);
        App::scene->sceneObjects->addObjectToSelection(it->getObjectHandle());
        App::undoRedo_sceneChanged("");
    ...
    {
        CPointCloud* it = App::scene->sceneObjects->getPointCloudFromHandle(cmd.intParams[0]);
        if (it != nullptr)
            it->setCellSize(cmd.doubleParams[0]);
    }
    ...
    {
        CPointCloud* it = App::scene->sceneObjects->getLastSelectionPointCloud();
        App::scene->sceneObjects->popLastSelection();
        std::vector<int> sel;
        App::scene->sceneObjects->getSelectedObjectHandles(sel, -1, true, true);
        App::scene->sceneObjects->deselectObjects();
    ...
    {
        CForceSensor* it = App::scene->sceneObjects->getForceSensorFromHandle(cmd.intParams[0]);
        if (it != nullptr)
    ...
    if (cmd.cmdId == APPLY_VISUALPROP_FORCESENSORGUITRIGGEREDCMD)
    {
        ...
        if (it != nullptr)
        {
            for (size_t i = 1; i < cmd.intParams.size(); i++)
            {
                CForceSensor* it2 = App::scene->sceneObjects->getForceSensorFromHandle(cmd.intParams[i]);
                if (it2 != nullptr)
                {
    ...
    {
        CShape* it = App::scene->sceneObjects->getShapeFromHandle(cmd.intParams[0]);
        if (it != nullptr)
    ...
    {
        CShape* last = App::scene->sceneObjects->getShapeFromHandle(cmd.intParams[0]);
        if ((last != nullptr) && (!last->isCompound()))
        {
            for (size_t i = 1; i < cmd.intParams.size(); i++)
            {
                CShape* it = App::scene->sceneObjects->getShapeFromHandle(cmd.intParams[i]);
    ...
    if (cmd.cmdId == CLEAR_TEXTURES_SHAPEGUITRIGGEREDCMD)
    {
        for (size_t i = 0; i < cmd.intParams.size(); i++)
        {
            CShape* shape = App::scene->sceneObjects->getShapeFromHandle(cmd.intParams[i]);
            if (shape != nullptr)
            {
    ...
    if (cmd.cmdId == COMMAND_QUEUE_UITRIGGEREDCMD)
        App::scene->outsideCommandQueue_old->addCommand(cmd.intParams[0], cmd.intParams[1], cmd.intParams[2], cmd.intParams[3], cmd.intParams[4], nullptr, 0);
    ...
    {
        App::scene->environment->setActiveLayers(App::scene->environment->getActiveLayers() ^ cmd.intParams[0]);
    }
    if (cmd.cmdId == TOGGLE_SHOWDYNCONTENT_LAYERGUITRIGGEREDCMD)
    {
        App::scene->simulation->setDynamicContentVisualizationOnly(
            !App::scene->simulation->getDynamicContentVisualizationOnly());
    }

    if (cmd.cmdId == SET_ROLLEDUPSIZES_ROLLEDUPGUITRIGGEREDCMD)
    {
        CButtonBlock* it = App::scene->buttonBlockContainer_old->getBlockWithID(cmd.intParams[0]);
        if (it != nullptr)
        {
            VPoint s;
            s.x = cmd.intParams[1];
    ...
    {
        CJoint* last = App::scene->sceneObjects->getJointFromHandle(cmd.intParams[0]);
        if (last != nullptr)
        {
            for (size_t i = 1; i < cmd.intParams.size(); i++)
            {
                CJoint* it = App::scene->sceneObjects->getJointFromHandle(cmd.intParams[i]);
    ...
    if (cmd.cmdId == APPLY_ALLENGINEPARAMS_JOINTDYNGUITRIGGEREDCMD)
    {
        ...
        if (it != nullptr)
        {
            for (size_t i = 1; i < cmd.intParams.size(); i++)
            {
                CJoint* anotherJoint = App::scene->sceneObjects->getJointFromHandle(cmd.intParams[i]);
    ...
    {
        CPath_old* it = App::scene->sceneObjects->getPathFromHandle(cmd.intParams[0]);
    ...
    if (cmd.cmdId == DELETE_SCRIPT_SCRIPTGUITRIGGEREDCMD)
    {
        ...
        if (script != nullptr)
        {
            ...
            App::scene->sceneObjects->embeddedScriptContainer->removeScript(scriptID);
        }
    }
    ...
    if (cmd.cmdId == SET_ALL_SCRIPTSIMULPARAMETERGUITRIGGEREDCMD)
    {
        ...
        CSceneObject* it = App::scene->sceneObjects->getObjectFromHandle(objID);
        if (it != nullptr)
        {
    ...
    {
        CGraph* it = App::scene->sceneObjects->getGraphFromHandle(cmd.intParams[0]);
        if (it != nullptr)
            it->setExplicitHandling(!it->getExplicitHandling());
    }
    ...
    {
        CIkGroup_old* ikGroup = App::scene->ikGroups_old->getObjectFromHandle(cmd.intParams[0]);
        if (ikGroup != nullptr)
        {
    ...
    if (cmd.cmdId == ADD_ELEMENT_IKELEMENTGUITRIGGEREDCMD)
    {
        ...
        if (ikGroup != nullptr)
        {
            CDummy* it = App::scene->sceneObjects->getDummyFromHandle(cmd.intParams[1]);
            if (it != nullptr)
            {
    ...
    {
        App::scene->mainSettings_old->ikCalculationEnabled = !App::scene->mainSettings_old->ikCalculationEnabled;
    }
    if (cmd.cmdId == REMOVE_IKGROUP_IKGROUPGUITRIGGEREDCMD)
    {
        CIkGroup_old* it = App::scene->ikGroups_old->getObjectFromHandle(cmd.intParams[0]);
        if (it != nullptr)
    ...
    if (cmd.cmdId == ADD_IKGROUP_IKGROUPGUITRIGGEREDCMD)
    {
        ...
        App::scene->ikGroups_old->addIkGroup(newGroup, false);
        // Now select the object in the UI. We need to post it so that it arrives after the dialog refresh!:
        SSimulationThreadCommand cmd2;
        cmd2.cmdId = CALL_DIALOG_FUNCTION_GUITRIGGEREDCMD;
    ...
    if (cmd.cmdId == SHIFT_IKGROUP_IKGROUPGUITRIGGEREDCMD)
    {
        ...
        if (it != nullptr)
        {
            App::scene->ikGroups_old->shiftIkGroup(cmd.intParams[0], up);
            SSimulationThreadCommand cmd2;
            cmd2.cmdId = CALL_DIALOG_FUNCTION_GUITRIGGEREDCMD;
            cmd2.intParams.push_back(IK_DLG);
            cmd2.intParams.push_back(0);
    ...
    if (cmd.cmdId == SET_CONDITIONALPARAMS_IKGROUPGUITRIGGEREDCMD)
    {
        ...
        if (it != nullptr)
        {
            CIkGroup_old* it2 = App::scene->ikGroups_old->getObjectFromHandle(cmd.intParams[1]);
            int id = -1;
            if (it2 != nullptr)
    ...
    if (cmd.cmdId == PATHEDIT_MAKEDUMMY_GUITRIGGEREDCMD)
    {
        ...
        App::scene->sceneObjects->addObjectToScene(newDummy, false, true);
    ...
    {
        App::scenes->switchToScene(cmd.intParams[0]);
    }
    ...
    {
        App::scene->environment->modelThumbnail_notSerializedHere.setUncompressedThumbnailImage(
    ...
    if (cmd.cmdId == INVERT_SELECTION_GUITRIGGEREDCMD)
    {
        for (size_t i = 0; i < App::scene->sceneObjects->getObjectCount(); i++)
            App::scene->sceneObjects->xorAddObjectToSelection(
    ...
    // UNDO/REDO:
    //        if (cmd.cmdId==999995)
    //            App::scene->undoBufferContainer->emptyRedoBuffer();
    if (cmd.cmdId == 999996)
        App::scene->undoBufferContainer->announceChangeGradual();
    if (cmd.cmdId == 999997)
        App::scene->undoBufferContainer->announceChangeStart();
    if (cmd.cmdId == 999998)
        App::scene->undoBufferContainer->announceChangeEnd();
    if (cmd.cmdId == 999999)
        App::scene->undoBufferContainer->announceChange();
#endif
}
...
#endif
    if (CSimFlavor::getBoolVal(16) && (App::userSettings->autoSaveDelay > 0) && (!App::scene->environment->getSceneLocked()) && App::scene->simulation->isSimulationStopped() && noEditMode)
    {
        ...
        if (okToGoOn)
        {
            if (VDateTime::getSecondsSince1970() > (App::scene->environment->autoSaveLastSaveTimeInSecondsSince1970 + App::userSettings->autoSaveDelay * 60))
            {
                std::string savedLoc = App::scene->environment->getScenePathAndName();
                std::string testScene = App::folders->getAutoSavedScenesPath() + "/";
                testScene += std::to_string(App::scenes->getCurrentSceneIndex() + 1) + "-" + App::scene->environment->getUniquePersistentIdString() + ".";
                testScene += SIM_SCENE_EXTENSION;
                CFileOperations::saveScene(testScene.c_str(), false, false, nullptr, nullptr, nullptr, true);
                App::scene->environment->setScenePathAndName(savedLoc.c_str());
                App::scene->environment->autoSaveLastSaveTimeInSecondsSince1970 = VDateTime::getSecondsSince1970();
#ifdef SIM_WITH_GUI
...
void CSimThread::_handleClickRayIntersection_old(SSimulationThreadCommand cmd)
{
    ...
    CCamera* cam = App::scene->sceneObjects->getCameraFromHandle(cameraHandle);
    if (cam != nullptr)
    {
        std::vector<int> currentSelectionState(App::scene->sceneObjects->getSelectedObjectHandlesPtr()[0]);
        ...
        CProxSensor* prox = App::scene->sceneObjects->getProximitySensorFromHandle(psh);
        double dist = DBL_MAX;
        bool ptValid = false;
        for (size_t i = 0; i < App::scene->sceneObjects->getObjectCount(); i++)
        {
            CSceneObject* object = App::scene->sceneObjects->getObjectFromIndex(i);
        ...
        App::scene->outsideCommandQueue_old->addCommand(msg, obj, 0, 0, 0, ptdata, 6);
        App::scene->sceneObjects->setSelectedObjectHandles(currentSelectionState.data(), currentSelectionState.size());
    }
}
...
bool CSimThread::_idleSleepAndCheckIfRenderRequired()
{
    ...
    bool render = false;
    if (App::scene->simulation->getSimulationState() & sim_simulation_advancing)
    {
        frameCount++;
        render = (frameCount >= App::scene->simulation->getPassesPerRendering());
        if (render)
            frameCount = 0;
    }
    else
