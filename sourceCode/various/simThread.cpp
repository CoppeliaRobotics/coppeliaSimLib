...
//                it->setCheckOcclusions(cmd.boolParams[4]);
    ...
    {
        float* col=GuiApp::getRGBPointerFromItem(cmd.intParams[0],cmd.intParams[1],cmd.intParams[2],cmd.intParams[3],nullptr);
        if (col!=nullptr)
        {
    ...
    if (cmd.cmdId==SET_SHININESS_MATERIALGUITRIGGEREDCMD)
    {
        ...
        CColorObject* it=GuiApp::getVisualParamPointerFromItem(cmd.intParams[0],cmd.intParams[1],cmd.intParams[2],nullptr,&allowedParts);
    ...
    {
        CTextureProperty* it=GuiApp::getTexturePropertyPointerFromItem(cmd.intParams[0],cmd.intParams[1],cmd.intParams[2],nullptr,nullptr,nullptr,nullptr);
        if (it!=nullptr)
        {
    ...
    if (cmd.cmdId==LOAD_ANDAPPLY_TEXTUREGUITRIGGEREDCMD)
    {
        ...
        bool valid=false;
        bool is3D=false;
        GuiApp::getTexturePropertyPointerFromItem(cmd.intParams[0],cmd.intParams[1],cmd.intParams[2],nullptr,&is3D,&valid,&geom);
        if (valid)
        {
...
void CSimThread::_handleAutoSaveSceneCommand(SSimulationThreadCommand cmd)
{
    ...
    #endif
    if ( (!CSimFlavor::getBoolVal(15))&&CSimFlavor::getBoolVal(16)&&(App::userSettings->autoSaveDelay>0)&&(!App::currentWorld->environment->getSceneLocked())&&App::currentWorld->simulation->isSimulationStopped()&&noEditMode )
    {
