...
{   // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    int buffer=0;
    if (CPluginContainer::geomPlugin_getMeshPointDistanceIfSmaller(shape->geomData->collInfo,shape->getCumulativeTransformation(),dummyPos,dist,&rayPart0,&buffer))
    {
        rayPart0.copyTo(ray);
...
{   // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    if (CPluginContainer::geomPlugin_getMeshMeshDistanceIfSmaller(shape1->geomData->collInfo,shape1->getCumulativeTransformation(),shape2->geomData->collInfo,shape2->getCumulativeTransformation(),dist,&minDistPt1,&minDistPt2,cache1+1,cache2+1))
    {
...
{   // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    C3Vector dummyPos(dummy->getCumulativeTransformation().X);
    C3Vector minDistPt;
    if (CPluginContainer::geomPlugin_getOctreePointDistanceIfSmaller(octree->getOctreeInfo(),octree->getCumulativeTransformation(),dummyPos,dist,&minDistPt,&cacheV))
    {
...
{   // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    C3Vector distPt1;
    C3Vector distPt2;
    if (CPluginContainer::geomPlugin_getMeshOctreeDistanceIfSmaller(shape->geomData->collInfo,shape->getCumulativeTransformation(),octree->getOctreeInfo(),octree->getCumulativeTransformation(),dist,&distPt2,&distPt1,cache2+1,&cache1V))
    {
...
{   // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    C3Vector distPt1;
    C3Vector distPt2;
    if (CPluginContainer::geomPlugin_getOctreeOctreeDistanceIfSmaller(octree1->getOctreeInfo(),octree1->getCumulativeTransformation(),octree2->getOctreeInfo(),octree2->getCumulativeTransformation(),dist,&distPt1,&distPt2,&cache1V,&cache2V))
    {
        distPt1.getInternalData(ray+0);
        distPt2.getInternalData(ray+3);
        ray[6]=dist;
...
{   // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    C3Vector dummyPos(dummy->getCumulativeTransformation().X);
    C3Vector distPt;
    if (CPluginContainer::geomPlugin_getPtcloudPointDistanceIfSmaller(pointCloud->getPointCloudInfo(),pointCloud->getCumulativeTransformation(),dummyPos,dist,&distPt,&cacheV))
    {
...
{   // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    C3Vector distPt1;
    C3Vector distPt2;
    if (CPluginContainer::geomPlugin_getMeshPtcloudDistanceIfSmaller(shape->geomData->collInfo,shape->getCumulativeTransformation(),pointCloud->getPointCloudInfo(),pointCloud->getCumulativeTransformation(),dist,&distPt2,&distPt1,cache2+1,&cache1V))
    {
        distPt1.getInternalData(ray+0);
        distPt2.getInternalData(ray+3);
        ray[6]=dist;
...
{   // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    C3Vector distPt1;
    C3Vector distPt2;
    if (CPluginContainer::geomPlugin_getOctreePtcloudDistanceIfSmaller(octree->getOctreeInfo(),octree->getCumulativeTransformation(),pointCloud->getPointCloudInfo(),pointCloud->getCumulativeTransformation(),dist,&distPt1,&distPt2,&cache1V,&cache2V))
    {
        distPt1.getInternalData(ray+0);
        distPt2.getInternalData(ray+3);
        ray[6]=dist;
...
{   // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    C3Vector distPt1;
    C3Vector distPt2;
    if (CPluginContainer::geomPlugin_getPtcloudPtcloudDistanceIfSmaller(pointCloud1->getPointCloudInfo(),pointCloud1->getCumulativeTransformation(),pointCloud2->getPointCloudInfo(),pointCloud2->getCumulativeTransformation(),dist,&distPt1,&distPt2,&cache1V,&cache2V))
    {
        distPt1.getInternalData(ray+0);
        distPt2.getInternalData(ray+3);
        ray[6]=dist;
