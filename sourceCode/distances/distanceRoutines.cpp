...
bool CDistanceRoutine::getOctreesHaveCoherentMovement(COcTree *octree1, COcTree *octree2)
{
    ...
    for (size_t i = 0; i < _objectCoherency.size(); i++)
    {
        if ((_objectCoherency[i].object1Id == octree1->getObjectHandle()) &&
            (_objectCoherency[i].object2Id == octree2->getObjectHandle()))
        {
...
bool CDistanceRoutine::getDistanceBetweenEntitiesIfSmaller(int entity1ID, int entity2ID, double &dist, double ray[7],
    ...
    { // We have an object against...
        ...
        { // ...another object
            ...
            { // check against cached values:
                ...
                returnValue =
                    _getCachedDistanceIfSmaller(dist, ray, cache1, cache2, overrideMeasurableFlagIfNonCollection1,
                                                overrideMeasurableFlagIfNonCollection2, cachedPairWasProcessed) ||
                    returnValue;
                doTheCheck = !cachedPairWasProcessed;
            }
            if (doTheCheck)
                returnValue = _getObjectObjectDistanceIfSmaller(object1, object2, dist, ray, cache1, cache2,
                                                                overrideMeasurableFlagIfNonCollection1,
                                                                overrideMeasurableFlagIfNonCollection2) ||
                              returnValue;
        }
        else
        { // an objects VS a collection or all other objects
            ...
            if (group.size() != 0)
            {
                ...
                {                           // check against cached values:
                    bool cachedPairWasProcessed = false;
                    returnValue = _getCachedDistanceIfSmaller_pairs(
                                      pairs, dist, ray, cache1, cache2, overrideMeasurableFlagIfNonCollection1,
                                      overrideMeasurableFlagIfNonCollection2, cachedPairWasProcessed) ||
                                  returnValue;
                }
                returnValue = _getObjectPairsDistanceIfSmaller(pairs, dist, ray, cache1, cache2,
                                                               overrideMeasurableFlagIfNonCollection1, true) ||
                              returnValue;
            }
        }
    }
    ...
    { // We have a group against...
        ...
        if (group1.size() != 0)
        {
            ...
            { //...an object
                ...
                returnValue = _getObjectPairsDistanceIfSmaller(pairs, dist, ray, cache1, cache2, true,
                                                               overrideMeasurableFlagIfNonCollection2) ||
                              returnValue;
            }
            else
            { // ...another group or all other objects (entity2ID could be -1)
                ...
                if (group2.size() != 0)
                {
                    ...
                    returnValue = _getCachedDistanceIfSmaller_pairs(
                                  ...
                                  returnValue;
                    returnValue =
                        _getObjectPairsDistanceIfSmaller(pairs, dist, ray, cache1, cache2, true, true) || returnValue;
                }
            }
        }
    }
...
{ // the returned distance is always same or smaller than the real distance!
    ...
    { // pt vs ...
        if (isPt[1])
            ...
            return (App::worldContainer->pluginContainer->geomPlugin_getBoxPointDistance(m[1], halfSizes[1], true,
                                                                                         m[0].X)); // pt vs box
    }
    else
    { // box vs ...
        if (isPt[1])
            return (App::worldContainer->pluginContainer->geomPlugin_getBoxPointDistance(m[0], halfSizes[0], true,
                                                                                         m[1].X)); // box vs pt
        else
        {
            // box box is too slow here
            return (App::worldContainer->pluginContainer->geomPlugin_getApproxBoxBoxDistance(
...
bool CDistanceRoutine::_getDummyDummyDistanceIfSmaller(CDummy *dummy1, CDummy *dummy2, double &dist, double ray[7],
...
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    if (dummy1 == dummy2)
        return (false);
    if (((dummy1->getCumulativeObjectSpecialProperty() & sim_objectspecialproperty_measurable) == 0) &&
        (!overrideMeasurableFlagDummy1))
        return (false);
    if (((dummy2->getCumulativeObjectSpecialProperty() & sim_objectspecialproperty_measurable) == 0) &&
        (!overrideMeasurableFlagDummy2))
        return (false);
...
bool CDistanceRoutine::_getShapeDummyDistanceIfSmaller(CShape *shape, CDummy *dummy, double &dist, double ray[7],
...
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    if (((shape->getCumulativeObjectSpecialProperty() & sim_objectspecialproperty_measurable) == 0) &&
        (!overrideMeasurableFlagShape))
        return (false);
    if (((dummy->getCumulativeObjectSpecialProperty() & sim_objectspecialproperty_measurable) == 0) &&
        (!overrideMeasurableFlagDummy))
        return (false);
...
bool CDistanceRoutine::_getDummyShapeDistanceIfSmaller(CDummy *dummy, CShape *shape, double &dist, double ray[7],
...
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    double _ray[7];
    if (_getShapeDummyDistanceIfSmaller(shape, dummy, dist, _ray, cache2, cache1, overrideMeasurableFlagShape,
                                        overrideMeasurableFlagDummy))
    {
...
bool CDistanceRoutine::_getShapeShapeDistanceIfSmaller(CShape *shape1, CShape *shape2, double &dist, double ray[7],
...
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    if (shape1 == shape2)
        return (false);
    if (((shape1->getCumulativeObjectSpecialProperty() & sim_objectspecialproperty_measurable) == 0) &&
        (!overrideMeasurableFlagShape1))
        return (false);
    if (((shape2->getCumulativeObjectSpecialProperty() & sim_objectspecialproperty_measurable) == 0) &&
        (!overrideMeasurableFlagShape2))
        return (false);
    ...
    if (App::worldContainer->pluginContainer->geomPlugin_getMeshMeshDistanceIfSmaller(
...
bool CDistanceRoutine::_getOctreeDummyDistanceIfSmaller(COcTree *octree, CDummy *dummy, double &dist, double ray[7],
...
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    if (((octree->getCumulativeObjectSpecialProperty() & sim_objectspecialproperty_measurable) == 0) &&
        (!overrideMeasurableFlagOctree))
        return (false);
    if (((dummy->getCumulativeObjectSpecialProperty() & sim_objectspecialproperty_measurable) == 0) &&
        (!overrideMeasurableFlagDummy))
        return (false);
    ...
    C3Vector dummyPos(dummy->getFullCumulativeTransformation().X);
    C3Vector minDistPt;
    if (App::worldContainer->pluginContainer->geomPlugin_getOctreePointDistanceIfSmaller(
...
bool CDistanceRoutine::_getDummyOctreeDistanceIfSmaller(CDummy *dummy, COcTree *octree, double &dist, double ray[7],
                                                        int cache1[2], int cache2[2], bool overrideMeasurableFlagDummy,
...
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    double _ray[7];
    if (_getOctreeDummyDistanceIfSmaller(octree, dummy, dist, _ray, cache2, cache1, overrideMeasurableFlagOctree,
                                         overrideMeasurableFlagDummy))
    {
        _copyInvertedRay(_ray, ray);
        return (true);
...
bool CDistanceRoutine::_getOctreeShapeDistanceIfSmaller(COcTree *octree, CShape *shape, double &dist, double ray[7],
                                                        int cache1[2], int cache2[2], bool overrideMeasurableFlagOctree,
                                                        bool overrideMeasurableFlagShape)
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    // If the distance is bigger, 'dist' doesn't change and the return value is false
    ...
    C3Vector distPt1;
    C3Vector distPt2;
    if (App::worldContainer->pluginContainer->geomPlugin_getMeshOctreeDistanceIfSmaller(
...
bool CDistanceRoutine::_getShapeOctreeDistanceIfSmaller(CShape *shape, COcTree *octree, double &dist, double ray[7],
                                                        int cache1[2], int cache2[2], bool overrideMeasurableFlagShape,
                                                        bool overrideMeasurableFlagOctree)
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    // If the distance is bigger, 'dist' doesn't change and the return value is false
    ...
    double _ray[7];
    if (_getOctreeShapeDistanceIfSmaller(octree, shape, dist, _ray, cache2, cache1, overrideMeasurableFlagOctree,
                                         overrideMeasurableFlagShape))
    {
        _copyInvertedRay(_ray, ray);
        return (true);
...
bool CDistanceRoutine::_getOctreeOctreeDistanceIfSmaller(COcTree *octree1, COcTree *octree2, double &dist,
                                                         double ray[7], int cache1[2], int cache2[2],
                                                         bool overrideMeasurableFlagOctree1,
...
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    if (octree2->getOctreeInfo() == nullptr)
        return (false); // Octree is empty
    if (((octree1->getCumulativeObjectSpecialProperty() & sim_objectspecialproperty_measurable) == 0) &&
        (!overrideMeasurableFlagOctree1))
        return (false);
    if (((octree2->getCumulativeObjectSpecialProperty() & sim_objectspecialproperty_measurable) == 0) &&
        (!overrideMeasurableFlagOctree2))
        return (false);
    ...
    C3Vector distPt1;
    C3Vector distPt2;
    if (App::worldContainer->pluginContainer->geomPlugin_getOctreeOctreeDistanceIfSmaller(
...
bool CDistanceRoutine::_getPointCloudDummyDistanceIfSmaller(CPointCloud *pointCloud, CDummy *dummy, double &dist,
                                                            double ray[7], int cache1[2], int cache2[2],
...
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    if (((pointCloud->getCumulativeObjectSpecialProperty() & sim_objectspecialproperty_measurable) == 0) &&
        (!overrideMeasurableFlagPointCloud))
        return (false);
    if (((dummy->getCumulativeObjectSpecialProperty() & sim_objectspecialproperty_measurable) == 0) &&
        (!overrideMeasurableFlagDummy))
        return (false);
    ...
    C3Vector dummyPos(dummy->getFullCumulativeTransformation().X);
    C3Vector distPt;
    if (App::worldContainer->pluginContainer->geomPlugin_getPtcloudPointDistanceIfSmaller(
...
bool CDistanceRoutine::_getDummyPointCloudDistanceIfSmaller(CDummy *dummy, CPointCloud *pointCloud, double &dist,
                                                            double ray[7], int cache1[2], int cache2[2],
                                                            bool overrideMeasurableFlagDummy,
                                                            bool overrideMeasurableFlagPointCloud)
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    double _ray[7];
    if (_getPointCloudDummyDistanceIfSmaller(pointCloud, dummy, dist, _ray, cache2, cache1,
                                             overrideMeasurableFlagPointCloud, overrideMeasurableFlagDummy))
    {
        _copyInvertedRay(_ray, ray);
        return (true);
    }
...
bool CDistanceRoutine::_getPointCloudShapeDistanceIfSmaller(CPointCloud *pointCloud, CShape *shape, double &dist,
                                                            double ray[7], int cache1[2], int cache2[2],
                                                            bool overrideMeasurableFlagPointCloud,
                                                            bool overrideMeasurableFlagShape)
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    C3Vector distPt1;
    C3Vector distPt2;
    if (App::worldContainer->pluginContainer->geomPlugin_getMeshPtcloudDistanceIfSmaller(
...
bool CDistanceRoutine::_getShapePointCloudDistanceIfSmaller(CShape *shape, CPointCloud *pointCloud, double &dist,
                                                            double ray[7], int cache1[2], int cache2[2],
                                                            bool overrideMeasurableFlagShape,
                                                            bool overrideMeasurableFlagPointCloud)
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    double _ray[7];
    if (_getPointCloudShapeDistanceIfSmaller(pointCloud, shape, dist, _ray, cache2, cache1,
                                             overrideMeasurableFlagPointCloud, overrideMeasurableFlagShape))
    {
        _copyInvertedRay(_ray, ray);
        return (true);
    }
...
bool CDistanceRoutine::_getOctreePointCloudDistanceIfSmaller(COcTree *octree, CPointCloud *pointCloud, double &dist,
                                                             double ray[7], int cache1[2], int cache2[2],
                                                             bool overrideMeasurableFlagOctree,
                                                             bool overrideMeasurableFlagPointCloud)
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    C3Vector distPt1;
    C3Vector distPt2;
    if (App::worldContainer->pluginContainer->geomPlugin_getOctreePtcloudDistanceIfSmaller(
...
bool CDistanceRoutine::_getPointCloudOctreeDistanceIfSmaller(CPointCloud *pointCloud, COcTree *octree, double &dist,
                                                             double ray[7], int cache1[2], int cache2[2],
                                                             bool overrideMeasurableFlagPointCloud,
                                                             bool overrideMeasurableFlagOctree)
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    double _ray[7];
    if (_getOctreePointCloudDistanceIfSmaller(octree, pointCloud, dist, _ray, cache2, cache1,
                                              overrideMeasurableFlagOctree, overrideMeasurableFlagPointCloud))
    {
        _copyInvertedRay(_ray, ray);
        return (true);
    }
...
bool CDistanceRoutine::_getPointCloudPointCloudDistanceIfSmaller(CPointCloud *pointCloud1, CPointCloud *pointCloud2,
                                                                 double &dist, double ray[7], int cache1[2],
                                                                 int cache2[2], bool overrideMeasurableFlagPointCloud1,
...
{ // If the distance is smaller than 'dist', 'dist' is replaced and the return value is true
    ...
    if (pointCloud2->getPointCloudInfo() == nullptr)
        return (false); // Octree is empty
    if (((pointCloud1->getCumulativeObjectSpecialProperty() & sim_objectspecialproperty_measurable) == 0) &&
        (!overrideMeasurableFlagPointCloud1))
        return (false);
    if (((pointCloud2->getCumulativeObjectSpecialProperty() & sim_objectspecialproperty_measurable) == 0) &&
        (!overrideMeasurableFlagPointCloud2))
        return (false);
    ...
    C3Vector distPt1;
    C3Vector distPt2;
    if (App::worldContainer->pluginContainer->geomPlugin_getPtcloudPtcloudDistanceIfSmaller(
...
bool CDistanceRoutine::_getObjectPairsDistanceIfSmaller(const std::vector<CSceneObject *> &unorderedPairs, double &dist,
                                                        double ray[7], int cache1[2], int cache2[2],
                                                        ...
                                                        bool overrideMeasurableFlagObject2)
{
    ...
    for (size_t i = 0; i < pairs.size() / 2; i++)
        retVal = _getObjectObjectDistanceIfSmaller(pairs[2 * i + 0], pairs[2 * i + 1], dist, ray, cache1, cache2,
...
bool CDistanceRoutine::_getObjectObjectDistanceIfSmaller(CSceneObject *object1, CSceneObject *object2, double &dist,
                                                         double ray[7], int cache1[2], int cache2[2],
                                                         bool overrideMeasurableFlagObject1,
                                                         bool overrideMeasurableFlagObject2)
{
...
void CDistanceRoutine::_generateValidPairsFromObjectGroup(CSceneObject *obj, const std::vector<CSceneObject *> &group,
                                                          std::vector<CSceneObject *> &pairs)
{
...
void CDistanceRoutine::_generateValidPairsFromGroupObject(const std::vector<CSceneObject *> &group, CSceneObject *obj,
                                                          std::vector<CSceneObject *> &pairs)
{
    for (size_t i = 0; i < group.size(); i++)
    {
...
void CDistanceRoutine::_generateValidPairsFromGroupGroup(const std::vector<CSceneObject *> &group1,
                                                         const std::vector<CSceneObject *> &group2,
                                                         std::vector<CSceneObject *> &pairs,
                                                         bool collectionSelfDistanceCheck)
{
    for (size_t i = 0; i < group1.size(); i++)
    {
        ...
        for (size_t j = 0; j < group2.size(); j++)
        {
            ...
            if (obj1 != obj2)
            { // We never check an object against itself!
                if ((!collectionSelfDistanceCheck) ||
                    ((abs(csci1 - csci2) != 1) && (abs(csci1 - csci2) != 10) && (abs(csci1 - csci2) != 100) &&
                     (abs(csci1 - csci2) != 1000) && (abs(csci1 - csci2) != 10000) && (abs(csci1 - csci2) != 100000)))
...
bool CDistanceRoutine::_getCachedDistanceIfSmaller(double &dist, double ray[7], int cache1[2], int cache2[2],
                                                   bool overrideMeasurableFlagObject1,
...
{ // If the distance is smaller than 'dist', 'dist' is replaced and return is true
    ...
    bool retVal = _getObjectObjectDistanceIfSmaller(object1, object2, dist, ray, cache1, cache2,
                                                    overrideMeasurableFlagObject1, overrideMeasurableFlagObject2);
    cachedPairWasProcessed = true;
    return (retVal);
}

bool CDistanceRoutine::_getCachedDistanceIfSmaller_pairs(std::vector<CSceneObject *> &unorderedPairsCanBeModified,
...
{ // If the distance is smaller than 'dist', 'dist' is replaced and return is true
    ...
    for (size_t i = 0; i < unorderedPairsCanBeModified.size() / 2; i++)
    {
        ...
        { // we have found the pair!
            isSmaller = _getCachedDistanceIfSmaller(dist, ray, cache1, cache2, overrideMeasurableFlagObject1,
                                                    overrideMeasurableFlagObject2, cachedPairWasProcessed) ||
                        isSmaller;
            unorderedPairsCanBeModified.erase(unorderedPairsCanBeModified.begin() + 2 * i,
                                              unorderedPairsCanBeModified.begin() + 2 * i + 2);
            break;
        }
    }
