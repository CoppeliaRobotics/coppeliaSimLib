...
void CPathPointManipulation_old::addPathPointToSelection_nonEditMode(int pathPointIdentifier)
{
    ...
    if (pathPointIdentifier >= NON_OBJECT_PICKING_ID_PATH_PTS_START)
    {
        ...
        { // Ok, we have one path selected
            if (!isPathPointIndexSelected_nonEditMode(pathPointIdentifier - NON_OBJECT_PICKING_ID_PATH_PTS_START,
                                                      false))
                _selectedPathPointIndices_nonEditMode.push_back(pathPointIdentifier -
                                                                NON_OBJECT_PICKING_ID_PATH_PTS_START);
        }
    }
}
