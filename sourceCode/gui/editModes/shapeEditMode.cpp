                               ...
                               CTextureContainer *textureCont, CUiThread *uiThread)
{
    ...
    if (_editionTextureCoords.size() == 0)
        _editionTextureCoords.resize(_editionIndices.size() * 2,
                                     0.0); // when we work with no textures, we just use dummy texture coordinates
...
{ // return true means: select the edition object, otherwise it was erased
    ...
    if (!cancelChanges)
    {
        ...
        { // The shape is not empty
            ...
            {
                CMeshRoutines::removeDuplicateVerticesAndTriangles(_editionVertices, &_editionIndices, nullptr, nullptr,
...
{                                                       // Use only in edit mode!!! Solid is true by default
    ...
    if ((displayAttrib & sim_displayattribute_colorcodedpickpass) == 0)
    {
        ...
        if (displayAttrib & sim_displayattribute_renderpass)
        {
            ...
            glBegin(GL_TRIANGLES);
            for (int i = 0; i < int(_editionIndices.size()); i++)
                glVertex3d(_editionVertices[3 * _editionIndices[i] + 0], _editionVertices[3 * _editionIndices[i] + 1],
                           _editionVertices[3 * _editionIndices[i] + 2]);
            glEnd();
        }
...
double CShapeEditMode::getEdgeAngle(int edgeID) // all edit mode routines should go somewhere else!!!
{
    ...
    {
        int ind[3] = {_editionIndices[3 * i + 0], _editionIndices[3 * i + 1], _editionIndices[3 * i + 2]};
        if (((ev[0] == ind[0]) && (ev[1] == ind[1])) || ((ev[1] == ind[0]) && (ev[0] == ind[1])) ||
            ((ev[0] == ind[0]) && (ev[1] == ind[2])) || ((ev[1] == ind[0]) && (ev[0] == ind[2])) ||
            ((ev[0] == ind[1]) && (ev[1] == ind[2])) || ((ev[1] == ind[1]) && (ev[0] == ind[2])))
...
void CShapeEditMode::selectionFromVertexToTriangleEditMode(std::vector<int> *newTriangleSel)
{
    ...
    for (int i = 0; i < int(_editionIndices.size()) / 3; i++)
    {
        if (used[_editionIndices[3 * i + 0]] && used[_editionIndices[3 * i + 1]] && used[_editionIndices[3 * i + 2]])
        {
            if (newTriangleSel == nullptr)
...
void CShapeEditMode::selectionFromVertexToEdgeEditMode(std::vector<int> *newEdgeSel)
{
    ...
    for (int i = 0; i < int(_edgeCont.allEdges.size()) / 2; i++)
    {
        if (used[_edgeCont.allEdges[2 * i + 0]] && used[_edgeCont.allEdges[2 * i + 1]])
        {
            if (newEdgeSel == nullptr)
                editModeBuffer.push_back(i);
            else
...
void CShapeEditMode::selectionFromEdgeToTriangleEditMode(std::vector<int> *newTriangleSel)
{
    ...
    {
        if (used[_editionIndicesToEdgesIndex[3 * i + 0]] && used[_editionIndicesToEdgesIndex[3 * i + 1]] &&
        ...
        if (!alreadyInEditModeBuffer(item))
        {
            ...
            while (automaticallyFollowEdges && (_editModeType & EDGE_EDIT_MODE) && (!disableEdgeFollowing))
            {
                ...
                for (int i = 0; i < int(nextEdges.size()); i++)
                {
                    ...
                    if ((a > highestAngleValue) && (a2 < edgeDirectionChangeMaxAngle) &&
                        (a > (piValue - edgeMaxAngle))) // a>20.0... added on 2009/05/08
                    {
                        highestAngleValue = a;
                        highestAngleIndex = i;
                        foundValid = true;
...
void CShapeEditMode::copySelection(std::vector<int> *selection)
{
    ...
    { // We have to copy the selected triangles with their vertices
        copySelectedFaces(selection, &editBufferVerticesCopy, &editBufferIndicesCopy, &editBufferNormalsCopy,
...
void CShapeEditMode::deleteSelection(std::vector<int> *selection)
{
    if (_editModeType & TRIANGLE_EDIT_MODE)
    {
        ...
        { // We have to remove the triangles from the highest index to the lowest:
            ...
            _editionTextureCoords.erase(_editionTextureCoords.begin() + 6 * max,
                                        _editionTextureCoords.begin() + 6 * max + 6);
    ...
    if (_editModeType & VERTEX_EDIT_MODE)
    {
        ...
        {
            if ((_editionIndices[3 * i + 0] == -1) || (_editionIndices[3 * i + 1] == -1) ||
                (_editionIndices[3 * i + 2] == -1))
            {
                ...
                _editionTextureCoords.erase(_editionTextureCoords.begin() + 6 * i,
                                            _editionTextureCoords.begin() + 6 * i + 6);
            }
            else
                i++;
        }
    ...
    if (_editModeType & EDGE_EDIT_MODE)
    {
        ...
        for (int i = int((_editionIndices.size()) / 3) - 1; i >= 0; i--)
        {
            ...
            if (edgesWhichNeedToBeRemoved[ind[0]] || edgesWhichNeedToBeRemoved[ind[1]] ||
...
void CShapeEditMode::addMenu(VMenu *menu)
{
    ...
    menu->appendMenuItem(true, triangleEditMode, SHAPE_EDIT_MODE_SWAP_TO_TRIANGLE_EDIT_MODE_EMCMD,
                         IDS_TRIANGLE_EDIT_MODE_MENU_ITEM, true);
    menu->appendMenuItem(true, vertexEditMode, SHAPE_EDIT_MODE_SWAP_TO_VERTEX_EDIT_MODE_EMCMD,
                         IDS_VERTEX_EDIT_MODE_MENU_ITEM, true);
    menu->appendMenuItem(true, edgeEditMode, SHAPE_EDIT_MODE_SWAP_TO_EDGE_EDIT_MODE_EMCMD, IDS_EDGE_EDIT_MODE_MENU_ITEM,
                         true);
    ...
    if (_editModeType & VERTEX_EDIT_MODE)
    {
        menu->appendMenuItem(true, showHiddenVerticeAndEdges, SHAPE_EDIT_MODE_TOGGLE_HIDDEN_VERTICE_EDGE_SHOW_EMCMD,
                             IDS_SHOW_HIDDEN_VERTICES_MENU_ITEM, true);

        menu->appendMenuSeparator();

        menu->appendMenuItem(selSize > 0, false, SHAPE_EDIT_MODE_TRIANGLE_VERTEX_COPY_EMCMD,
                             IDS_COPY_SELECTED_VERTICES_MENU_ITEM);
        menu->appendMenuItem(buffSize > 0, false, SHAPE_EDIT_MODE_PASTE_TRIANGLE_VERTEX_EMCMD,
                             IDS_PASTE_VERTICES_MENU_ITEM);
        menu->appendMenuItem(selSize > 0, false, SHAPE_EDIT_MODE_DELETE_TRIANGLE_VERTEX_EDGE_EMCMD,
                             IDS_DELETE_SELECTED_VERTICES_MENU_ITEM);
        menu->appendMenuItem(selSize > 0, false, SHAPE_EDIT_MODE_TRIANGLE_VERTEX_CUT_EMCMD,
                             IDS_CUT_SELECTED_VERTICES_MENU_ITEM);

        menu->appendMenuSeparator();

        menu->appendMenuItem(selSize >= 3, false, SHAPE_EDIT_MODE_INSERT_TRIANGLES_EMCMD,
                             IDS_INSERT_TRIANGLES_MENU_ITEM);
        menu->appendMenuItem(selSize >= 3, false, SHAPE_EDIT_MODE_INSERT_TRIANGLE_FAN_EMCMD,
                             IDS_INSERT_TRIANGLE_FAN_MENU_ITEM);
        menu->appendMenuItem(selSize > 0, false, SHAPE_EDIT_MODE_MAKE_DUMMIES_WITH_SELECTED_VERTICES_EMCMD,
                             IDS_MAKE_DUMMIES_WITH_SELECTED_VERTICES_MENU_ITEM);
    ...
    {
        menu->appendMenuItem(selSize > 0, false, SHAPE_EDIT_MODE_TRIANGLE_VERTEX_COPY_EMCMD,
                             IDS_COPY_SELECTED_TRIANGLES_MENU_ITEM);
        menu->appendMenuItem(buffSize > 0, false, SHAPE_EDIT_MODE_PASTE_TRIANGLE_VERTEX_EMCMD,
                             IDS_PASTE_TRIANGLES_MENU_ITEM);
        menu->appendMenuItem(selSize > 0, false, SHAPE_EDIT_MODE_DELETE_TRIANGLE_VERTEX_EDGE_EMCMD,
                             IDS_DELETE_SELECTED_TRIANGLES_MENU_ITEM);
        menu->appendMenuItem(selSize > 0, false, SHAPE_EDIT_MODE_TRIANGLE_VERTEX_CUT_EMCMD,
                             IDS_CUT_SELECTED_TRIANGLES_MENU_ITEM);

        menu->appendMenuSeparator();

        menu->appendMenuItem(selSize > 0, false, SHAPE_EDIT_MODE_FLIP_TRIANGLES_SIDE_EMCMD,
                             IDS_FLIP_SELECTED_TRIANGLES_MENU_ITEM);
        menu->appendMenuItem(selSize > 0, false, SHAPE_EDIT_MODE_MAKE_SHAPE_WITH_SELECTED_TRIANGLES_EMCMD,
                             IDS_MAKE_SHAPE_WITH_SELECTED_TRIANGLES_MENU_ITEM);
        menu->appendMenuItem(selSize > 0, false, SHAPE_EDIT_MODE_MAKE_RECTANGLE_WITH_SELECTED_TRIANGLES_EMCMD,
                             IDS_MAKE_RECTANGLE_WITH_SELECTED_TRIANGLES_MENU_ITEM);
        menu->appendMenuItem(selSize > 0, false, SHAPE_EDIT_MODE_MAKE_SPHERE_WITH_SELECTED_TRIANGLES_EMCMD,
                             IDS_MAKE_SPHERE_WITH_SELECTED_TRIANGLES_MENU_ITEM);
        menu->appendMenuItem(selSize > 0, false, SHAPE_EDIT_MODE_MAKE_CYLINDER_WITH_SELECTED_TRIANGLES_EMCMD,
                             IDS_MAKE_CYLINDER_WITH_SELECTED_TRIANGLES_MENU_ITEM);
        menu->appendMenuItem(true, false, SHAPE_EDIT_MODE_DIVIDE_LARGEST_TRIANGLES_EMCMD,
                             IDS_REDUCE_TRIANGLE_SIZE_MENU_ITEM);
    ...
    {
        menu->appendMenuItem(true, showHiddenVerticeAndEdges, SHAPE_EDIT_MODE_TOGGLE_HIDDEN_VERTICE_EDGE_SHOW_EMCMD,
                             IDS_SHOW_HIDDEN_EDGES_MENU_ITEM, true);
        menu->appendMenuItem(true, automaticallyFollowEdges, SHAPE_EDIT_MODE_TOGGLE_AUTOMATIC_EDGE_FOLLOWING_EMCMD,
                             IDS_AUTOMATIC_EDGE_FOLLOWING_MENU_ITEM, true);

        menu->appendMenuSeparator();

        menu->appendMenuItem(selSize > 0, false, SHAPE_EDIT_MODE_DELETE_TRIANGLE_VERTEX_EDGE_EMCMD,
                             IDS_DELETE_SELECTED_EDGES_MENU_ITEM);
...
void CShapeEditMode::subdivideTriangles()
{
    ...
    {
        deselectEditModeBuffer();
        CMeshManip::reduceTriangleSize(_editionVertices, _editionIndices, &_editionNormals, &_editionTextureCoords,
                                       0.0); // 0.0 simple uses the half of the max edge!
        actualizeEditModeEditionEdges();
    }
}
...
void CShapeEditMode::makeDummies()
{
    ...
    for (size_t i = 0; i < relPathPts.size() / 3; i++)
    {
        ...
        {
            txt += utils::getPosString(false, relPathPts[3 * i + j]);
    ...
    for (size_t i = 0; i < relPathPts.size() / 3; i++)
    {
        ...
        for (size_t j = 0; j < 3; j++)
        {
            ...
            v = tr * v;
            txt += utils::getPosString(false, v(j));
            if ((j != 2) || (i < (relPathPts.size() / 3) - 1))
                txt += ",";
        }
        //            txt+="}";
    ...
    { // Only consecutive edges will be used! NOOOO! (modified on 2009/03/14)
        ...
        else
        {
            ...
            for (int i = 1; i < int(sel.size()); i++)
            {
                ...
                // We have to add the vertex that wasn't added yet. If both weren't added yet, we add the closest one
