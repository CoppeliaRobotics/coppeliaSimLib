                                                ...
                                                const char* extension8, const char* extension9, const char* extension10)
{
    ...
    // but only partially processed (e.g.
    // App::qtApp->processEvents(QEventLoop::ExcludeUserInputEvents|QEventLoop::ExcludeSocketNotifiers)). A quick/dirty
    // fix is tweak1 below. The full fix is to process all messages while waiting for a lock, which is done now by
    // default.
    ...
    {
        if (open)
            retString = QFileDialog::getOpenFileName(parent, title, pathAndOptionalFilename, filter.c_str(), 0,
                                                     QFileDialog::DontUseNativeDialog);
        else
            retString = QFileDialog::getSaveFileName(parent, title, pathAndOptionalFilename, filter.c_str(), 0,
                                                     QFileDialog::DontUseNativeDialog);
    }
    else
    {
    ...
    if ((!open) && (std::string(extension1).compare("*") != 0) && (strlen(extension1) != 0) &&
        (strlen(extension2) == 0) && (rs.size() > 0))
    {
                                   ...
                                   const char* extension8, const char* extension9, const char* extension10)
{
    ...
    // we use the non-static version on those platforms! The "show()/hide()" below might be important, otherwise we have
    // the same problem as with the static version of the functions!
    QFileDialog dlg(parent, title, pathAndOptionalFilename, filter.c_str());
    dlg.setNameFilter(filter.c_str());
...
#else
    if (!native)
        qfiles = QFileDialog::getOpenFileNames(parent, title, pathAndOptionalFilename, filter.c_str(), 0,
                                               QFileDialog::DontUseNativeDialog);
    else
