...
void displayEmitterOrReceiver(CBroadcastDataVisual *it)
{
    if (it->_emitter)
    {
        ...
        double distances[11] = {0.0,
                                ...
                                0.00215 * it->_actionRadius,
                                0.00462 * it->_actionRadius,
                                0.01 * it->_actionRadius,
                                0.0215 * it->_actionRadius,
                                0.0462 * it->_actionRadius,
                                0.1 * it->_actionRadius,
                                0.215 * it->_actionRadius,
            ...
            for (int j = 0; j < horizSubdiv; j++)
            {
                ...
                for (int k = 0; k < vertSubdiv; k++)
                {
                    ...
                    { // Not 360 degrees. We have to close the edges:
                        ...
                        ogl::addBuffer3DPoints(d1 * cos(a) * cos(b + dB), d1 * cos(a) * sin(b + dB), d1 * sin(a));
                        ogl::addBuffer3DPoints(d1 * cos(a) * cos(b + dB), d1 * cos(a) * sin(b + dB), d1 * sin(a));
                        ogl::addBuffer3DPoints(d1 * cos(a + dA) * cos(b + dB), d1 * cos(a + dA) * sin(b + dB),
                                               d1 * sin(a + dA));
                    }
                    a += dA;
                }
