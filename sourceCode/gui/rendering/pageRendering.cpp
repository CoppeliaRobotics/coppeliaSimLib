...
void displayPage(CSPage *page, int auxViewResizingAction, int viewIndexOfResizingAction)
{
    ...
    for (size_t i = page->getRegularViewCount(); i < page->getViewCount(); i++)
    {
        ...
        // int
        // avs[2]={int(_allViewAuxSizesAndPos[4*i+0]*double(_pageSize[0])),int(_allViewAuxSizesAndPos[4*i+1]*double(_pageSize[1]))};
        // int
        // avp[2]={int(_allViewAuxSizesAndPos[4*i+2]*double(_pageSize[0])),int(_allViewAuxSizesAndPos[4*i+3]*double(_pageSize[1]))};
        double ss_[2];
        double pp_[2];
        ...
        VPoint pos(_pageSize[0] - avp[0] - avs[0] / 2 - auxViewsBtSize * GuiApp::sc / 2,
                   _pageSize[1] - avp[1] - auxViewsBtSize * GuiApp::sc / 2);
        ...
        ogl::drawButton(pos, size, txtCol, bkgrndCol, bkgrndCol, name, buttonAttrib, false, 0, 0.0, false, 0, nullptr,
