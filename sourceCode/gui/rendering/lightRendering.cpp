...
void displayLight(CLight *light, CViewableBase *renderingObject, int displayAttrib)
{
    ...
    if (light->getShouldObjectBeDisplayed(renderingObject->getObjectHandle(), displayAttrib))
    {
        ...
        float m = std::max<float>(std::max<float>(normalizedAmbientColor(0), normalizedAmbientColor(1)),
                                  normalizedAmbientColor(2));
        if (m > 0.00001)
            normalizedAmbientColor /= m;
        C3Vector normalizedDiffuseColor;
        normalizedDiffuseColor.setData(light->getColor(true)->getColorsPtr() + 3);
        m = std::max<float>(std::max<float>(normalizedDiffuseColor(0), normalizedDiffuseColor(1)),
                            normalizedDiffuseColor(2));
        if (m > 0.00001)
        ...
        m = std::max<float>(std::max<float>(normalizedSpecularColor(0), normalizedSpecularColor(1)),
                            normalizedSpecularColor(2));
        if (m > 0.00001)
            normalizedSpecularColor /= m;
        normalizedAmbientColor += normalizedDiffuseColor * 0.2 + normalizedSpecularColor * 0.1;
        m = std::max<float>(std::max<float>(normalizedAmbientColor(0), normalizedAmbientColor(1)),
                            normalizedAmbientColor(2));
        if (m > 0.00001)
            normalizedAmbientColor /= m;
