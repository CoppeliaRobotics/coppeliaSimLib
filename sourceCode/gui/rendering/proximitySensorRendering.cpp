...
void displayProximitySensor(CProxSensor *proxSensor, CViewableBase *renderingObject, int displayAttrib)
{
    ...
    C3Vector normalVectorForLinesAndPoints(proxSensor->getFullCumulativeTransformation().Q.getInverse() *
    ...
    if (proxSensor->getShouldObjectBeDisplayed(renderingObject->getObjectHandle(), displayAttrib))
    {
            ...
            if (!proxSensor->getHideDetectionRay())
            {
                    ...
                    proxSensor->getRandomizedDetection())
                {
                    ...
                    for (size_t i = 0; i < _randomizedVectors.size(); i++)
                    {
                        ...
                        {
                            ogl::addBuffer3DPoints(0.0, 0.0, 0.0);
                            ogl::addBuffer3DPoints(_randomizedVectors[i](0) * _randomizedVectorDetectionStates[i],
                                                   _randomizedVectors[i](1) * _randomizedVectorDetectionStates[i],
                                                   _randomizedVectors[i](2) * _randomizedVectorDetectionStates[i]);
                        }
                    }
                    ogl::drawRandom3dLines(&ogl::buffer[0], 2, false, normalVectorForLinesAndPoints.data);
                    ogl::buffer.clear();
        ...
        if (proxSensor->getShowVolume())
        {
            if (proxSensor->convexVolume->volumeEdges.size() != 0)
                ogl::drawRandom3dLines(&proxSensor->convexVolume->volumeEdges[0],
                                       (int)proxSensor->convexVolume->volumeEdges.size() / 3, false,
            ...
            if (proxSensor->convexVolume->nonDetectingVolumeEdges.size() != 0)
                ogl::drawRandom3dLines(&proxSensor->convexVolume->nonDetectingVolumeEdges[0],
                                       (int)proxSensor->convexVolume->nonDetectingVolumeEdges.size() / 3, false,
                                       normalVectorForLinesAndPoints.data);
        }

        ogl::setMaterialColor(ogl::colorBlack, ogl::colorBlack, ogl::colorRed);
        if (proxSensor->convexVolume->normalsInside.size() != 0)
            ogl::drawRandom3dLines(&proxSensor->convexVolume->normalsInside[0],
