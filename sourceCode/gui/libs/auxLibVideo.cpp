...
{
    bool retVal = false;
    std::string vidLibPathAndName(App::getApplicationDir());
    vidLibPathAndName += "/";
#ifdef WIN_SIM
...
bool CAuxLibVideo::_getAuxLibProcAddresses()
{
    ...
    if (video_recorderGetEncoderString == 0)
        return false;
    if (video_recorderInitialize == 0)
        return false;
    if (video_recorderAddFrame == 0)
        return false;
    if (video_recorderEnd == 0)
        return false;
    return true;
}
