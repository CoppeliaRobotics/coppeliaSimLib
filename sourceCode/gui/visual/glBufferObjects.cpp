...
{ // textureCoords can be nullptr, in which case texCoordBufferId can also be nullptr
...
#ifdef SIM_WITH_GUI
    forceNotUsingBuffers = (App::userSettings->vboOperation == 0) ||
                           (GuiApp::mainWindow == nullptr); // in headless mode: we don't use VBO's for now (crash)
#endif
    ...
    {
        if (VDateTime::getTimeDiffInMs(lastTimeInMs, currentTimeInMs) >
            _maxTimeInMsBeforeBufferRemoval - 1000) // we haven't rendered a mesh since a while. Modal dlg?
