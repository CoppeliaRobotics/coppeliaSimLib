...
SOglFont ogl::oglFonts[] = {{"MS Sans Serif", 16, 0, {}}, {"courier new", 16, 0, {}}, {"", 0, 0, {}}};
...
{ // Color rgb is merged with color rgb2 according to weight (0.0=no merging, 1.0=full merging)
    setMaterialColor(colorMode, rgb[0] + (rgb2[0] - rgb[0]) * weight, rgb[1] + (rgb2[1] - rgb[1]) * weight,
                     rgb[2] + (rgb2[2] - rgb[2]) * weight);
}
...
void ogl::drawBox(double x, double y, double z, bool solid, const double normalVectorForDiffuseComp[3])
{
    ...
    {
        if (normalVectorForDiffuseComp != nullptr)
            glNormal3f((float)normalVectorForDiffuseComp[0], (float)normalVectorForDiffuseComp[1],
                       (float)normalVectorForDiffuseComp[2]);
        else
            glNormal3f(0.0, 0.0, 1.0);
        glBegin(GL_LINE_STRIP);
        glVertex3f(-dx, -dy, -dz);
...
void ogl::drawSphere(double rr, int sides, int faces, bool smooth)
{
    ...
    for (int i = 0; i < sides; i++)
    {
        for (int j = 0; j < faces; j++)
        {
            ...
            {
                glNormal3f((float)(sin((j + 0.5) * fa) * cos((i + 0.5) * sa)), (float)cos((j + 0.5) * fa),
    ...
    while (separateWords.size() != 0)
    {
        ...
        { // We are allowed to cut words
            ...
            { // The whole word doesn't fit
                if (width + spaceWidth + oglFonts[fontIndex].fontWidths[(unsigned char)separateWords[0][0]] <=
                    pixelWidth)
...
void ogl::loadBitmapFont(const unsigned char *fontData, int fontHeight, int theFontIndex)
{
    ...
    while (letter < 32 + 96)
    {
        ...
        for (int y = 0; y < sizeY; y++)
        {
            ...
            {
                temp[y * 4 + x] =
                    fontData[p++]; // we use a 4 byte alignement because of MS error in the openGl implementation
            }
        }
...
void ogl::loadOutlineFont(const int *fontData, const float *fontVertices)
{
    ...
    for (int letter = 32; letter < 32 + 96; letter++)
    {
        ...
        outlineFontSizes[letter] =
            (double(fontData[p++]) / 1000.0) + 0.154 * 0.6; // 0.0.154 corresponds to the width of the "I"
...
{ // Draws a frame. borderSize is 3 by default
    ...
    else
    {
        ...
        if (type == 3)
        {
            ...
            drawBorder(posX + borderSize, posY + borderSize, sizeX - 2 * borderSize, sizeY - 2 * borderSize, col, 1,
                       borderSize);
        }
    }
}
            ...
            { // Yes!
                ...
                if (bufferedText.length() != 0)
                {
                    ...
                    { // We have to draw the text twice!
                        ...
                        glRasterPos3i(textP + 1, -1 + pos[1] - oglFonts[fontIndex].fontHeight / 4,
                                      0); // we have a small vertical correction
                        glPushAttrib(GL_LIST_BIT);
                        glListBase(oglFonts[fontIndex].fontBase);
                    ...
                    glRasterPos3i(textP, pos[1] - oglFonts[fontIndex].fontHeight / 4,
                                  0); // we have a small vertical correction
                    glPushAttrib(GL_LIST_BIT);
                    glListBase(oglFonts[fontIndex].fontBase);
                    glCallLists((GLsizei)bufferedText.length(), GL_UNSIGNED_BYTE, bufferedText.c_str());
                ...
                {                                         // This is a pseudo-icon: we just wanna change color!
                    ...
                    {
                        float tc[3] = {float(iconsAndPos[iconIndex + 2]) / 9.0f,
                                       float(iconsAndPos[iconIndex + 3]) / 9.0f,
                                       float(iconsAndPos[iconIndex + 4]) / 9.0f};
                        setRichTextColor(tc);
                    }
                    iconIndex += 5;
                }
...
int ogl::getRichTextInfo(std::string &text, std::vector<int> &iconsAndPos)
{
    ...
    while (i != std::string::npos)
    {
        ...
        if (contin && (text.compare(i, 4, "&&fg") == 0))
        {
            ...
            iconsAndPos.push_back(
                COLOR_AND_COLOR_RESET_RICH_TEXT);    // exception: this indicates color change, or color reset (fb@@@)!
...
{ // If sliderPos is >2.0, there's no slider. Else, sliderPos should be between -1.0 and 1.0
    ...
    if ((buttonTypeAndAttributes & sim_buttonproperty_transparent) && (backgroundTexture == nullptr) &&
    ...
    setBlending(false);
    if (((buttonTypeAndAttributes & 0x0007) == sim_buttonproperty_button) &&
        ((buttonTypeAndAttributes & sim_buttonproperty_borderless) == 0) &&
        ...
        ((buttonTypeAndAttributes & 0x0007) == sim_buttonproperty_label))
    {
        ...
        { // Normal text
            ...
            if (s.y > s.x)
                ssi = s.x; // 8*s.x/10;
            if ((lines.size() > 1) || ((buttonTypeAndAttributes & sim_buttonproperty_horizontallycentered) == 0) ||
                ((buttonTypeAndAttributes & sim_buttonproperty_verticallycentered) == 0))
                ssi = oglFonts[fontIndex].fontHeight; // We have more than one line of text or the line is aligned at
                                                      // the top or left. We switch to small icons
    ...
    { // Here we have to draw a slider. The slider button has to be 20% of the
        ...
        int dbs[2] = {int(double(size[0]) * BUTTON_SLIDER_X_SIZE / 2.0),
                      int(double(size[1]) * BUTTON_SLIDER_Y_SIZE / 2.0)};
        if (vertical)
        {
    ...
    { // Here we have to draw an edit-box:
        size[0] = s.x + 2;
        size[1] = s.y + 2;
        if (((buttonTypeAndAttributes & sim_buttonproperty_borderless) == 0) &&
            ((buttonTypeAndAttributes & sim_buttonproperty_nobackgroundcolor) == 0))
        {
            glLineWidth(2.0);
