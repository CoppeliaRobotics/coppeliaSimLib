...
void GuiApp::runGui(int options)
{
    ...
    App::scenes->pluginContainer->unloadLegacyPlugins();
...
{ // auxDlgTitle can be nullptr
    ...
    if (objType == COLOR_ID_AMBIENT_LIGHT)
    {
        ...
        return (App::scene->environment->ambientLightColor);
    }
    if (objType == COLOR_ID_BACKGROUND_UP)
    {
        _auxDlgTitle->assign("Background (up)");
        return (App::scene->environment->backGroundColor);
    }
    if (objType == COLOR_ID_BACKGROUND_DOWN)
    {
        _auxDlgTitle->assign("Background (down)");
        return (App::scene->environment->backGroundColorDown);
    }
    if (objType == COLOR_ID_FOG)
    {
        _auxDlgTitle->assign("Fog");
        return (App::scene->environment->fogBackgroundColor);
    }
    if (objType == COLOR_ID_MIRROR)
    {
        _auxDlgTitle->assign("Mirror");
        CMirror* it = App::scene->sceneObjects->getMirrorFromHandle(objID1);
    ...
    if (objType == COLOR_ID_OCTREE)
    {
        ...
        COcTree* it = App::scene->sceneObjects->getOctreeFromHandle(objID1);
        if (it != nullptr)
    ...
    if (objType == COLOR_ID_POINTCLOUD)
    {
        ...
        CPointCloud* it = App::scene->sceneObjects->getPointCloudFromHandle(objID1);
        if (it != nullptr)
            return (it->getColor()->getColorsPtr());
    }
    if (objType == COLOR_ID_GRAPH_2DCURVE)
    {
        ...
        CGraph* it = App::scene->sceneObjects->getGraphFromHandle(objID1);
        if (it != nullptr)
        {
        ...
        (objType == COLOR_ID_OPENGLBUTTON_TEXT))
    {
        ...
        CButtonBlock* block = App::scene->buttonBlockContainer_old->getBlockWithID(objID1);
        if (block != nullptr)
        {
...
{ // auxDlgTitle and allowedParts can be nullptr. Bit-coded: 1=ambient/diffuse, 2=diffuse(light only), 4=spec, 8=emiss.,
    ...
    if (objType == COLOR_ID_CAMERA_A)
    {
        ...
        CCamera* it = App::scene->sceneObjects->getCameraFromHandle(objID1);
        if (it != nullptr)
    ...
    if (objType == COLOR_ID_FORCESENSOR_A)
    {
        ...
        _allowedParts[0] = 1 + 4 + 8 + 16 + 32 + 64;
        CForceSensor* it = App::scene->sceneObjects->getForceSensorFromHandle(objID1);
        if (it != nullptr)
            return (it->getColor(false));
    }
    if (objType == COLOR_ID_JOINT_A)
    {
        ...
        _allowedParts[0] = 1 + 4 + 8 + 16 + 32 + 64;
        CJoint* it = App::scene->sceneObjects->getJointFromHandle(objID1);
        if (it != nullptr)
    ...
    if (objType == COLOR_ID_PATH)
    {
        ...
        _allowedParts[0] = 1 + 4 + 8 + 16 + 32 + 64;
        CPath_old* it = App::scene->sceneObjects->getPathFromHandle(objID1);
    ...
    if (objType == COLOR_ID_COLLISIONCONTOUR)
    {
        ...
        _allowedParts[0] = 1 + 4 + 8 + 16 + 32 + 64;
        CCollisionObject_old* it = App::scene->collisions_old->getObjectFromHandle(objID1);
        if (it != nullptr)
    ...
    if (objType == COLOR_ID_DISTANCESEGMENT)
    {
        ...
        _allowedParts[0] = 1 + 4 + 8 + 16 + 32 + 64;
        CDistanceObject_old* it = App::scene->distances_old->getObjectFromHandle(objID1);
        if (it != nullptr)
    ...
    if (objType == COLOR_ID_LIGHT_CASING)
    {
        ...
        CLight* it = App::scene->sceneObjects->getLightFromHandle(objID1);
        if (it != nullptr)
            return (it->getColor(false));
    }
    ...
    if (objType == COLOR_ID_DUMMY)
    {
        ...
        _allowedParts[0] = 1 + 4 + 8 + 16 + 32 + 64;
        CDummy* it = App::scene->sceneObjects->getDummyFromHandle(objID1);
        if (it != nullptr)
    ...
    if (objType == COLOR_ID_VISIONSENSOR)
    {
        ...
        CVisionSensor* it = App::scene->sceneObjects->getVisionSensorFromHandle(objID1);
        if (it != nullptr)
            return (it->getColor());
    }
    if (objType == COLOR_ID_PROXSENSOR_VOLUME)
    {
        ...
        _allowedParts[0] = 1 + 4 + 8 + 16 + 32;
        CProxSensor* it = App::scene->sceneObjects->getProximitySensorFromHandle(objID1);
        if (it != nullptr)
    ...
    if (objType == COLOR_ID_MILL_PASSIVE)
    {
        ...
        _allowedParts[0] = 1 + 4 + 8 + 16 + 32;
        CMill* it = App::scene->sceneObjects->getMillFromHandle(objID1);
        if (it != nullptr)
            return (it->getColor(false));
    }
    ...
    if (objType == COLOR_ID_SHAPE)
    {
        ...
        CShape* it = App::scene->sceneObjects->getShapeFromHandle(objID1);
