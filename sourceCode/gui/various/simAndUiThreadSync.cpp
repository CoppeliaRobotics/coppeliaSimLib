...
CSimAndUiThreadSync::~CSimAndUiThreadSync()
{
    if (!_err)
    {
        ...
        if (_lockType == 2)
        {
            _sim_writeLevel--;
            _uiReadPermission
                .unlock(); // release the write permission for the SIM thread (i.e. allow the UI thread to read again)
        }
...
{ // mode: 0=minimal wait, 1=wait
    ...
    if (_lockFunctionResult != -1)
        return (false); // this condition is important, since this function will be called twice for the same object,
                        // and the second time it should return false!
...
{ // called by the SIM or UI thread, from the C API!
    ...
    {
        if (_lockFunctionResult != -1)
            return (false); // this condition is important, since this function will be called twice for the same
                            // object, and the second time it should return false!
