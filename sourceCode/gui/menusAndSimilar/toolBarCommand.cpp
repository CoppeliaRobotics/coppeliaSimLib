...
{ // Return value is true if the command belonged to toolbar menu and was executed
                ...
                if (page != nullptr)
                {
                    ...
                    if (view != nullptr)
                    {
                        ...
                        if ((cam != nullptr))
                        {
                            ...
                            cam->frameSceneOrSelectedObjects(double(viewSize[0]) / double(viewSize[1]),
                                                             view->getPerspectiveDisplay(), nullptr, true, true, 1.0,
                                                             view);
    ...
    if (commandID == OBJECT_ROTATE_NAVIGATION_CMD)
    {
        ...
        if ((GuiApp::mainWindow != nullptr) && rot &&
            (!GuiApp::mainWindow->oglSurface->isScenePageOrViewSelectionActive()))
        {
            if (!VThread::isUiThread())
            { // we are NOT in the UI thread. We execute the command now:
    ...
    if ((commandID == SIMULATION_COMMANDS_START_RESUME_SIMULATION_REQUEST_SCCMD) ||
        (commandID == SIMULATION_COMMANDS_PAUSE_SIMULATION_REQUEST_SCCMD) ||
