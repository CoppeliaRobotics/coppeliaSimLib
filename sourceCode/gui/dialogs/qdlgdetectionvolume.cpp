...
void CQDlgDetectionVolume::refresh()
{
    ...
    bool prox = App::scene->sceneObjects->isLastSelectionOfType(sim_sceneobject_proximitysensor);
    bool mill = App::scene->sceneObjects->isLastSelectionOfType(sim_sceneobject_mill);
    bool ssel = false;
    if (prox)
        ssel = (App::scene->sceneObjects->getObjectCountInSelection(sim_sceneobject_proximitysensor) > 1);
    if (mill)
        ssel = (App::scene->sceneObjects->getObjectCountInSelection(sim_sceneobject_mill) > 1);
    CConvexVolume* cv = nullptr;
    CProxSensor* proxIt = App::scene->sceneObjects->getLastSelectionProxSensor();
    CMill* millIt = App::scene->sceneObjects->getLastSelectionMill();
    if (prox)
...
void CQDlgDetectionVolume::on_qqOffset_editingFinished()
{
    ...
    {
        CProxSensor* proxSensor = App::scene->sceneObjects->getLastSelectionProxSensor();
        CMill* mill = App::scene->sceneObjects->getLastSelectionMill();
