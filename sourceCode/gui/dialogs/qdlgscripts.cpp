...
void CQDlgScripts::refresh()
{
    ...
    if (theScript != nullptr)
        associatedObject = App::currentWorld->sceneObjects->getObjectFromHandle(
            App::currentWorld->embeddedScriptContainer->getObjectHandleFromScriptHandle(theScript->getScriptHandle()));
    ui->qqExecutionOrder->setEnabled((associatedObject != nullptr) && noEditModeAndNoSim);
    ui->qqTreeTraversalDirection->setEnabled((theScript != nullptr) && noEditModeAndNoSim &&
...
void CQDlgScripts::on_qqTreeTraversalDirection_currentIndexChanged(int index)
{
    if (!inMainRefreshRoutine)
    {
        ...
        {
            int scriptID = getSelectedObjectID();
            int treeTraversalDirection =
                ui->qqTreeTraversalDirection->itemData(ui->qqTreeTraversalDirection->currentIndex()).toInt();
            App::appendSimulationThreadCommand(SET_TREETRAVERSALDIR_SCRIPTGUITRIGGEREDCMD, scriptID,
