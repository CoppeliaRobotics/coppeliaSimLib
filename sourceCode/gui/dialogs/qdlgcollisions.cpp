...
void CQDlgCollisions::refresh()
{
    ...
    ui->qqEnableAll->setChecked(App::scene->mainSettings_old->collisionDetectionEnabled);
    ...
    CCollisionObject_old* it = App::scene->collisions_old->getObjectFromHandle(getSelectedObjectID());
...
void CQDlgCollisions::updateObjectsInList()
{
    ...
    {
        CCollisionObject_old* it = App::scene->collisions_old->getObjectFromIndex(i);
...
void CQDlgCollisions::on_qqCollisionList_itemSelectionChanged()
{
    IF_UI_EVENT_CAN_READ_DATA
    {
        ...
        CCollisionObject_old* coll = App::scene->collisions_old->getObjectFromHandle(objID);
        if (coll != nullptr)
    ...
    IF_UI_EVENT_CAN_READ_DATA
    {
        if (item != nullptr)
        {
            ...
            CCollisionObject_old* it =
                App::scene->collisions_old->getObjectFromHandle(item->data(Qt::UserRole).toInt());
            if ((it != nullptr) && (newName != ""))
            {
                if (it->getObjectName() != newName)
                {
                    tt::removeIllegalCharacters(newName, true);
                    if (App::scene->collisions_old->getObjectFromName(newName.c_str()) == nullptr)
                    {
