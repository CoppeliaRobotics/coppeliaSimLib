    ...
    : CDlgEx(parent), ui(new Ui::CQDlgColor)
{
    ...
    ui->setupUi(this);
    inRefreshPart = false;
    _validityCheck1 = App::scene->sceneObjects->getLastSelectionHandle();
    _validityCheck2 = App::scene->sceneObjects->getSelectionCount();
}
...
{
    if (_validityCheck1 != App::scene->sceneObjects->getLastSelectionHandle())
        return (false);
    if (_validityCheck2 != App::scene->sceneObjects->getSelectionCount())
        return (false);
