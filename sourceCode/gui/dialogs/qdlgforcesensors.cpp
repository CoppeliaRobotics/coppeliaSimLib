...
void CQDlgForceSensors::refresh()
{
    ...
    bool sel = App::scene->sceneObjects->isLastSelectionOfType(sim_sceneobject_forcesensor);
    bool bigSel = (App::scene->sceneObjects->getObjectCountInSelection(sim_sceneobject_forcesensor) > 1);
    CForceSensor* it = App::scene->sceneObjects->getLastSelectionForceSensor();
...
void CQDlgForceSensors::on_qqColorA_clicked()
{
    IF_UI_EVENT_CAN_READ_DATA
    {
        CQDlgMaterial::displayMaterialDlg(
            COLOR_ID_FORCESENSOR_A, App::scene->sceneObjects->getLastSelectionHandle(), -1, GuiApp::mainWindow);
    }
}
