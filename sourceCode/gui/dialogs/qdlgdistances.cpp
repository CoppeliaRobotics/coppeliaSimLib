...
void CQDlgDistances::refresh()
{
    ...
    ui->qqEnableAll->setChecked(App::scene->mainSettings_old->distanceCalculationEnabled);
    ...
    CDistanceObject_old* it = App::scene->distances_old->getObjectFromHandle(getSelectedObjectID());
...
void CQDlgDistances::updateObjectsInList()
{
    ...
    {
        CDistanceObject_old* it = App::scene->distances_old->getObjectFromIndex(i);
...
void CQDlgDistances::on_qqDistanceList_itemSelectionChanged()
{
    IF_UI_EVENT_CAN_READ_DATA
    {
        ...
        CDistanceObject_old* dist = App::scene->distances_old->getObjectFromHandle(objID);
        if (dist != nullptr)
    ...
    IF_UI_EVENT_CAN_READ_DATA
    {
        if (item != nullptr)
        {
            ...
            CDistanceObject_old* it =
                App::scene->distances_old->getObjectFromHandle(item->data(Qt::UserRole).toInt());
            if ((it != nullptr) && (newName != ""))
            {
                if (it->getObjectName() != newName)
                {
                    tt::removeIllegalCharacters(newName, true);
                    if (App::scene->distances_old->getObjectFromName(newName.c_str()) == nullptr)
                    {
