...
void CQDlgIk::refresh()
{
    ...
    CIkGroup_old* it = App::scene->ikGroups_old->getObjectFromHandle(groupID);
    ...
    ui->qqIkEnabled->setChecked(App::scene->mainSettings_old->ikCalculationEnabled);
...
void CQDlgIk::updateObjectsInList()
{
    ...
    {
        CIkGroup_old* ikg = App::scene->ikGroups_old->getObjectFromIndex(i);
...
void CQDlgIk::on_qqList_itemSelectionChanged()
{
    if (!noListSelectionAllowed)
    {
        IF_UI_EVENT_CAN_WRITE_DATA
        {
            ...
            CIkGroup_old* it = App::scene->ikGroups_old->getObjectFromHandle(objID);
            if (it != nullptr)
...
void CQDlgIk::on_qqEditConditional_clicked()
{
    IF_UI_EVENT_CAN_READ_DATA
    {
        CIkGroup_old* it = App::scene->ikGroups_old->getObjectFromHandle(getSelectedObjectID());
        if (it != nullptr)
        {
