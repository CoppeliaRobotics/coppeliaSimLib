...
void CQDlgLights::refresh()
{
    ...
    CLight* it = App::scene->sceneObjects->getLastSelectionLight();
...
void CQDlgLights::on_qqLightColor_clicked()
{
    IF_UI_EVENT_CAN_READ_DATA
    {
        CQDlgLightMaterial::displayMaterialDlg(
            COLOR_ID_LIGHT_LIGHT, App::scene->sceneObjects->getLastSelectionHandle(), -1, GuiApp::mainWindow);
    }
}
...
void CQDlgLights::on_qqCasingColor_clicked()
{
    IF_UI_EVENT_CAN_READ_DATA
    {
        CQDlgMaterial::displayMaterialDlg(
            COLOR_ID_LIGHT_CASING, App::scene->sceneObjects->getLastSelectionHandle(), -1, GuiApp::mainWindow);
    }
}
