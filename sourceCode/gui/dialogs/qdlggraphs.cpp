...
void CQDlgGraphs::refresh()
{
    ...
    bool sel = App::scene->sceneObjects->isLastSelectionOfType(sim_sceneobject_graph);
    ...
    if (sel)
    {
        it = App::scene->sceneObjects->getLastSelectionGraph();
    ...
    IF_UI_EVENT_CAN_READ_DATA
    {
        if (item != nullptr)
        {
            App::appendSimulationThreadCommand(
                RENAME_DATASTREAM_GRAPHGUITRIGGEREDCMD, App::scene->sceneObjects->getLastSelectionHandle(),
...
{
    CQDlgColor::displayDlg(COLOR_ID_GRAPH_BACKGROUND, App::scene->sceneObjects->getLastSelectionHandle(), -1,
                           sim_materialcomponent_diffuse, GuiApp::mainWindow);
}

void CQDlgGraphs::on_qqAdjustGridColor_clicked()
{
    CQDlgColor::displayDlg(COLOR_ID_GRAPH_GRID, App::scene->sceneObjects->getLastSelectionHandle(), -1,
                           sim_materialcomponent_diffuse, GuiApp::mainWindow);
}
