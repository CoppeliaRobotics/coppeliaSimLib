...
void CQDlgOldScripts::refresh()
{
    ...
    if (theScript != nullptr)
        associatedObject = App::scene->sceneObjects->getObjectFromHandle(
            App::scene->sceneObjects->embeddedScriptContainer->getObjectHandleFromScriptHandle(theScript->getScriptHandle()));
...
void CQDlgOldScripts::updateObjectsInList()
{
    ...
    { // Main and simulation scripts
        ...
        {
            it = App::scene->sceneObjects->embeddedScriptContainer->allScripts[i];
...
void CQDlgOldScripts::onDeletePressed()
{
    ...
    {
        if ((focusWidget() == ui->qqScriptList) && App::scene->simulation->isSimulationStopped())
        {
...
void CQDlgOldScripts::on_qqExecutionOrder_currentIndexChanged(int index)
{
    if (!inMainRefreshRoutine)
    {
        ...
        {
            int scriptID = getSelectedObjectID();
            int objectId = App::scene->sceneObjects->embeddedScriptContainer->getObjectHandleFromScriptHandle(scriptID);
            if (objectId != -1)
            {
