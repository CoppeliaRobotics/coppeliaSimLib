...
{
    ui->qqAddNew->setEnabled(App::scene->simulation->isSimulationStopped());
    ui->qqUp->setEnabled(App::scene->simulation->isSimulationStopped());
    ui->qqDown->setEnabled(App::scene->simulation->isSimulationStopped());
...
void CQDlgUserParameters::updateObjectsInList()
{
    ...
    {
        if (((it->userParamEntries[i].properties & 1) == 0) || App::scene->simulation->isSimulationStopped())
        {
