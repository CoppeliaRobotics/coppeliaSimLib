...
void CQDlgPointclouds::refresh()
{
    ...
    bool noEditModeAndNoSim = noEditMode && App::scene->simulation->isSimulationStopped();

    bool sel = App::scene->sceneObjects->isLastSelectionOfType(sim_sceneobject_pointcloud);
    size_t objCnt = App::scene->sceneObjects->getSelectionCount();
...
void CQDlgPointclouds::on_qqColor_clicked()
{
    IF_UI_EVENT_CAN_READ_DATA
    {
        CQDlgColor::displayDlg(COLOR_ID_POINTCLOUD, App::scene->sceneObjects->getLastSelectionHandle(), -1, 0,
                               GuiApp::mainWindow);
    }
}
        ...
        if (it != nullptr)
        {
            if (!it->getDoNotUseCalculationStructure())
                GuiApp::uiThread->messageBox_warning(
                    GuiApp::mainWindow, "Point cloud octree calculation structure",
                    "Be aware that when disabling the octree calculation structure, your point cloud will not be "
                    "collidable, measurable nor detectable anymore. Also, some functionality might be limited in that "
                    "case.",
