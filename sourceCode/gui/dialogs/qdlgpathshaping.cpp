...
void CQDlgPathShaping::on_qqConvexHull_clicked()
{
    IF_UI_EVENT_CAN_READ_DATA
    {
        CPath_old* it=App::currentWorld->objCont->getLastSelection_path();
        if (it!=nullptr)
        {
...
void CQDlgPathShaping::on_qqAdjustColor_clicked()
{
    IF_UI_EVENT_CAN_READ_DATA
    {
        CQDlgMaterial::displayMaterialDlg(
            COLOR_ID_PATH_SHAPING, App::currentWorld->sceneObjects->getLastSelectionHandle(), -1, GuiApp::mainWindow);
    }
}
