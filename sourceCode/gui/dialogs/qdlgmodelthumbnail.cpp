...
{
    App::scene->sceneObjects->eraseObject(rs, false, true); // delayed destruction
    delete ui;
}
...
void CQDlgModelThumbnail::initialize()
{
    ...
    App::scene->sceneObjects->addObjectToScene(rs, false,
    ...
    App::scene->sceneObjects->getSelectedObjectHandles(sel, -1, true, false);
    actualizeBitmap();
}
...
void CQDlgModelThumbnail::on_qqFromFile_clicked()
{
    ...
    std::string filenameAndPath =
        GuiApp::uiThread->getOpenFileName(this, 0, "Loading texture...", tst.c_str(), "", true, "Image files", "tga",
    ...
    if (filenameAndPath.length() != 0)
    {
        if (VFile::doesFileExist(filenameAndPath.c_str()))
        {
            ...
            if (data == nullptr)
                GuiApp::uiThread->messageBox_critical(GuiApp::mainWindow, "Thumbnail",
            ...
            else
            {
                if ((resX != 128) || (resY != 128))
                    GuiApp::uiThread->messageBox_critical(GuiApp::mainWindow, "Thumbnail",
                                                          IDS_THUMBNAILS_REQUIRE_128X128_RESOLUTION, VMESSAGEBOX_OKELI,
                                                          VMESSAGEBOX_REPLY_OK);
                else
                {
