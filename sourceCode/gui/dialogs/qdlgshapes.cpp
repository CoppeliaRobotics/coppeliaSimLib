...
void CQDlgShapes::refresh()
{
    ...
    bool ssel = App::scene->sceneObjects->isLastSelectionASimpleShape();
    int sc = (int)App::scene->sceneObjects->getObjectCountInSelection(sim_sceneobject_shape);
    int ssc = (int)App::scene->sceneObjects->getSimpleShapeCountInSelection();
...
void CQDlgShapes::on_qqAdjustOutsideColor_clicked()
{
    IF_UI_EVENT_CAN_READ_DATA
    {
        CQDlgMaterial::displayMaterialDlg(COLOR_ID_SHAPE, App::scene->sceneObjects->getLastSelectionHandle(), -1,
                                          GuiApp::mainWindow);
    }
}
...
void CQDlgShapes::on_qqTexture_clicked()
{
    IF_UI_EVENT_CAN_READ_DATA
    {
        CQDlgTextures::displayDlg(TEXTURE_ID_SIMPLE_SHAPE, App::scene->sceneObjects->getLastSelectionHandle(),
                                  -1, GuiApp::mainWindow);
    }
}
...
void CQDlgShapes::on_qqGeometry_clicked()
{
    IF_UI_EVENT_CAN_READ_DATA
    {
        CQDlgGeometry::display(App::scene->sceneObjects->getLastSelectionHandle(), GuiApp::mainWindow);
    }
}
