...
void CQDlgFilters::onDeletePressed()
{
    IF_UI_EVENT_CAN_WRITE_DATA
    {
        if (focusWidget() == ui->qqFilterList)
        {
            ...
            if (objID != -1)
            {
                App::appendSimulationThreadCommand(DELETE_FILTER_VISIONSENSORFILTERGUITRIGGEREDCMD,
                                                   App::ct->objCont->getLastSelectionID(), objID);
        ...
        if ((filter != nullptr) && (filter->canFilterBeEdited()))
        {
            ...
            if (ft == sim_filtercomponent_rotate_deprecated)
            {
                ...
                if (theDialog.makeDialogModal() != VDIALOG_MODAL_RETURN_CANCEL)
                {
                    cmd.intParams.push_back(
                        theDialog.appendDialogValues(cmd.uint8Params, cmd.intParams, cmd.floatParams));
                    retVal = true;
                }
            }
            ...
            { // special here!
                ...
                if (params.size() == 0)
                    ...
                    callbackReturnVal = CPluginContainer::sendEventCallbackMessageToAllPlugins(
                        sim_message_eventcallback_imagefilter_adjustparams, auxData, &params[0], retVals);
...
void CQDlgFilters::on_qqAddFilter_clicked()
{
    IF_UI_EVENT_CAN_WRITE_DATA
    {
        ...
        App::appendSimulationThreadCommand(ADD_FILTER_VISIONSENSORFILTERGUITRIGGEREDCMD,
                                           App::ct->objCont->getLastSelectionID(), objID);
        App::appendSimulationThreadCommand(POST_SCENE_CHANGED_ANNOUNCEMENT_GUITRIGGEREDCMD);
        App::appendSimulationThreadCommand(FULLREFRESH_ALL_DIALOGS_GUITRIGGEREDCMD);
    }
...
void CQDlgFilters::on_qqComponentEnabled_clicked()
{
    ...
    {
        int data = getSelectedObjectID();
        App::appendSimulationThreadCommand(TOGGLE_FILTERENABLE_VISIONSENSORFILTERGUITRIGGEREDCMD,
                                           App::ct->objCont->getLastSelectionID(), data);
        App::appendSimulationThreadCommand(POST_SCENE_CHANGED_ANNOUNCEMENT_GUITRIGGEREDCMD);
        App::appendSimulationThreadCommand(FULLREFRESH_ALL_DIALOGS_GUITRIGGEREDCMD);
    }
}
