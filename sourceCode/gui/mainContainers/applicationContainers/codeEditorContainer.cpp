                                           ...
                                           const CScript* requestOrigin)
{
    ...
    CScript::getMatchingFunctions("", t, requestOrigin); // basically all functions
    for (const auto& str : t)
    {
        std::string tip(CScript::getFunctionCalltip(str.c_str(), requestOrigin));
...
{
    std::set<std::string> t;
    CScript::getMatchingConstants("", t, requestOrigin); // basically all constants
    for (const auto& str : t)
    {
        tinyxml2::XMLElement* itemNode = doc->NewElement("item");
        parentNode->InsertEndChild(itemNode);
...
void CCodeEditorContainer::announceScriptStateWillBeErased(int nakedScriptHandle, int64_t scriptUid)
{
    for (size_t i = 0; i < _allEditors.size(); i++)
    {
        if (_allEditors[i].callingScriptUid == scriptUid)
        {
            ...
            int pas[4];
            App::scenes->pluginContainer->codeEditor_close(handle, pas);
...
{
    CScript* it = App::scenes->getScriptFromHandle(callingScriptHandle);
    int retVal = -1;
    if (App::scenes->pluginContainer->isCodeEditorPluginAvailable())
    {
        if (it != nullptr)
        {
            if (!App::scene->environment->getSceneLocked())
            {
                retVal = App::scenes->pluginContainer->codeEditor_open(initText, xml);
                SCodeEditor inf;
                inf.handle = retVal;
                inf.scriptHandle = -1;
                inf.scriptUid = -1;
...
int CCodeEditorContainer::openSimulationScript(int scriptHandle)
{
    ...
    if (it != nullptr)
    {
        if (!App::scene->environment->getSceneLocked())
        {
            ...
            {
                int sceneId = App::scene->environment->getSceneUniqueID();
                for (size_t i = 0; i < _allEditors.size(); i++)
                {
                ...
                if (App::scenes->pluginContainer->isCodeEditorPluginAvailable())
                {
                    ...
                    retVal = App::scenes->pluginContainer->codeEditor_open(it->getScriptText(), printer.CStr());
                    SCodeEditor inf;
                    inf.handle = retVal;
...
int CCodeEditorContainer::openConsole(const char* title, int maxLines, int mode, const int position[2], const int size[2], const int textColor[3], const int backColor[3], int callingScriptHandle)
{
    ...
    if (App::scenes->pluginContainer->isCodeEditorPluginAvailable())
    {
        ...
        retVal = App::scenes->pluginContainer->codeEditor_open("", printer.CStr());
        SCodeEditor inf;
        inf.handle = retVal;
        inf.scriptHandle = -1;
        inf.scriptUid = -1;
        inf.callingScriptHandle = callingScriptHandle;
        CScript* so = App::scenes->getScriptFromHandle(callingScriptHandle);
        if (so != nullptr)
                                                      ...
                                                      bool oldXml) const
{
    ...
    if (App::scenes->pluginContainer->isCodeEditorPluginAvailable())
    {
        ...
        int posAndSize[4];
        App::scenes->pluginContainer->codeEditor_openModal(initText, newXml.c_str(), retVal, posAndSize);
                                             ...
                                             const CScript* requestOrigin)
{
    ...
    if (App::scenes->pluginContainer->isCodeEditorPluginAvailable())
    {
        ...
        retVal = App::scenes->pluginContainer->codeEditor_open(initText, newXml.c_str());
        SCodeEditor inf;
        inf.handle = retVal;
        inf.scriptHandle = -1;
        inf.scriptUid = -1;
        inf.callingScriptHandle = requestOrigin->getSceneObjectOrNakedScriptHandle();
...
bool CCodeEditorContainer::close(int handle, int posAndSize[4], std::string* txt, std::string* callback)
{
    for (size_t i = 0; i < _allEditors.size(); i++)
    {
        if (_allEditors[i].handle == handle)
        {
            ...
            std::string _txt;
            CScript* it = App::scenes->getScriptFromHandle(_allEditors[i].scriptHandle);
            if (App::scenes->pluginContainer->codeEditor_getText(handle, _txt, nullptr))
            {
                if (txt != nullptr)
                    txt[0] = _txt;
                if (it != nullptr)
                {
                    ...
                    {
                        CScript* itt = App::scenes->getScriptFromHandle(_allEditors[i].scriptHandle);
...
void CCodeEditorContainer::applyChanges(int handle) const
{
    if (App::userSettings->externalScriptEditor.size() > 0)
    {
        for (size_t i = 0; i < App::scene->sceneObjects->embeddedScriptContainer->allScripts.size(); i++)
            App::scene->sceneObjects->embeddedScriptContainer->allScripts[i]->fromFileToBuffer();
        for (size_t i = 0; i < App::scene->sceneObjects->getObjectCount(sim_sceneobject_script); i++)
        {
            CScriptObject* it = App::scene->sceneObjects->getScriptObjectFromIndex(i);
            if (it->nakedScript != nullptr)
                it->nakedScript->fromFileToBuffer();
        }
    }
    int sceneId = App::scene->environment->getSceneUniqueID();
    for (size_t i = 0; i < _allEditors.size(); i++)
    {
        if (_allEditors[i].sceneUniqueId == sceneId)
        {
            if ((_allEditors[i].handle == handle) || (handle == -1))
            {
                ...
                if (it != nullptr)
                {
                    if (App::scenes->pluginContainer->codeEditor_getText(_allEditors[i].handle, _txt, nullptr))
        ...
        for (size_t i = 0; i < _allEditors.size(); i++)
        {
            if (_allEditors[i].scriptUid == scriptUid)
            {
                ...
                if (!ignoreChange)
                    applyChanges(_allEditors[i].handle);
                int pas[4];
                App::scenes->pluginContainer->codeEditor_close(_allEditors[i].handle, pas);
                if (it != nullptr)
                    it->setPreviousEditionWindowPosAndSize(pas);
                if (posAndSize != nullptr)
                {
...
{ // before unloading the code editor plugin
    for (size_t i = 0; i < _allEditors.size(); i++)
        App::scenes->pluginContainer->codeEditor_close(_allEditors[i].handle, nullptr);
    _allEditors.clear();
}
...
void CCodeEditorContainer::restartScript(int handle) const
{
    for (size_t i = 0; i < _allEditors.size(); i++)
    {
        if (_allEditors[i].handle == handle)
        {
            std::string txt;
            CScript* it = App::scenes->getScriptFromHandle(_allEditors[i].scriptHandle);
            if (App::scenes->pluginContainer->codeEditor_getText(handle, txt, nullptr))
            {
                if (it != nullptr)
                {
                    applyChanges(_allEditors[i].handle);
...
std::string CCodeEditorContainer::getText(int handle, int posAndSize[4]) const
{
    ...
    for (size_t i = 0; i < _allEditors.size(); i++)
    {
        if (_allEditors[i].handle == handle)
        {
            App::scenes->pluginContainer->codeEditor_getText(handle, retVal, posAndSize);
            break;
        }
    }
    return (retVal);
...
bool CCodeEditorContainer::setText(int handle, const char* txt) const
{
    for (size_t i = 0; i < _allEditors.size(); i++)
    {
        if (_allEditors[i].handle == handle)
        {
            App::scenes->pluginContainer->codeEditor_setText(handle, txt, 0);
            return (true);
        }
    }
    return (false);
...
bool CCodeEditorContainer::appendText(int handle, const char* txt) const
{
    for (size_t i = 0; i < _allEditors.size(); i++)
    {
        if (_allEditors[i].handle == handle)
        {
            App::scenes->pluginContainer->codeEditor_setText(handle, txt, 1);
            return (true);
        }
    }
    return (false);
...
bool CCodeEditorContainer::hasSomethingBeenModifiedInCurrentScene() const
{
    ...
    for (size_t i = 0; i < _allEditors.size(); i++)
    {
        if (_allEditors[i].sceneUniqueId == sceneId)
        {
            ...
            std::string txt;
            if ((it != nullptr) &&
                App::scenes->pluginContainer->codeEditor_getText(_allEditors[i].handle, txt, nullptr))
            {
...
void CCodeEditorContainer::simulationAboutToStart() const
{
    if (App::userSettings->externalScriptEditor.size() == 0)
    {
        ...
        for (size_t i = 0; i < _allEditors.size(); i++)
        {
            ...
            {
                CScript* it = App::scene->sceneObjects->getScriptFromHandle(_allEditors[i].scriptHandle);
        ...
        for (size_t i = 0; i < _allEditors.size(); i++)
        {
            if ((_allEditors[i].sceneUniqueId == sceneId) && (!_allEditors[i].openAcrossScenes))
            {
                ...
                if (showState && _allEditors[i].userVisibility)
                    App::scenes->pluginContainer->codeEditor_show(_allEditors[i].handle, 1);
                else
                    App::scenes->pluginContainer->codeEditor_show(_allEditors[i].handle, 0);
            }
        }
    }
}
