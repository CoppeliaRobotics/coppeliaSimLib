...
bool CModelFolderWidget::_processDirectory(QTreeWidgetItem* parentTreeWidget, const fs::path& path, const std::string& rootPath, folderItem* parentItem, std::map<std::string, folderItem*>& rootPathMap, const char* nameOfSelectedFolder)
{
    bool retVal = false;
    for (const auto& entry : fs::directory_iterator(path))
    {
        if (entry.is_directory())
        {
            ...
            {
                folderItm = new folderItem(entry.path().string(), entry.path().filename().string(), entry.path().string());
                folderItm->widgetItem = new QTreeWidgetItem(parentTreeWidget, QStringList(QString(folderItm->name.c_str())));
            ...
            else
            {
                ...
                folderItm->widgetItem->setData(0, Qt::UserRole, QVariant((folderItm->widgetItem->data(0, Qt::UserRole).toString().toStdString() + "," + absPath).c_str()));
            }
            folderItm->associatedPaths.push_back(entry.path().string());
            if (_processDirectory(folderItm->widgetItem, entry.path(), rootPath, folderItm, rootPathMap, nameOfSelectedFolder))
                retVal = true;
        }
    }

    std::sort(parentItem->subfolders.begin(), parentItem->subfolders.end(), [](const folderItem* a, const folderItem* b)
    {
...
std::vector<folderItem*> CModelFolderWidget::_exploreAndMergePaths(const std::vector<std::string>& rootPaths, const char* nameOfSelectedFolder)
{
    ...
    for (const auto& rootPath : subfolders)
    {
        ...
        else
        {
            ...
            rootItem->widgetItem->setData(0, Qt::UserRole, QVariant((rootItem->widgetItem->data(0, Qt::UserRole).toString().toStdString()+","+rootPath).c_str()));
        }
        rootItem->associatedPaths.push_back(rootPath);
        if (_processDirectory(rootItem->widgetItem, rootPath, rootParent.parent_path().string(), rootItem, rootPathMap, nameOfSelectedFolder))
            rootItem->widgetItem->setExpanded(true);
    }
