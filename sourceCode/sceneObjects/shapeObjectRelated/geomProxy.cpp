...
void CGeomProxy::performSceneObjectLoadingMapping(const std::vector<int>* map)
{
    if (geomInfo!=nullptr)
        geomInfo->performSceneObjectLoadingMapping(map);
}

void CGeomProxy::performTextureObjectLoadingMapping(const std::vector<int>* map)
{
    if (geomInfo!=nullptr)
        geomInfo->performTextureObjectLoadingMapping(map);
}

void CGeomProxy::announceSceneObjectWillBeErased(int objectID)
{
    if (geomInfo!=nullptr)
        geomInfo->announceSceneObjectWillBeErased(objectID);
}
...
void CGeomProxy::initializeCalculationStructureIfNeeded()
{
    if ((collInfo==nullptr)&&(geomInfo!=nullptr))
    {
        ...
        float minTriSize=(std::max<float>(std::max<float>(_boundingBoxHalfSizes(0),_boundingBoxHalfSizes(1)),_boundingBoxHalfSizes(2)))*2.0f*App::currentWorld->environment->getCalculationMinRelTriangleSize();
...
void CGeomProxy::serialize(CSer& ar,const char* shapeName)
{
    if (ar.isBinary())
    {
        ...
        {       // Storing
            ...
            if (App::currentWorld->environment->getSaveExistingCalculationStructuresTemp()&&isCollisionInformationInitialized())
            {
