...
void CGraphCurve::makeStatic(CGraphDataStream* streams[3], int bufferSize, int startPt, int ptCnt)
{
    if (!_isStatic)
    {
        ...
        if (_dim == 2)
        {
            getCurveData_xy(streams, nullptr, bufferSize, startPt, ptCnt, nullptr, xVals, yVals, nullptr, nullptr,
                            nullptr);
        ...
        if (_dim == 3)
        {
            getCurveData_xyz(streams, nullptr, bufferSize, startPt, ptCnt, nullptr, xVals, nullptr, nullptr, nullptr,
                             nullptr);
            for (size_t i = 0; i < xVals.size(); i++)
                _staticCurveValues.push_back(xVals[i]);
        }
