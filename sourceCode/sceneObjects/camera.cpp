...
*/
    ...
    { // We report the same camera opening to all cameras attached to cameraParentProxy
        ...
        cameraParentProxy->setLocalTransformation(cameraParentProxy->getFullLocalTransformation()*cm.getTransformation()*getFullLocalTransformation().getInverse());
    }
    else
    ...
    { // regular rendering
        for (int passIndex=0;passIndex<3;passIndex++)
        {
            ...
            if (pass==RENDERPASS)
            {
                ...
                App::currentWorld->environment->activateFogIfEnabled(this,App::currentWorld->simulation->getDynamicContentVisualizationOnly());
...
void CCamera::_drawObjects(int renderingMode,int pass,int currentWinSize[2],CSView* subView,bool mirrored)
{
    ...
    if (App::currentWorld->sceneObjects->getSelectionCount()>0)
        lastSel=App::currentWorld->sceneObjects->getObjectHandleFromSelectionIndex(App::currentWorld->sceneObjects->getSelectionCount()-1);
            ...
            for (int rp=0;rp<int(toRender.size());rp++)
            {
                ...
                if (((it->getCumulativeObjectProperty()&sim_objectproperty_depthinvisible)==0)||(pass!=DEPTHPASS))
                {
                    ...
                    else
                    {
                        for (size_t i=0;i<App::currentWorld->sceneObjects->getSelectionCount();i++)
                        {
                            if (it->getObjectHandle()==App::currentWorld->sceneObjects->getObjectHandleFromSelectionIndex(i))
                            {
                                atr|=sim_displayattribute_selected;
                                if (it->getObjectHandle()==lastSel)
                                    atr|=sim_displayattribute_mainselection;
