...
{ // entityID==-1 --> checks all objects in the scene
    ...
    detectedObject = -1;
    CProxSensor* sensor = App::scene->sceneObjects->getProximitySensorFromHandle(sensorID);
    ...
    else
    {
        ...
        { // Special group here (all detectable objects):
            ...
            App::scene->sceneObjects->getAllDetectableObjectsFromSceneExcept(&exception, group,
                                                                                    sensor->getSensableType());
        }
        else
        { // Regular group here:
            App::scene->collections->getDetectableObjectsFromCollection(entityID, group,
                                                                               sensor->getSensableType());
        }
        if (group.size() != 0)
        {
    ...
    if (returnValue)
        triNormal.normalize();
    App::scenes->calcInfo->proximitySensorSimulationEnd(returnValue);
    return (returnValue);
}
...
{
    bool returnValue = false;
    CProxSensor* sens = App::scene->sceneObjects->getProximitySensorFromHandle(sensorID);
    if (sens == nullptr)
        return (false); // should never happen!
        ...
        { // detecting triangles (points and segments cannot be detected by (randomized) rays!)
            ...
            for (size_t j = 0; j < normalizedRays.size(); j++)
            {
                ...
                for (int i = 0; i < itemCount; i++)
                {
                    ...
                    if (angleLimitation)
                        _maxAngle = maxAngle;
                    if (App::scenes->pluginContainer->geomPlugin_volumeSensorDetectTriangleIfSmaller(
    ...
    else
    {
        for (int i = 0; i < itemCount; i++)
        {
            ...
            { // ray-type sensors cannot detect points or segments!
                ...
                { // detecting points
                    ...
                    { // ok, the point is closer..
                        detect = App::scenes->pluginContainer->geomPlugin_isPointInVolume1AndOutVolume2(
                ...
                { // detecting segments
                    ...
                    if (angleLimitation)
                        _maxAngle = maxAngle;
                    detect = App::scenes->pluginContainer->geomPlugin_volumeSensorDetectSegmentIfSmaller(
            ...
            { // detecting triangles
                ...
                if (angleLimitation)
                    _maxAngle = maxAngle;
                detect = App::scenes->pluginContainer->geomPlugin_volumeSensorDetectTriangleIfSmaller(
...
{ // -2: sensor triggered in the forbidden zone, -1: sensor didn't trigger. Otherwise the object handle that triggered
    ...
    if (sensor->getRandomizedDetection())
    {
        ...
        for (size_t i = 0; i < normalizedRays.size(); i++)
        {
            ...
            if (angleLimitation)
                _maxAngle = maxAngle;
            bool result = App::scenes->pluginContainer->geomPlugin_raySensorDetectMeshIfSmaller(
    ...
    else
    {
        ...
        { // non-ray-type sensors here:
            ...
            if (angleLimitation)
                _maxAngle = maxAngle;
            if (App::scenes->pluginContainer->geomPlugin_volumeSensorDetectMeshIfSmaller(
...
{ // -2: sensor triggered in the forbidden zone, -1: sensor didn't trigger. Otherwise the object handle that triggered
    ...
    if (sensor->getRandomizedDetection())
    {
        ...
        for (size_t i = 0; i < normalizedRays.size(); i++)
        {
            ...
            if (angleLimitation)
                _maxAngle = maxAngle;
            bool result = App::scenes->pluginContainer->geomPlugin_raySensorDetectOctreeIfSmaller(
    ...
    else
    {
        ...
        { // non-ray-type sensors here:
            ...
            if (angleLimitation)
                _maxAngle = maxAngle;
            if (App::scenes->pluginContainer->geomPlugin_volumeSensorDetectOctreeIfSmaller(
...
{ // the returned distance is always same or smaller than the real distance!
    ...
    if (isPointToo)
        ...
        return (
            App::scenes->pluginContainer->geomPlugin_getBoxPointDistance(tr, halfSize, true, point, nullptr));
}
...
bool CProxSensorRoutine::_doesSensorVolumeOverlapWithObjectBoundingBox(CProxSensor* sensor, CSceneObject* obj)
{
    ...
    if (obj->getObjectType() == sim_sceneobject_dummy)
        objectIsPoint = true;
    if (objectIsPoint)
        return (App::scenes->pluginContainer->geomPlugin_getBoxPointCollision(sensorTr, sensorHalfSize,
                                                                                      objectTr.X));
    else
        return (App::scenes->pluginContainer->geomPlugin_getBoxBoxCollision(sensorTr, sensorHalfSize, objectTr,
                                                                                    objectHalfSize, true));
}
