...
void CViewableBase::computeViewFrustumIfNeeded()
{
    if (!_planesCalculated)
    {
        ...
        else
        {
            ...
            if (getObjectType()==sim_object_camera_type)
            { // we are in othoview!
                nearPlane.setMultResult(viewableCumulTransf,C7Vector(C3Vector(0.0,0.0,ORTHO_CAMERA_NEAR_CLIPPING_PLANE)));
                farPlane.setMultResult(viewableCumulTransf,C7Vector(C3Vector(0.0,0.0,ORTHO_CAMERA_FAR_CLIPPING_PLANE)));
            }
            else
            {
                nearPlane.setMultResult(viewableCumulTransf,C7Vector(C3Vector(0.0,0.0,_nearClippingPlane)));
