...
{
    TRACE_INTERNAL;
    CSceneObject* it = App::scene->sceneObjects->getObjectFromHandle(linkedObjectID);
...
{ // Return value is true if the command belonged to hierarchy menu and was executed
    ...
    if (commandID == VIEW_FUNCTIONS_LOOK_THROUGH_CAMERA_VFCMD)
    {
        ...
        { // we are NOT in the UI thread. We execute the command now:
            ...
            if ((sel.size() == 1) && (App::scene->sceneObjects->getCameraFromHandle(sel[0]) != nullptr))
            {
                setDefaultValues();
    ...
    if (commandID==VIEW_FUNCTIONS_LOOK_AT_GRAPH_VFCMD)
    {
        ...
        { // we are NOT in the UI thread. We execute the command now:
            std::vector<int> sel;
            for (int i=0;i<App::scene->objCont->getSelSize();i++)
                sel.push_back(App::scene->objCont->getSelID(i));
            if ((sel.size()==1)&&(App::scene->objCont->getGraph(sel[0])!=nullptr))
            {
                setDefaultValues();
                linkedObjectID=sel[0];
                App::undoRedo_sceneChanged("");
    ...
    */
    if (commandID == VIEW_FUNCTIONS_LOOK_AT_VISION_SENSOR_VFCMD)
    {
        ...
        { // we are NOT in the UI thread. We execute the command now:
            ...
            for (size_t i = 0; i < App::scene->sceneObjects->getSelectionCount(); i++)
                sel.push_back(App::scene->sceneObjects->getObjectHandleFromSelectionIndex(i));
            if ((sel.size() == 1) && (App::scene->sceneObjects->getVisionSensorFromHandle(sel[0]) != nullptr))
            {
                setDefaultValues();
                linkedObjectID = sel[0];
                App::undoRedo_sceneChanged("");
    ...
    if (commandID == VIEW_FUNCTIONS_TRACK_OBJECT_VFCMD)
    {
        ...
        { // we are NOT in the UI thread. We execute the command now:
            ...
            for (size_t i = 0; i < App::scene->sceneObjects->getSelectionCount(); i++)
                sel.push_back(App::scene->sceneObjects->getObjectHandleFromSelectionIndex(i));
            CCamera* camera = App::scene->sceneObjects->getCameraFromHandle(linkedObjectID);
...
void CSView::addMenu(VMenu* menu)
{
    ...
    CGraph* graph = App::scene->sceneObjects->getGraphFromHandle(linkedObjectID);
    CCamera* camera = App::scene->sceneObjects->getCameraFromHandle(linkedObjectID);
    CVisionSensor* sensor = App::scene->sceneObjects->getVisionSensorFromHandle(linkedObjectID);
    size_t selSize = App::scene->sceneObjects->getSelectionCount();
    bool lastSelIsCamera = false;
    bool lastSelIsRendSens = false;
    if (selSize > 0)
    {
        if (App::scene->sceneObjects->getLastSelectionObject()->getObjectType() == sim_sceneobject_camera)
            lastSelIsCamera = true;
        if (App::scene->sceneObjects->getLastSelectionObject()->getObjectType() == sim_sceneobject_visionsensor)
    ...
    { // The linked object is a camera:
        ...
        CSceneObject* trkObj = App::scene->sceneObjects->getObjectFromHandle(camera->getTrackedObjectHandle());
        if (trkObj != nullptr)
        {
        ...
        {
            bool illegalLoop = (selSize == 1) && (App::scene->sceneObjects->getLastSelectionObject() == camera);
...
bool CSView::mouseWheel(int deltaZ, int x, int y)
{
    ...
    if (it != nullptr)
    {
        ...
        if ((it->getObjectType() == sim_sceneobject_graph) && (!App::scene->simulation->isSimulationRunning()))
        {
...
void CSView::_handleClickRayIntersection_old(int x, int y, bool mouseDown)
{
    ...
    CCamera* cam = App::scene->sceneObjects->getCameraFromHandle(linkedObjectID);
    if (cam == nullptr)
        return;
...
int CSView::modelDragMoveEvent(int x, int y, C3Vector* desiredModelPosition)
{
    ...
    CSceneObject* obj = App::scene->sceneObjects->getObjectFromHandle(linkedObjectID);
        ...
        { // Default popups
            ...
            else
            {
                ...
                if (t & PATH_EDIT_MODE_OLD)
                {
                    ...
                    GuiApp::mainWindow->editModeContainer->addMenu(
                        pathEditMenu, App::scene->sceneObjects->getObjectFromHandle(linkedObjectID));
            ...
            if (GuiApp::getEditModeType() == NO_EDIT_MODE)
            {
                if (linkedObj != -1)
                {
                    ...
                    App::scene->simulation->addMenu(simulationMenu);
            ...
            if (GuiApp::getEditModeType() & PATH_EDIT_MODE_OLD)
            {
                if (!processed)
                    processed = GuiApp::mainWindow->editModeContainer->processCommand(
                        command, App::scene->sceneObjects->getObjectFromHandle(linkedObjectID));
            }
            if (!processed)
                processed = App::scene->simulation->processCommand(command);
        }
    }
    return (false);
}
