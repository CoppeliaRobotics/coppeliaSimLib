...
int CPageContainer::getMainCameraHandle() const
{
    int retVal = -1;
#ifdef SIM_WITH_GUI
    CSPage* page = App::currentScene->pageContainer->getPage(App::currentScene->pageContainer->getActivePageIndex());
    if (page != nullptr)
    {
...
void CPageContainer::setPageSizeAndPosition(int sizeX, int sizeY, int posX, int posY)
{
    ...
    if (App::currentScene->buttonBlockContainer_old != nullptr)
        App::currentScene->buttonBlockContainer_old->setViewSizeAndPosition(sizeX, sizeY, posX, posY);
...
{ // Return value is true if the command belonged to hierarchy menu and was executed
    ...
    if ((commandID == PAGE_CONT_FUNCTIONS_CREATE_SINGLE_VIEW_TYPE_PCCMD) ||
        (commandID == PAGE_CONT_FUNCTIONS_CREATE_FOUR_VIEW_TYPE_PCCMD) ||
        (commandID == PAGE_CONT_FUNCTIONS_CREATE_SIX_VIEW_TYPE_PCCMD) ||
        (commandID == PAGE_CONT_FUNCTIONS_CREATE_EIGHT_VIEW_TYPE_PCCMD) ||
        (commandID == PAGE_CONT_FUNCTIONS_CREATE_HORIZONTAL_VIEW_TYPE_PCCMD) ||
        (commandID == PAGE_CONT_FUNCTIONS_CREATE_VERTICAL_VIEW_TYPE_PCCMD) ||
        (commandID == PAGE_CONT_FUNCTIONS_CREATE_HORIZONTAL_3_VIEW_TYPE_PCCMD) ||
        (commandID == PAGE_CONT_FUNCTIONS_CREATE_VERTICAL_3_VIEW_TYPE_PCCMD) ||
        (commandID == PAGE_CONT_FUNCTIONS_CREATE_HORIZONTAL_1_PLUS_3_VIEW_TYPE_PCCMD) ||
        (commandID == PAGE_CONT_FUNCTIONS_CREATE_VERTICAL_1_PLUS_3_VIEW_TYPE_PCCMD) ||
        (commandID == PAGE_CONT_FUNCTIONS_CREATE_HORIZONTAL_1_PLUS_4_VIEW_TYPE_PCCMD) ||
        (commandID == PAGE_CONT_FUNCTIONS_CREATE_VERTICAL_1_PLUS_4_VIEW_TYPE_PCCMD))
    {
...
void CPageContainer::renderCurrentPage(bool hideWatermark)
{
    ...
    if (getenv("_HWM"))
        *tmp = 1;
    if ((!hideWatermark) && (GuiApp::mainWindow != nullptr) &&
...
void CPageContainer::addPageMenu(VMenu* menu)
{
    ...
    menu->appendMenuItem(true, false, PAGE_CONT_FUNCTIONS_CREATE_HORIZONTAL_VIEW_TYPE_PCCMD,
                         IDS_VIEW_TYPE_TWO_HORIZ_MENU_ITEM);
    menu->appendMenuItem(true, false, PAGE_CONT_FUNCTIONS_CREATE_VERTICAL_VIEW_TYPE_PCCMD,
                         IDS_VIEW_TYPE_TWO_VERT_MENU_ITEM);
    menu->appendMenuItem(true, false, PAGE_CONT_FUNCTIONS_CREATE_HORIZONTAL_3_VIEW_TYPE_PCCMD,
                         IDS_VIEW_TYPE_THREE_HORIZ_MENU_ITEM);
    menu->appendMenuItem(true, false, PAGE_CONT_FUNCTIONS_CREATE_VERTICAL_3_VIEW_TYPE_PCCMD,
                         IDS_VIEW_TYPE_THREE_VERT_MENU_ITEM);
    menu->appendMenuItem(true, false, PAGE_CONT_FUNCTIONS_CREATE_HORIZONTAL_1_PLUS_3_VIEW_TYPE_PCCMD,
                         IDS_VIEW_TYPE_HORIZONTAL_ONE_PLUS_THREE_MENU_ITEM);
    menu->appendMenuItem(true, false, PAGE_CONT_FUNCTIONS_CREATE_VERTICAL_1_PLUS_3_VIEW_TYPE_PCCMD,
                         IDS_VIEW_TYPE_VERTICAL_ONE_PLUS_THREE_MENU_ITEM);
    menu->appendMenuItem(true, false, PAGE_CONT_FUNCTIONS_CREATE_HORIZONTAL_1_PLUS_4_VIEW_TYPE_PCCMD,
                         IDS_VIEW_TYPE_HORIZONTAL_ONE_PLUS_FOUR_MENU_ITEM);
    menu->appendMenuItem(true, false, PAGE_CONT_FUNCTIONS_CREATE_VERTICAL_1_PLUS_4_VIEW_TYPE_PCCMD,
                         IDS_VIEW_TYPE_VERTICAL_ONE_PLUS_FOUR_MENU_ITEM);
...
{ // NOT FULLY IMPLEMENTED! objType=-1 --> not supported, 0 --> hierarchy, 1 --> 3DViewable
    ...
    if ((App::currentScene->buttonBlockContainer_old != nullptr) &&
        App::currentScene->buttonBlockContainer_old->mouseDownTest(x, y, _activePageIndex))
    { // not yet supported
        objType = -1;
        return (true);
    }
...
bool CPageContainer::leftMouseButtonDown(int x, int y, int selectionStatus)
{
    ...
    if (App::currentScene->buttonBlockContainer_old != nullptr)
        App::currentScene->buttonBlockContainer_old->clearCaughtElements(0xffff - sim_left_button);
    ...
    leftMouseCaughtSoftDialog = false;
    if ((App::currentScene->buttonBlockContainer_old != nullptr) &&
        App::currentScene->buttonBlockContainer_old->mouseDown(mouseRelativePosition[0], mouseRelativePosition[1],
...
void CPageContainer::leftMouseButtonUp(int x, int y)
{
    ...
    if (App::currentScene->buttonBlockContainer_old != nullptr)
    {
        if (App::currentScene->buttonBlockContainer_old->getCaughtElements() & sim_left_button)
            App::currentScene->buttonBlockContainer_old->mouseUp(mouseRelativePosition[0], mouseRelativePosition[1],
                                                                 _activePageIndex);
    }
...
void CPageContainer::mouseMove(int x, int y, bool passiveAndFocused)
{
    ...
    if (App::currentScene->buttonBlockContainer_old != nullptr)
    {
        ...
        {
            if (App::currentScene->buttonBlockContainer_old->getCaughtElements() & bts)
                App::currentScene->buttonBlockContainer_old->mouseMove(mouseRelativePosition[0], mouseRelativePosition[1]);
        }
        else
            App::currentScene->buttonBlockContainer_old->mouseMove(mouseRelativePosition[0], mouseRelativePosition[1]);
    }
...
{
    if (App::currentScene->buttonBlockContainer_old != nullptr)
        App::currentScene->buttonBlockContainer_old->looseFocus();
...
void CPageContainer::setFocusObject(int obj)
{
    ...
    if (focusObject == FOCUS_ON_PAGE)
    {
        if (App::currentScene->buttonBlockContainer_old != nullptr)
        {
            App::currentScene->buttonBlockContainer_old->setEditBoxEdition(-1, -1, false);
            App::currentScene->buttonBlockContainer_old->looseFocus();
        }
    }
}
...
{
    if ((focusObject == FOCUS_ON_SOFT_DIALOG) && (App::currentScene->buttonBlockContainer_old != nullptr))
    {
        App::currentScene->buttonBlockContainer_old->onKeyDown(key);
        return;
    }

    if ((key == CTRL_V_KEY) || (key == DELETE_KEY) || (key == BACKSPACE_KEY) || (key == CTRL_X_KEY) ||
        (key == CTRL_C_KEY) || (key == ESC_KEY) || (key == CTRL_A_KEY) || (key == CTRL_Y_KEY) || (key == CTRL_Z_KEY))
    {
    ...
    {
        App::currentScene->simulation->keyPress(key);
        return;
    }

    if (key == CTRL_E_KEY)
    {
        App::sceneContainer->keyPress(key);
        return;
    }
    ...
    App::sceneContainer->pluginContainer->sendEventCallbackMessageToAllPlugins_old(sim_message_eventcallback_keypress,
                                                                                   data);
    data[0] = key;
    data[1] = flags;
    App::sceneContainer->pluginContainer->sendEventCallbackMessageToAllPlugins_old(sim_message_keypress, data); // for backward compatibility
    ...
    App::sceneContainer->setModificationFlag(1024); // key was pressed
}
...
bool CPageContainer::rightMouseButtonDown(int x, int y)
{
    ...
    if (App::currentScene->buttonBlockContainer_old != nullptr)
        App::currentScene->buttonBlockContainer_old->clearCaughtElements(0xffff - sim_right_button);
    for (int i = 0; i < PAGES_COUNT; i++)
    {
        if (_allPages[i] != nullptr)
    ...
    if (App::currentScene->buttonBlockContainer_old != nullptr)
    {
        ...
        // return true!)
        if (App::currentScene->buttonBlockContainer_old->mouseDownTest(mouseRelativePosition[0], mouseRelativePosition[1],
                                                                       _activePageIndex))
            return (false);
    }
...
void CPageContainer::rightMouseButtonUp(int x, int y, int absX, int absY, QWidget* mainWindow)
{
    ...
    else
    {
        ...
        {
            if ((prepareForPopupMenu >= 0) && (prepareForPopupMenu < PAGES_COUNT) &&
                (prepareForPopupMenu == _activePageIndex))
            {
...
bool CPageContainer::middleMouseButtonDown(int x, int y)
{
    ...
    if (App::currentScene->buttonBlockContainer_old != nullptr)
        App::currentScene->buttonBlockContainer_old->clearCaughtElements(0xffff - sim_middle_button);
...
bool CPageContainer::leftMouseButtonDoubleClick(int x, int y, int selectionStatus)
{
    ...
    if (App::currentScene->buttonBlockContainer_old != nullptr)
    {
        if (App::currentScene->buttonBlockContainer_old->leftMouseButtonDoubleClick(x, y, _activePageIndex))
        {
            setFocusObject(FOCUS_ON_SOFT_DIALOG);
            return (true);
        }
...
int CPageContainer::getCaughtElements() const
{
    ...
    if (App::currentScene->buttonBlockContainer_old != nullptr)
        retVal |= App::currentScene->buttonBlockContainer_old->getCaughtElements();
...
void CPageContainer::clearCaughtElements(int keepMask)
{
    ...
    if (App::currentScene->buttonBlockContainer_old != nullptr)
        App::currentScene->buttonBlockContainer_old->clearCaughtElements(keepMask);
