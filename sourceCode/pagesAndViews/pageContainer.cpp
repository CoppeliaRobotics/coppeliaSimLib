...
void CPageContainer::setPageSizeAndPosition(int sizeX, int sizeY, int posX, int posY)
{
    ...
    if (App::scene->buttonBlockContainer_old != nullptr)
        App::scene->buttonBlockContainer_old->setViewSizeAndPosition(sizeX, sizeY, posX, posY);
...
{ // NOT FULLY IMPLEMENTED! objType=-1 --> not supported, 0 --> hierarchy, 1 --> 3DViewable
    ...
    if ((App::scene->buttonBlockContainer_old != nullptr) &&
        App::scene->buttonBlockContainer_old->mouseDownTest(x, y, _activePageIndex))
    { // not yet supported
        objType = -1;
        return (true);
    }
...
bool CPageContainer::leftMouseButtonDown(int x, int y, int selectionStatus)
{
    ...
    if (App::scene->buttonBlockContainer_old != nullptr)
        App::scene->buttonBlockContainer_old->clearCaughtElements(0xffff - sim_left_button);
    ...
    leftMouseCaughtSoftDialog = false;
    if ((App::scene->buttonBlockContainer_old != nullptr) &&
        App::scene->buttonBlockContainer_old->mouseDown(mouseRelativePosition[0], mouseRelativePosition[1],
...
void CPageContainer::leftMouseButtonUp(int x, int y)
{
    ...
    if (App::scene->buttonBlockContainer_old != nullptr)
    {
        if (App::scene->buttonBlockContainer_old->getCaughtElements() & sim_left_button)
            App::scene->buttonBlockContainer_old->mouseUp(mouseRelativePosition[0], mouseRelativePosition[1],
                                                                 _activePageIndex);
    }
...
void CPageContainer::mouseMove(int x, int y, bool passiveAndFocused)
{
    ...
    if (App::scene->buttonBlockContainer_old != nullptr)
    {
        ...
        {
            if (App::scene->buttonBlockContainer_old->getCaughtElements() & bts)
                App::scene->buttonBlockContainer_old->mouseMove(mouseRelativePosition[0], mouseRelativePosition[1]);
        }
        else
            App::scene->buttonBlockContainer_old->mouseMove(mouseRelativePosition[0], mouseRelativePosition[1]);
    }
...
{
    if (App::scene->buttonBlockContainer_old != nullptr)
        App::scene->buttonBlockContainer_old->looseFocus();
...
void CPageContainer::setFocusObject(int obj)
{
    ...
    if (focusObject == FOCUS_ON_PAGE)
    {
        if (App::scene->buttonBlockContainer_old != nullptr)
        {
            App::scene->buttonBlockContainer_old->setEditBoxEdition(-1, -1, false);
            App::scene->buttonBlockContainer_old->looseFocus();
        }
    }
}
...
{
    if ((focusObject == FOCUS_ON_SOFT_DIALOG) && (App::scene->buttonBlockContainer_old != nullptr))
    {
        App::scene->buttonBlockContainer_old->onKeyDown(key);
        return;
    }
    ...
    {
        App::scene->simulation->keyPress(key);
        return;
    }

    if (key == CTRL_E_KEY)
    {
        App::scenes->keyPress(key);
        return;
    }
    ...
    App::scenes->pluginContainer->sendEventCallbackMessageToAllPlugins_old(sim_message_eventcallback_keypress,
                                                                                   data);
    data[0] = key;
    data[1] = flags;
    App::scenes->pluginContainer->sendEventCallbackMessageToAllPlugins_old(sim_message_keypress, data); // for backward compatibility
    ...
    App::scenes->setModificationFlag(1024); // key was pressed
}
...
bool CPageContainer::rightMouseButtonDown(int x, int y)
{
    ...
    if (App::scene->buttonBlockContainer_old != nullptr)
        App::scene->buttonBlockContainer_old->clearCaughtElements(0xffff - sim_right_button);
    for (int i = 0; i < PAGES_COUNT; i++)
    {
        if (_allPages[i] != nullptr)
    ...
    if (App::scene->buttonBlockContainer_old != nullptr)
    {
        ...
        // return true!)
        if (App::scene->buttonBlockContainer_old->mouseDownTest(mouseRelativePosition[0], mouseRelativePosition[1],
                                                                       _activePageIndex))
            return (false);
    }
...
bool CPageContainer::middleMouseButtonDown(int x, int y)
{
    ...
    if (App::scene->buttonBlockContainer_old != nullptr)
        App::scene->buttonBlockContainer_old->clearCaughtElements(0xffff - sim_middle_button);
...
bool CPageContainer::leftMouseButtonDoubleClick(int x, int y, int selectionStatus)
{
    ...
    if (App::scene->buttonBlockContainer_old != nullptr)
    {
        if (App::scene->buttonBlockContainer_old->leftMouseButtonDoubleClick(x, y, _activePageIndex))
        {
            setFocusObject(FOCUS_ON_SOFT_DIALOG);
            return (true);
        }
...
int CPageContainer::getCaughtElements() const
{
    ...
    if (App::scene->buttonBlockContainer_old != nullptr)
        retVal |= App::scene->buttonBlockContainer_old->getCaughtElements();
...
void CPageContainer::clearCaughtElements(int keepMask)
{
    ...
    if (App::scene->buttonBlockContainer_old != nullptr)
        App::scene->buttonBlockContainer_old->clearCaughtElements(keepMask);
