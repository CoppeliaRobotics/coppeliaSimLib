...
enum
{
    AUX_VIEW_SHIFTING = 0,
    AUX_VIEW_TOP_BORDER,
    AUX_VIEW_BOTTOM_BORDER,
    AUX_VIEW_RIGHT_BORDER,
    AUX_VIEW_LEFT_BORDER,
    AUX_VIEW_TOP_RIGHT_BORDER,
    AUX_VIEW_TOP_LEFT_BORDER,
    AUX_VIEW_BOTTOM_RIGHT_BORDER,
    AUX_VIEW_BOTTOM_LEFT_BORDER,
...
bool CSPage::setPageType(int newType)
{
    ...
    for (size_t i = 0; i < newSize; i++)
    {
        ...
        _allViewAuxSizesAndPos.push_back(0.25);
        _allViewAuxSizesAndPos.push_back(0.05);
        _allViewAuxSizesAndPos.push_back(0.05);
    }
...
void CSPage::getViewSizeAndPosition(int sViewSize[2], int sViewPos[2], size_t subViewIndex) const
{
    ...
    { // We have a floating window here:
        // Positions and sizes are relative now (2009/05/22)
        int fvs[2] = {int(_allViewAuxSizesAndPos[4 * subViewIndex + 0] * double(_pageSize[0])),
                      int(_allViewAuxSizesAndPos[4 * subViewIndex + 1] * double(_pageSize[1]))};
        int fvp[2] = {int(_allViewAuxSizesAndPos[4 * subViewIndex + 2] * double(_pageSize[0])),
                      int(_allViewAuxSizesAndPos[4 * subViewIndex + 3] * double(_pageSize[1]))};
...
void CSPage::getFloatingViewRelativeSizeAndPosition(double sViewSize[2], double sViewPos[2], size_t subViewIndex) const
{
    if (subViewIndex >= getRegularViewCount())
    { // We have a floating window here:
...
bool CSPage::removeFloatingView(size_t viewIndex)
{
    ...
    { // Only floating views can be removed!
        ...
        _allViewAuxSizesAndPos.erase(_allViewAuxSizesAndPos.begin() + 4 * viewIndex,
                                     _allViewAuxSizesAndPos.begin() + 4 * viewIndex + 4);
        return (true);
    }
    return (false);
}
...
void CSPage::addFloatingView()
{
    ...
    _allViewAuxSizesAndPos.push_back(0.25);
    _allViewAuxSizesAndPos.push_back(0.25);
    _allViewAuxSizesAndPos.push_back(0.05 + floatingViewAddOffset);
    _allViewAuxSizesAndPos.push_back(0.05 + floatingViewAddOffset);
    setViewSizesAndPositions();
...
void CSPage::serialize(CSer& ar)
{
    if (ar.isBinary())
    {
        ...
        else
        {
                ...
                if (theName.compare(SER_END_OF_OBJECT) != 0)
                {
                    ...
                    { // for backward comp. (flt->dbl)
                        ...
                        for (int i = 0; i < n; i++)
                        {
                            float dummy;
                            ar >> dummy;
                            _allViewAuxSizesAndPos.push_back((double)dummy);
                        }
                    }
            ...
            // Following is to correct for a bug where deleted views would still have their sizes and pos stored
            // (19/7/2011):
            std::vector<double> copy(_allViewAuxSizesAndPos);
...
bool CSPage::doubleClickActionForView(size_t viewIndex)
{
    ...
    if ((_pageType >= SINGLE_VIEW) && (_pageType < CLOSING_VIEW_ITEM))
    {
        ...
        if ((viewIndex >= getRegularViewCount()) && (viewIndex < _allViews.size()))
        {
            ...
            cmd.intParams.push_back(App::scene->pageContainer->getActivePageIndex());
        ...
        {
            if ((_pageType == SIX_VIEWS_old) || (_pageType == SIX_VIEWS) || (_pageType == EIGHT_VIEWS) ||
                (_pageType == HORIZONTALLY_DIVIDED_3) || (_pageType == VERTICALLY_DIVIDED_3) ||
                (_pageType == HORIZONTAL_1_PLUS_3_VIEWS) || (_pageType == VERTICAL_1_PLUS_3_VIEWS) ||
                (_pageType == HORIZONTAL_1_PLUS_4_VIEWS) || (_pageType == VERTICAL_1_PLUS_4_VIEWS))
            {
                SSimulationThreadCommand cmd;
                cmd.cmdId = SWAP_VIEWS_CMD;
                cmd.intParams.push_back(App::scene->pageContainer->getActivePageIndex());
...
void CSPage::swapViews(size_t index1, size_t index2, bool alsoSizeAndPosInfo)
{
    ...
    if (it->getCanSwapViewWithMainView() || ((index1 != 0) && (index2 != 0)))
    {
        ...
        if (alsoSizeAndPosInfo)
        {
            ...
            {
                // Positions and sizes are relative now (2009/05/22)
                double v = _allViewAuxSizesAndPos[4 * index1 + i];
...
bool CSPage::leftMouseButtonDown(int x, int y, int selectionStatus)
{
    ...
    {
        auxViewResizingAction =
            getMousePosRelativeToFloatingViewBorders(mouseRelativePosition[0], mouseRelativePosition[1], i);
...
void CSPage::mouseMove(int x, int y, bool passiveAndFocused)
{
    ...
    if (_caughtElements & sim_left_button)
    {
        if ((auxViewResizingAction != -1) && (viewIndexOfResizingAction < int(_allViewAuxSizesAndPos.size()) / 4))
        {
            ...
            int vp[2];
            // Positions and sizes are relative now (2009/05/22)
            vs[0] = int(_allViewAuxSizesAndPos[4 * viewIndexOfResizingAction + 0] * double(_pageSize[0]) + 0.5);
            vs[1] = int(_allViewAuxSizesAndPos[4 * viewIndexOfResizingAction + 1] * double(_pageSize[1]) + 0.5);
            vp[0] = int(_allViewAuxSizesAndPos[4 * viewIndexOfResizingAction + 2] * double(_pageSize[0]) + 0.5);
            vp[1] = int(_allViewAuxSizesAndPos[4 * viewIndexOfResizingAction + 3] * double(_pageSize[1]) + 0.5);
            // Make sure we are not smaller than the minimum size (2009/05/22):
            ...
            if ((auxViewResizingAction == AUX_VIEW_TOP_BORDER) || (auxViewResizingAction == AUX_VIEW_TOP_LEFT_BORDER) ||
                (auxViewResizingAction == AUX_VIEW_TOP_RIGHT_BORDER))
            {
                int val = y - py;
                if (val < FLOATING_VIEW_MIN_SIZE)
            ...
            if (((auxViewResizingAction == AUX_VIEW_SHIFTING) &&
                 (_allViews[viewIndexOfResizingAction]->getCanBeShifted())) ||
                ((auxViewResizingAction != AUX_VIEW_SHIFTING) && (auxViewResizingAction != AUX_VIEW_CLOSING_BUTTON) &&
                 (_allViews[viewIndexOfResizingAction]->getCanBeResized())))
            {
                if ((auxViewResizingAction != AUX_VIEW_SHIFTING) && (auxViewResizingAction != AUX_VIEW_CLOSING_BUTTON))
                {
                    _allViewAuxSizesAndPos[4 * viewIndexOfResizingAction + 0] = double(vs[0]) / double(_pageSize[0]);
                    _allViewAuxSizesAndPos[4 * viewIndexOfResizingAction + 1] = double(vs[1]) / double(_pageSize[1]);
                }
                _allViewAuxSizesAndPos[4 * viewIndexOfResizingAction + 2] = double(vp[0]) / double(_pageSize[0]);
                _allViewAuxSizesAndPos[4 * viewIndexOfResizingAction + 3] = double(vp[1]) / double(_pageSize[1]);
            }
            ...
            { // Check if we are still on the button:
                auxViewResizingAction = getMousePosRelativeToFloatingViewBorders(
                    mouseRelativePosition[0], mouseRelativePosition[1], viewIndexOfResizingAction);
                if (auxViewResizingAction != AUX_VIEW_CLOSING_BUTTON)
