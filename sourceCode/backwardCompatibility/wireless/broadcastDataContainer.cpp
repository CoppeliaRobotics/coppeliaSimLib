                                           ...
                                           int& dataHeaderR, std::string& dataNameR)
{
    ...
    for (size_t i = 0; i < _allObjects.size(); i++)
    {
        ...
        if (retVal != nullptr)
        {
            if (originalIndex == -1)
            {
                if (App::scene->environment->getVisualizeWirelessReceivers() || _wirelessForceShow_reception)
                {
                    bool err = false;
                    ...
                    {
                        CSceneObject* it =
                            App::scene->sceneObjects->getObjectFromHandle(_allObjects[i]->getAntennaHandle());
                        if (it != nullptr)
