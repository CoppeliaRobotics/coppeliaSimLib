...
{ // should only be called when simulation is stopped
    if ((index >= 0) && (index < int(userParamEntries.size())) && (newIndex >= 0) &&
        (newIndex < int(userParamEntries.size())))
    {
        SUserParamEntry tmp;
...
bool CUserParameters::setParameterValue(const char* paramName, const char* paramValue, size_t paramValueLength)
{
    ...
    if (ind >= 0)
    {
        ...
        userParamEntries[ind].properties =
            (userParamEntries[ind].properties | 4) - 4; // Indicates that string doesn't contain embedded 0's
