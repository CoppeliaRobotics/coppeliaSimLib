    ...
    const int theDirectionConstraints[2], const double clearanceAndMaxDistance[2])
{
    ...
    gdEuler(0) = 0.0;
    gdEuler(1) = 0.0;
    gd.X(2) = 0.0; // startDummyLocal.X(2);
    fromStart.push_back(new CNonHolonomicPathNode_old(
        0.0, 0.0, 0.0)); // startDummyLocal.X(0),startDummyLocal.X(1),startDummyLocalEuler(2)));
            ...
            if ((closest != nullptr) || firstPass)
            {
                ...
                if ((closest != nullptr) || firstPass)
                {
                    ...
                    if ((closestConnect != nullptr) || firstPass)
                    {
                        ...
                        closestConnect =
                            connect(nextCurrent, current, closestConnect, closest, i == 1, true, false, startDummy);
...
{ // Return value is !=nullptr if extention was performed to some extent
    ...
    for (i = 0; i < 1000; i++)
    {
        ...
        // 1. Valid values?
        if ((x > searchMinVal[0]) && (x < searchMinVal[0] + searchRange[0]) && (y > searchMinVal[1]) &&
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        { // new values are valid
            ...
            if (!doCollide(nullptr))
            {
                ...
                if ((dy * dy + dx * dx) < (1.2 * stepSize * stepSize)) // 1.2 is important!!
                {
                    if (fabs(dTheta) < 0.06 * degToRad) // 0.06 degrees
                    {
                        couldReach = true;
                        break;
                    }
    ...
    while (true)
    {
        ...
        {                     // If randomPass==0, the pass is not random, i.e. the low and high indices are calculated
            ...
            { // Now let's try to link highIndex from lowIndex with a "straight" line:
                ...
                if (connect(&newPathElementsBetweenAndIncludingLowAndHigh, nullptr, startP, endP, true, true, true,
                            startDummy) != nullptr)
                { // The path can be performed!
                    connect(&newPathElementsBetweenAndIncludingLowAndHigh, nullptr, startP, endP, true, true, false,
                            startDummy);
                    ...
                    { // The new path section is shorter. We replace current path section with this one:
                        ...
                        foundPathSameStraightLineID_forSteppedSmoothing.erase(
                            foundPathSameStraightLineID_forSteppedSmoothing.begin() + lowIndex,
                            foundPathSameStraightLineID_forSteppedSmoothing.begin() + highIndex + 1);

                        foundPath.insert(foundPath.begin() + lowIndex,
                                         newPathElementsBetweenAndIncludingLowAndHigh.begin(),
                                         newPathElementsBetweenAndIncludingLowAndHigh.end());
                        foundPathSameStraightLineID_forSteppedSmoothing.insert(
                            foundPathSameStraightLineID_forSteppedSmoothing.begin() + lowIndex, elementsAfter,
                    ...
                    delete newPathElementsBetweenAndIncludingLowAndHigh[0]; // Impossible to connect, we have to erase
                                                                            // the only element in the list
                _simSetObjectLocalTransformation_internal(startDummy, startDummyOriginalLocalTr.X.data,
                                                          startDummyOriginalLocalTr.Q.data,
                                                          0.0); // Restore the original transformation
            }
        }
    }
