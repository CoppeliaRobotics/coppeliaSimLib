...
void CHolonomicPathPlanning_old::setAngularCoefficient(float coeff)
{
    angularCoeff=coeff;
}

void CHolonomicPathPlanning_old::setStepSize(float size)
{
    stepSize=size;
}

void CHolonomicPathPlanning_old::getSearchTreeData(std::vector<float>& data,bool fromTheStart)
{
...
int CHolonomicPathPlanning_old::getVector(CHolonomicPathNode_old* fromPoint,CHolonomicPathNode_old* toPoint,float vect[7],float e,float& artificialLength,bool dontDivide)
...
bool CHolonomicPathPlanning_old::addVector(C3Vector& pos,C4Vector& orient,float vect[7])
...
void CHolonomicPathPlanning_old::getPathData(std::vector<float>& data)
{
    data.clear();
    if (invalidData)
        return;
...
bool CHolonomicPathPlanning_old::areDirectionConstraintsRespected(float vect[7])
{
    if (planningType==sim_holonomicpathplanning_xy)
    {
...
bool CHolonomicPathPlanning_old::areSomeValuesForbidden(float values[7])
{
    float gamma=0.0f; 
    if (planningType==sim_holonomicpathplanning_xy)
    {
