...
void CPathPlanningTask::setCollisionDetection(bool c)
{
    ...
    if (c)
    {
        ...
        CDummy* it = App::scene->sceneObjects->getDummyFromHandle(_robotEntityID);
        if (it != nullptr)
            _robotEntityID = -1;
        it = App::scene->sceneObjects->getDummyFromHandle(_obstacleEntityID);
        if (it != nullptr)
            _obstacleEntityID = -1;
    }
}
...
{
    if (App::scene->sceneObjects->getPathFromHandle(_pathID) == nullptr)
        return (false); // No path!
    if (pathPlanningType == sim_pathplanning_holonomic)
    { // Holonomic path planning
        if (App::scene->sceneObjects->getDummyFromHandle(_startDummyID) == nullptr)
            return (false);
        if (App::scene->sceneObjects->getDummyFromHandle(_goalDummyID) == nullptr)
            return (false);
...
bool CPathPlanningTask::initiateSteppedSearch(bool showProgressDlg, double maxTime, double subDt)
{
    if (!isTaskValid())
        return (false);
    CPath_old* thePath = App::scene->sceneObjects->getPathFromHandle(_pathID);
    if (thePath == nullptr)
        return (false);
...
{ // retVal: -1: error, -2: search not yet finished, 0: no path found, 1: partial path found, 2: full path found
    ...
    if (pathPlanningType == sim_pathplanning_nonholonomic)
    {
        ...
        { // we have a partial or full path!
            ...
            if (res == 1)
            {
                CPath_old* it = App::scene->sceneObjects->getPathFromHandle(_pathID);
                int nodeCount;
...
{
    if ((_startDummyID == -1) || (!_visualizeSearchArea) || (!App::scene->simulation->isSimulationStopped()))
        return;
    CSceneObject* it = App::scene->sceneObjects->getObjectFromHandle(_startDummyID);
    if (it != nullptr)
    {
