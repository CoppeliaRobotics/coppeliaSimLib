...
CPathPlanningTask::~CPathPlanningTask()
{
    App::currentScene->drawingCont->removeObject(_searchTree1Handle);
    App::currentScene->drawingCont->removeObject(_searchTree2Handle);
...
void CPathPlanningTask::setCollisionDetection(bool c)
{
    ...
    if (c)
    {
        ...
        CDummy* it = App::currentScene->sceneObjects->getDummyFromHandle(_robotEntityID);
        if (it != nullptr)
            _robotEntityID = -1;
        it = App::currentScene->sceneObjects->getDummyFromHandle(_obstacleEntityID);
        if (it != nullptr)
            _obstacleEntityID = -1;
    }
}
...
{
    _startDummyID = CScene::getLoadingMapping(map, _startDummyID);
    _goalDummyID = CScene::getLoadingMapping(map, _goalDummyID);
    _pathID = CScene::getLoadingMapping(map, _pathID);
    if (_robotEntityID <= sim_object_sceneobjectend)
        _robotEntityID = CScene::getLoadingMapping(map, _robotEntityID);
    if (_obstacleEntityID <= sim_object_sceneobjectend)
        _obstacleEntityID = CScene::getLoadingMapping(map, _obstacleEntityID);
}
...
{
    if (App::currentScene->sceneObjects->getPathFromHandle(_pathID) == nullptr)
        return (false); // No path!
    if (pathPlanningType == sim_pathplanning_holonomic)
    { // Holonomic path planning
        if (App::currentScene->sceneObjects->getDummyFromHandle(_startDummyID) == nullptr)
            return (false);
        if (App::currentScene->sceneObjects->getDummyFromHandle(_goalDummyID) == nullptr)
            return (false);
...
bool CPathPlanningTask::initiateSteppedSearch(bool showProgressDlg, double maxTime, double subDt)
{
    if (!isTaskValid())
        return (false);
    CPath_old* thePath = App::currentScene->sceneObjects->getPathFromHandle(_pathID);
    if (thePath == nullptr)
        return (false);
    ...
    if (pathPlanningType == sim_pathplanning_nonholonomic)
    {
        ...
        void* p = CPathPlanningInterface::createNonHolonomicPathPlanningObject(
            _startDummyID, _goalDummyID, _robotEntityID, _obstacleEntityID, angularCoeff, 1.0, 0.01, maxSteeringAngle,
            stepSize, searchMinValue, searchRange, searchDirection, clearanceAndMaxDistance);
        _steppedSearchTemp_theTask = p;
    }
    if (pathPlanningType == sim_pathplanning_holonomic)
    {
        void* p = CPathPlanningInterface::createHolonomicPathPlanningObject(
            _startDummyID, _goalDummyID, _robotEntityID, _obstacleEntityID, holonomicTaskType, angularCoeff, stepSize,
            searchMinValue, searchRange, searchDirection, clearanceAndMaxDistance, _gammaAxis.getNormalized().data);
        _steppedSearchTemp_theTask = p;
    }
...
{ // retVal: -1: error, -2: search not yet finished, 0: no path found, 1: partial path found, 2: full path found
    ...
    if (pathPlanningType == sim_pathplanning_nonholonomic)
    {
        ...
        { // we have to search more
            if (VDateTime::getTimeDiffInMs(_steppedSearchTemp_initTimeInMs) < _steppedSearchTemp_maxTimeInMs)
            {
                ...
                { // we found a path!
                    ...
                    {
                        // GuiApp::uiThread->setProgressBarText(IDS_PATH_WAS_FOUND_,IDS_NOW_PERFORMING_POST_PROCESSING___,nullptr);
                    }
            ...
            {               // We ran out of time!
                ...
                if (_partialPathIsOk)
                {
                    if (CPathPlanningInterface::setPartialPath(p) != 0)
                    {
                        if (_steppedSearchTemp_showProgressDlg)
                        {
                            // GuiApp::uiThread->setProgressBarText(IDS_PARTIAL_PATH_WAS_FOUND_,IDS_NOW_PERFORMING_POST_PROCESSING___,nullptr);
                        }
        ...
        { // we have a partial or full path!
            ...
            if (res == 1)
            {
                CPath_old* it = App::currentScene->sceneObjects->getPathFromHandle(_pathID);
                int nodeCount;
    ...
    { // This is the last time in this routine. Display the trees and clean-up!
        if ((retVal > 0) && _showSearchTrees)
        {
            ...
            if (fromStart != nullptr)
            {
                ...
                _searchTree1Handle = App::currentScene->drawingCont->addObject(it);
            }
            if (fromGoal != nullptr)
            {
                CDrawingObject* it = new CDrawingObject(sim_drawing_lines, 1.0, 0.0, -1, 1000000, -1);
                ...
                _searchTree2Handle = App::currentScene->drawingCont->addObject(it);
            }
...
{
    if ((_startDummyID == -1) || (!_visualizeSearchArea) || (!App::currentScene->simulation->isSimulationStopped()))
        return;
    CSceneObject* it = App::currentScene->sceneObjects->getObjectFromHandle(_startDummyID);
    if (it != nullptr)
    {
            ...
            for (int j = 0; j < 2; j++)
            {
                for (int k = 0; k < 2; k++)
                {
                    ...
                    if (pathPlanningType == sim_pathplanning_nonholonomic)
                        theZ = 0.0;
                    corners[4 * i + 2 * j + k] =
                        m.X + (m.M.axis[0] * searchMinValue[0] + m.M.axis[0] * double(i) * searchRange[0]) * theX;
                    corners[4 * i + 2 * j + k] +=
                        (m.M.axis[1] * searchMinValue[1] + m.M.axis[1] * double(j) * searchRange[1]) * theY;
                    corners[4 * i + 2 * j + k] +=
                        (m.M.axis[2] * searchMinValue[2] + m.M.axis[2] * double(k) * searchRange[2]) * theZ;
                }
            }
        }
