...
void CIkGroup_old::serialize(CSer& ar)
{
    if (ar.isBinary())
    {
        ...
        { // Storing
            ...
            // 14/12/2011       SIM_SET_CLEAR_BIT(nothing,7,_avoidanceCheckAgainstAll); DO NOT USE FOR BACKWARD
...
{ // Overridden from _CIkGroup_old
    ...
    if (check)
        it = App::scene->ikGroups_old->getObjectFromHandle(_objectHandle);
    if (it != this)
    ...
    { // object is in world
        ...
        if (nm.size() > 0)
        {
            if (getObjectName() != nm)
            {
                while (App::scene->ikGroups_old->getObjectFromName(nm.c_str()) != nullptr)
...
{ // Overridden from _CIkGroup_old
    ...
    { // object is in world
        ...
        {
            CIkGroup_old* prev = App::scene->ikGroups_old->getObjectFromIndex(i);
...
{ // Overridden from _CIkGroup_old
    ...
    if ((theMethod == sim_ik_pseudo_inverse_method) || (theMethod == sim_ik_damped_least_squares_method) ||
        (theMethod == sim_ik_jacobian_transpose_method) || (theMethod == sim_ik_undamped_pseudo_inverse_method))
...
{ // If return value if false, the calling function has to destroy anElement (invalid)
    ...
    if (App::scene->sceneObjects->getDummyFromHandle(anElement->getTipHandle()) == nullptr)
...
{
    bool retVal = false;
    retVal = App::scenes->pluginContainer->oldIkPlugin_computeJacobian(_ikPluginCounterpartHandle, options);
    _setLastJacobian(App::scenes->pluginContainer->oldIkPlugin_getJacobian(_ikPluginCounterpartHandle));
    return (retVal);
}
    ...
    if (_enabled)
    {
        ...
        if (!independentComputation)
        {
            ...
            { // Conditional execution part:
                CIkGroup_old* it = App::scene->ikGroups_old->getObjectFromHandle(_doOnFailOrSuccessOf);
                if (it != nullptr)
                {
                    if (_doOnPerformed)
                    {
        ...
        if (doIt)
        {
            retVal = App::scenes->pluginContainer->oldIkPlugin_handleIkGroup(_ikPluginCounterpartHandle);
            ...
            if (!independentComputation)
                _setLastJacobian(
                    App::scenes->pluginContainer->oldIkPlugin_getJacobian(_ikPluginCounterpartHandle));
            /*
...
{ // activeRows and activeColumns are nullptr by default (--> all rows and columns are active)
    ...
    { // We compute this directly, we have a three-by-three matrix:
        double retVal = 0.0;
        retVal += m(activeRows->at(0), activeColumns->at(0)) *
                  ((m(activeRows->at(1), activeColumns->at(1)) * m(activeRows->at(2), activeColumns->at(2))) -
                   (m(activeRows->at(1), activeColumns->at(2)) * m(activeRows->at(2), activeColumns->at(1))));
        retVal -= m(activeRows->at(0), activeColumns->at(1)) *
                  ((m(activeRows->at(1), activeColumns->at(0)) * m(activeRows->at(2), activeColumns->at(2))) -
                   (m(activeRows->at(1), activeColumns->at(2)) * m(activeRows->at(2), activeColumns->at(0))));
        retVal += m(activeRows->at(0), activeColumns->at(2)) *
                  ((m(activeRows->at(1), activeColumns->at(0)) * m(activeRows->at(2), activeColumns->at(1))) -
                   (m(activeRows->at(1), activeColumns->at(1)) * m(activeRows->at(2), activeColumns->at(0))));
        return (retVal);
    }
    ...
    for (int rowInd = 0; rowInd < int(activeRows->size()); rowInd++)
    {
        ...
        retVal += m(i, activeColumns->at(0)) * _getDeterminant(m, &actR, &actC) *
                  pow(-1.0, double(rowInd + 2)); // was rowInd+1 until 3.1.3 rev2.
    }
    return (retVal);
}
...
{ // Overridden from _CIkGroup_old
    ...
    if (_ikPluginCounterpartHandle != -1)
    {
        ...
        if (_restoreIfOrientationNotReached)
            flags |= 8;
        App::scenes->pluginContainer->oldIkPlugin_setIkGroupFlags(_ikPluginCounterpartHandle, flags);
    }
}
...
{ // Overridden from _CIkGroup_old
    ...
    // Synchronize with IK plugin:
    if (_ikPluginCounterpartHandle != -1)
        App::scenes->pluginContainer->oldIkPlugin_setIkGroupCalculation(_ikPluginCounterpartHandle,
...
{ // Overridden from _CIkGroup_old
    ...
    // Synchronize with IK plugin:
    if (_ikPluginCounterpartHandle != -1)
        App::scenes->pluginContainer->oldIkPlugin_setIkGroupCalculation(_ikPluginCounterpartHandle, m,
...
void CIkGroup_old::buildOrUpdate_oldIk()
{
    ...
    _ikPluginCounterpartHandle = App::scenes->pluginContainer->oldIkPlugin_createIkGroup();
...
{
    if (_ikPluginCounterpartHandle != -1)
        App::scenes->pluginContainer->oldIkPlugin_eraseIkGroup(_ikPluginCounterpartHandle);
    _ikPluginCounterpartHandle = -1;
}
...
{ // ret: -1: error, 0: nothing found, 1: solution found
    int retVal = -1;
    retVal = App::scenes->pluginContainer->oldIkPlugin_getConfigForTipPose(
...
{ // ret: -1: object not tagger for explicit handling, -2: invalid handles, otherwise an Ik calculation result
    ...
    if (retVal >= sim_ikresult_not_performed)
    {
        ...
        {
            if (App::scene->sceneObjects->getJointFromHandle(jointHandles[i]) == nullptr)
                retVal = -2;
        }
    }
    ...
    if (retVal >= sim_ikresult_not_performed)
    {
        ...
        for (int i = 0; i < jointCnt; i++)
        {
            CJoint* joint = App::scene->sceneObjects->getJointFromHandle(jointHandles[i]);
...
{ // ret: -1: invalidHandles, -2: ik Element inexistent, -3: invalid arg, -4: ik tip not linked to target, -5: invalid
    ...
    if ((retVal == 0) && (collisionPairCnt > 0) && (collisionPairs != nullptr))
    {
        for (int i = 0; i < collisionPairCnt; i++)
        {
            ...
            {
                if (App::scene->collections->getObjectFromHandle(collisionPairs[2 * i + 0]) == nullptr)
                    retVal = -5;
                else
                {
                    if (collisionPairs[2 * i + 1] != sim_handle_all)
                    {
                        if (App::scene->collections->getObjectFromHandle(collisionPairs[2 * i + 1]) == nullptr)
                            retVal = -5;
                    }
                }
            }
