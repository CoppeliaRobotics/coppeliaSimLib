            ...
            if (true) // GuiApp::operationalUIParts&sim_gui_infobar)
            {
                ...
                if (cont != nullptr)
                {
                    if (cont->allBlocks[0] != nullptr)
                    {
                        ...
                        infoBox->setAttributes(infoBox->getAttributes() | sim_ui_property_systemblock |
...
void CButtonBlockContainer::updateInfoWindowColorStyle(int colorStyle)
{
    ...
    for (int i = 0; i < 20; i++)
    {
        ...
        if ((b != nullptr) && (b2 != nullptr) && (b3 != nullptr))
        {
            ...
            if (background)
            {
                b->setAttributes(b->getAttributes() &
                                 (0xffff - sim_buttonproperty_transparent - sim_buttonproperty_nobackgroundcolor));
                b2->setAttributes(b2->getAttributes() &
                                  (0xffff - sim_buttonproperty_transparent - sim_buttonproperty_nobackgroundcolor));
                if (i > 1)
                    b3->setAttributes(b3->getAttributes() &
                                      (0xffff - sim_buttonproperty_transparent - sim_buttonproperty_nobackgroundcolor));
                if (lineActive)
                {
...
{
    {
        if (infoBox != nullptr)
        {
            ...
            if (butt == 0)
                App::currentWorld->mainSettings_old->infoWindowOpenState =
                    !App::currentWorld->mainSettings_old->infoWindowOpenState;
            if (butt == 200)
            {
                App::currentWorld->mainSettings_old->infoWindowColorStyle++;
                if (App::currentWorld->mainSettings_old->infoWindowColorStyle > 2)
                    App::currentWorld->mainSettings_old->infoWindowColorStyle = 0;
            }
            // Following needs to be run at every frame, since the amount of displayed info changes (when sim runs. for
            // example). Tested!
            updateInfoWindowColorStyle(App::currentWorld->mainSettings_old->infoWindowColorStyle);
            ...
            if (previousOpenState != App::currentWorld->mainSettings_old->infoWindowOpenState)
            {
                ...
                previousOpenState = App::currentWorld->mainSettings_old->infoWindowOpenState;
            }
        }
    }
    bool showInfoAndStatusBox =
        ((GuiApp::mainWindow == nullptr) || (!GuiApp::mainWindow->simulationRecorder->getIsRecording()) ||
    ...
    for (int i = 0; i < int(allBlocks.size()); i++)
    {
        ...
        if (((it->viewToAppearOn == -1) || (it->viewToAppearOn == currentView)) &&
            ...
            it->displayBlock(winSize,
                             (i == int(allBlocks.size()) - 1) && (_lastBlockIdInFront != allBlocks[i]->getBlockID()));
    }
    if (allBlocks.size() != 0)
...
bool CButtonBlockContainer::mouseDown(int xCoord, int yCoord, int currentView, int selectionStatus)
{
    ...
    for (int i = (int)allBlocks.size() - 1; i >= 0; i--)
    {
        ...
        if ((itBlock->viewToAppearOn == -1) || (itBlock->viewToAppearOn == currentView))
        {
            if ((showInfoAndStatusBox || (itBlock != infoBox)) &&
                ((!editModeButNotButtonEditMode) || (itBlock == infoBox)))
            {
                cursorCatch = false;
    ...
    if (caught)
    {
        bool simNotPausedOrActiveAtPause =
            (!App::currentWorld->simulation->isSimulationPaused()) ||
        ...
        if ((itButton->getAttributes() & sim_buttonproperty_enabled) &&
            (itButton->getButtonType() == sim_buttonproperty_button) && simNotPausedOrActiveAtPause)
        {
            setEditBoxEdition(-1, -1, true);
        ...
        else if ((itButton->getAttributes() & sim_buttonproperty_enabled) &&
                 (itButton->getButtonType() == sim_buttonproperty_slider) && simNotPausedOrActiveAtPause)
        {
            setEditBoxEdition(-1, -1, true);
            ...
            else
            {
                ...
                caughtButtonDown = false;
                 ...
                 (itButton->getButtonType() == sim_buttonproperty_editbox) && simNotPausedOrActiveAtPause)
        ...
        else
        {
            ...
            caughtBlockForDownUpEvent = -1;
            caughtButtonForDownUpEvent = -1;
            if (((itButton->getButtonType() == sim_buttonproperty_label) &&
                 (itBlock->getAttributes() & sim_ui_property_moveable)) ||
                (!simNotPausedOrActiveAtPause))
...
bool CButtonBlockContainer::mouseUp(int xCoord, int yCoord, int currentView)
{
    ...
    if (caught)
    {
        ...
        { // Here a button was pressed
            ...
            if ((itButton->getAttributes() & sim_buttonproperty_rollupaction) &&
                (itButton->getAttributes() & sim_buttonproperty_staydown))
            {
            ...
            (App::currentWorld->buttonBlockContainer_old->caughtButtonForDownUpEvent != -1))
        {
            ...
            if ((udButton != nullptr) && App::currentWorld->buttonBlockContainer_old->caughtButtonDownForDownUpEvent)
            { // We have to generate an up event:
                App::currentWorld->outsideCommandQueue_old->addCommand(
                    ...
                    App::currentWorld->buttonBlockContainer_old->caughtButtonForDownUpEvent, udButton->getAttributes(), 0,
                    nullptr, 0);
                int auxVals[2] = {udButton->getAttributes(), 0};
                udBlock->setLastEventButtonID(App::currentWorld->buttonBlockContainer_old->caughtButtonForDownUpEvent,
                                              auxVals);
            }
        }
        caughtBlockForDownUpEvent = -1;
    ...
    if (aa != nullptr)
    {
        ...
        if (bb != nullptr)
        {
            ...
            { // We have to move the slider
                if (!bb->getVertical())
                { // Horizontal slider
                    double buttXSize = ((double)((aa->getButtonWidthRetina() + 2) * bb->getLength() - 2)) *
                                       (1.0 - (double)BUTTON_SLIDER_X_SIZE);
                    double dx = (xCoord - mousePos.x) * 2.0 / buttXSize;
                    bb->setSliderPos(bb->getSliderPos() + dx);
                }
                else
                { // Vertical slider
                    double buttYSize = ((double)((aa->getButtonHeightRetina() + 2) * bb->getHeight() - 2)) *
                                       (1.0 - (double)BUTTON_SLIDER_X_SIZE);
