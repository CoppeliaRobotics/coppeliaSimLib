...
void CSoftButton::setAttributes(int attr)
{
    ...
    if (afterType != sim_buttonproperty_button)
        _buttonAttributes &=
            (0xffff - sim_buttonproperty_staydown - sim_buttonproperty_isdown - sim_buttonproperty_rollupaction -
             sim_buttonproperty_closeaction - sim_buttonproperty_downupevent);
...
void CSoftButton::setSliderPos(double pos)
{
    tt::limitValue(-1.0, 1.0, pos);
    sliderPos = pos;
}
double CSoftButton::getSliderPos()
{
    return (sliderPos);
}
...
int CSoftButton::_nextButtonUniqueID =
    1; // start at 1 because 0 is for the block (so that texture dependencies don't get confused!!)
...
void CSoftButton::drawArray(VPoint p, VPoint s)
{
    ...
    if (((_buttonAttributes & 0x0003) == sim_buttonproperty_button) &&
    ...
    if (((_buttonAttributes & 0x0003) == sim_buttonproperty_editbox) &&
        ((_buttonAttributes & sim_buttonproperty_borderless) == 0) &&
        ((_buttonAttributes & sim_buttonproperty_nobackgroundcolor) == 0))
    {
