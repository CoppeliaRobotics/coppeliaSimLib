...
{
    int id = CScript::getScriptObjectOrNakedScriptHandleFromInterpreterState_lua(L);
    CScript* script = App::scenes->getScriptFromHandle(id);
    if (script == nullptr)
        return (false);
...
{ // Returns all scripts that are built under the current one
    ...
    if (it != nullptr)
    {
        ...
        { // we have a main script here
            ...
            {
                CSceneObject* q = App::scene->sceneObjects->getObjectFromIndex(i);
                CScript* lso = App::scene->sceneObjects->embeddedScriptContainer->getScriptFromObjectAttachedTo(
                    sim_scripttype_simulation, q->getObjectHandle());
                if (lso != nullptr)
                    scriptHandles.push_back(lso->getSceneObjectOrNakedScriptHandle());
            }
        }
        ...
        { // we have a customization script
            ...
            { // should always pass
                if (selfIncluded)
                {
                    CScript* aScript = App::scene->sceneObjects->embeddedScriptContainer->getScriptFromObjectAttachedTo(
                        sim_scripttype_simulation, obj->getObjectHandle());
                    if (aScript != nullptr)
                        scriptHandles.push_back(aScript->getSceneObjectOrNakedScriptHandle());
                }
    ...
    if (it != nullptr)
    {
        ...
        { // we have a simulation script here
            ...
            if (obj != nullptr)
            {
                ...
                if (mainIncluded)
                {
                    CScript* lso = App::scene->sceneObjects->embeddedScriptContainer->getMainScript();
                    if (lso != nullptr)
                        scriptHandles.push_back(lso->getSceneObjectOrNakedScriptHandle());
                }
            }
...
{ // for backward compatibility with simMoveToPosition on old threaded scripts
    ...
    { // Those are the arguments that are always required! (the rest can be ignored or set to nil!
        ...
        double maxVelocity;
        CSceneObject* object = App::scene->sceneObjects->getObjectFromHandle(objID);
        ...
        {
            relToObject = App::scene->sceneObjects->getObjectFromHandle(relativeToObjID);
        ...
        { // do the job here!
            ...
            { // if the path length is 0 (the two positions might still be not-coincident, depending on the calculation
                // method!)
                if (App::scene->sceneObjects->getObjectFromHandle(objID) ==
            ...
            else
            {
                ...
                mem->lastTime = App::scene->simulation->getSimulationTime();
...
{ // for backward compatibility with simMoveToPosition on old threaded scripts
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0))
    {
        ...
        if (mem != nullptr)
        {
            ...
            bool movementFinished = false;
            double currentTime =
                App::scene->simulation->getSimulationTime() + App::scene->simulation->getTimeStep();
                ...
                mem->object) // make sure the object is still valid (running in a thread)
            {
                if ((mem->relToObject != nullptr) &&
                    (App::scene->sceneObjects->getObjectFromHandle(mem->relativeToObjID) != mem->relToObject))
    ...
    { // Ok we have 2 tables with same sizes.
        ...
        { // do the job here!
            ...
            for (int i = 0; i < tableLen; i++)
            {
                ...
                CJoint* it = App::scene->sceneObjects->getJointFromHandle(jointHandles[i]);
            ...
            bool movementFinished = false;
            double dt = App::scene->simulation->getTimeStep(); // this is the time left if we leave here
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0))
    {
        ...
        if (mem != nullptr)
        {
            ...
            if (mem->sameTimeFinish)
            {
                ...
                if (App::scene->sceneObjects->getJointFromHandle(mem->jointHandles[mem->maxVirtualDistIndex]) !=
                    nullptr)
                {
                    if (mem->accel == 0.0)
            ...
            else
            {
                ...
                for (int i = 0; i < tableLen; i++)
                {
                    if (App::scene->sceneObjects->getJointFromHandle(mem->jointHandles[i]) != nullptr)
                    {
            ...
            for (int i = 0; i < tableLen; i++)
            {
                CJoint* joint = App::scene->sceneObjects->getJointFromHandle(mem->jointHandles[i]);
...
{ // DEPRECATED
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_number, 0))
    {
        ...
        if (handle == sim_handle_self)
            handle = CScript::getScriptObjectOrNakedScriptHandleFromInterpreterState_lua(L);
...
{ // DEPRECATED
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_string, 0))
    {
        bool goOn = true;
        int handle = luaToInt(L, 1);
        int selfScriptHandle = CScript::getScriptObjectOrNakedScriptHandleFromInterpreterState_lua(L);
        ...
        else
        {
            if (handle == sim_handle_self)
            {
                ...
                CScript* it = App::scenes->getScriptFromHandle(handle);
                if (it->getScriptType() == sim_scripttype_customization)
                {
                    ...
                    if (it != nullptr)
                        handle = it->getSceneObjectOrNakedScriptHandle();
                    else
                        goOn = false;
                }
            }
...
{ // DEPRECATED
    ...
    { // we want the suffix of current script
        luaWrap_lua_pushinteger(L, CScript::getScriptNameIndexFromInterpreterState_lua_old(L));
        LUA_END(1);
    }
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_string, 0))
...
{ // DEPRECATED
    ...
    {
        int nb = luaToInt(L, 1);
        CScript::setScriptNameIndexToInterpreterState_lua_old(L, nb);
        retVal = 1;
    }
...
{ // for backward compatibility with simMoveToObject on old threaded scripts
    ...
    { // Those are the arguments that are always required! (the rest can be ignored or set to nil!
        ...
        CSceneObject* object = App::scene->sceneObjects->getObjectFromHandle(objID);
        CSceneObject* targetObject = App::scene->sceneObjects->getObjectFromHandle(targetObjID);
        double accel = 0.0; // means infinite accel!! (default value)
        bool foundError = false;
...
{ // for backward compatibility with simMoveToObject on old threaded scripts
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0))
    {
        ...
        if (mem != nullptr)
        {
            ...
            if (ll > 1.0)
                ll = 1.0;
            if ((App::scene->sceneObjects->getObjectFromHandle(mem->objID) == mem->object) &&
...
{ // deprecated on 01.10.2020
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_string, 0, lua_arg_number, 0))
    {
        ...
        int currentScriptID = CScript::getScriptObjectOrNakedScriptHandleFromInterpreterState_lua(L);
        CScript* it = App::scenes->getScriptFromHandle(currentScriptID);
        retVal = App::scene->commTubeContainer_old->openTube(luaToInt(L, 1), strTmp.c_str(),
                        ...
                        else
                        {
                            ...
                            if (!err)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        CSceneObject* ant =
                                            App::scene->sceneObjects->getObjectFromHandle(antennaHandle);
                                        if (ant == nullptr)
                                        {
                                            errorString = SIM_ERROR_INVALID_ANTENNA_HANDLE;
                                            err = true;
                            ...
                            if (!err)
                            {
                                ...
                                App::scene->sceneObjects->embeddedScriptContainer->broadcastDataContainer.broadcastData(
                                    ...
                                    App::scene->simulation->getSimulationTime() + persistence, actionRadius,
    ...
    if ((it->getScriptType() == sim_scripttype_main) || (it->getScriptType() == sim_scripttype_simulation))
    {
        ...
        if (!err)
        {
            ...
            if (!err)
            {
                ...
                if (antennaHandle != sim_handle_default)
                {
                    CSceneObject* ant = App::scene->sceneObjects->getObjectFromHandle(antennaHandle);
                    if (ant == nullptr)
                    {
                        errorString = SIM_ERROR_INVALID_ANTENNA_HANDLE;
                        err = true;
        ...
        if (!err)
        {
            ...
            char* data0 = App::scene->sceneObjects->embeddedScriptContainer->broadcastDataContainer.receiveData(
                currentScriptID, App::scene->simulation->getSimulationTime(), dataHeader, dataName,
...
{ // for backward compatibility with simFollowPath on old threaded scripts
    ...
    { // Those are the arguments that are always required! (the rest can be ignored or set to nil!
        ...
        CSceneObject* object = App::scene->sceneObjects->getObjectFromHandle(objID);
        CPath_old* path = App::scene->sceneObjects->getPathFromHandle(pathID);
        double accel = 0.0; // means infinite accel!! (default value)
        bool foundError = false;
        if ((!foundError) && (object == nullptr))
        {
...
{ // for backward compatibility with simFollowPath on old threaded scripts
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0))
    {
        ...
        {
            if ((App::scene->sceneObjects->getObjectFromHandle(mem->objID) == mem->object) &&
                (App::scene->sceneObjects->getPathFromHandle(mem->pathID) ==
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_string, 0))
    {
        ...
        if (handle == sim_handle_self)
        {
            ...
            { // is a CScriptObject
                ...
                {
                    CSceneObject* obj = App::scene->sceneObjects->getObjectFromHandle(handle);
                    obj = obj->getParent();
                    if (obj != nullptr)
...
{ // deprecated on 20.11.2020
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        if ((retVal > 0) && (objHandle >= 0))
        {
            ...
            CCollisionObject_old* it = App::scene->collisions_old->getObjectFromHandle(objHandle);
            if (it != nullptr)
            {
        ...
        if (okToGo)
        {
            ...
            if (okToGo)
            {
                ...
                { // following condition added on 2011/01/06 so as to not remove objects created from the c/c++
                    ...
                    CBannerObject* anObj = App::scene->bannerCont_old->getObject(retVal);
                    if (anObj != nullptr)
...
{ // deprecated on 23.11.2020
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        { // following condition added here on 2011/01/06 so as not to remove objects created from a C/c++ call
            App::scene->bannerCont_old->eraseAllObjects(true);
            retVal = 1;
        }
        else
...
{ // deprecated on 08.06.2021
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        if (a == sim_handle_self)
            a = CScript::getScriptObjectOrNakedScriptHandleFromInterpreterState_lua(L);
...
{ // deprecated on 18.06.2021
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        if (id == sim_handle_self)
        {
            ...
            if (id != -1)
            {
                CScriptObject* s = App::scene->sceneObjects->getScriptObjectFromHandle(id);
                if (s != nullptr)
                {
                    if (s->nakedScript->getParentIsProxy())
                    {
...
{ // deprecated in 2019-2020 sometimes
    ...
    else
    {
        ...
        { // We try to check whether we have sim_handle_all as a number:
            ...
            {
                handleAll = true;
                App::scenes->pluginContainer->sendEventCallbackMessageToAllPlugins(action);
                luaWrap_lua_pushinteger(L, 1);
            }
        }
        if (!handleAll)
    ...
    else
    {
        ...
        { // Ok we have a table. Now what size is it?
            ...
            if (getInt32ArrayFromTable(L, 1, tableLen, buffer))
            {
                for (int i = 0; i < tableLen; i++)
                {
                    if (App::scene->sceneObjects->getObjectFromHandle(buffer[i]) != nullptr)
                        App::scene->sceneObjects->addObjectToSelection(buffer[i]);
                }
                retVal = 1;
            }
            else
...
{ // deprecated since 08.10.2021
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        { // for backward compatibility
            int cnt = int(App::scene->sceneObjects->getObjectCount());
...
{ // deprecated on 03.12.2021
    ...
    {                                                        // argument sim.handle_self
        if (luaToInt(L, 1) == sim_handle_self)
        {
            ...
            int a = CScript::getScriptObjectOrNakedScriptHandleFromInterpreterState_lua(L);
            if (a <= sim_object_sceneobjectend)
            { // is a CScriptObject
                CScript* it = App::scene->sceneObjects->getScriptFromHandle(a);
                if (it->getParentIsProxy())
                {
                    CSceneObject* obj = App::scene->sceneObjects->getObjectFromHandle(a);
                    obj = obj->getParent();
                    if (obj != nullptr)
                ...
                if ((res == 0) || (res == 2))
                {
                    ...
                    if ((res == 0) || (res == 2))
                    {
                        ...
                        if ((res == 0) || (res == 2))
                        {
                            ...
                            CVisionSensor* rs =
                                App::scene->sceneObjects->getVisionSensorFromHandle(objectHandle);
                            if (rs != nullptr)
                            {
                                if ((sizeX == 0) && (sizeY == 0))
...
{ // deprecated on 11.04.2022
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        CSceneObject* it = App::scene->sceneObjects->getObjectFromHandle(sensHandle);
        if (it != nullptr)
            ...
            if ((res == 0) || (res == 2))
            {
                ...
                if ((res == 0) || (res == 2))
                {
                    ...
                    if ((res == 0) || (res == 2))
                    {
                        ...
                        CVisionSensor* rs = App::scene->sceneObjects->getVisionSensorFromHandle(sensHandle);
                        if (rs != nullptr)
                        {
                            if ((sizeX == 0) && (sizeY == 0))
                            { // we have default values here (the whole picture)
...
{ // deprecated on 06.05.2022
    ...
    int retVal = -1; // means error
    if (luaWrap_lua_gettop(L) == 0)
        retVal = CScript::getScriptObjectOrNakedScriptHandleFromInterpreterState_lua(L); // no arguments, return itself
    else
    {
        if (checkInputArguments(L, nullptr, argOffset, lua_arg_nil, 0))
            retVal = CScript::getScriptObjectOrNakedScriptHandleFromInterpreterState_lua(
        ...
        else
        {
            ...
            { // script type arg.
                ...
                if (scriptType != sim_handle_self)
                {
                    ...
                    retVal = CScript::getScriptObjectOrNakedScriptHandleFromInterpreterState_lua(L); // for backward compatibility
            }
            else
...
{ // deprecated on 19.05.2023
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_string, 0, lua_arg_string, 0))
    {
        ...
        {
            retVal = 1;
            if (App::scenes->scriptCustomFuncAndVarContainer->removeCustomFunction(
...
{ // deprecated on 19.05.2023
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_string, 0))
    {
        ...
        retVal = 1;
        App::scenes->scriptCustomFuncAndVarContainer->insertCustomVariable(varNameAtPluginName.c_str(), nullptr,
                                                                                   0);
    }
