                                            ...
                                            int* detectedObjectHandle, float* normalVector)
{
    ...
    double n[3];
    int retVal =
        simCheckProximitySensorEx_internal(sensorHandle, entityHandle, detectionMode, (double)detectionThreshold,
                                           (double)maxAngle, pt, detectedObjectHandle, n);
    if (retVal > 0)
    {
        for (size_t i = 0; i < 3; i++)
        {
                                             ...
                                             float* detectedPoint, float* normalVector)
{
    ...
    double pt[3];
    double n[3];
    int retVal = simCheckProximitySensorEx2_internal(sensorHandle, v.data(), itemType, itemCount, detectionMode,
                                                     (double)detectionThreshold, (double)maxAngle, pt, n);
    if (retVal > 0)
    {
        for (size_t i = 0; i < 3; i++)
        {
                                   ...
                                   int curveWidth)
{
    ...
    return (
        simAddGraphCurve_internal(graphHandle, curveName, dim, streamIds, defV, unitStr, options, color, curveWidth));
}
...
{ // no effect anymore
    App::logMsg(sim_verbosity_errors,
                "support for API function simRemoveParticleObject was dropped: call has no effect");
    return (-1);
}
                                             ...
                                             int* outBufferSize)
{
    return (simExtCallScriptFunction_internal(scriptHandleOrType, functionNameAtScriptName, inIntData, inIntCnt,
                                              ...
                                              inBufferCnt, outIntData, outIntCnt, outFloatData, outFloatCnt,
                                              outStringData, outStringCnt, outBufferData, outBufferSize));
}
//************************
...
{
    App::logMsg(sim_verbosity_errors,
                "support for API function simInitializePathSearch was dropped: call has no effect");
    return (-1);
}
