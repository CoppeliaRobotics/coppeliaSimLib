...
{ // DEPRECATED since 21/1/2016   referenceConfigs can be nullptr, as well as jointWeights
    C_API_START;
    CApiErrors::setLastError(__func__, "not supported anymore.");
    return (-1);
}
...
{ // deprecated (23/02/2016)
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_WRITE_DATA
    {
        ...
        { // script is identified by its ID
            ...
            script = App::worldContainer->getDetachedScriptFromHandle(scriptHandleOrType);
        }
        else
        {
                         ...
                         int* buttonHandles)
{
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_WRITE_DATA
    {
        ...
        if (b > s[0])
        {
            CApiErrors::setLastError(__func__, SIM_ERROR_INVALID_DIMENSIONS);
            return (-1);
        }
...
int simCreateUIButton_internal(int elementHandle, const int* position, const int* size, int buttonProperty)
{
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_WRITE_DATA
    {
        ...
        {
            delete but;
            CApiErrors::setLastError(__func__, SIM_ERROR_WRONG_POS_SIZE_PARAMS);
            return (-1);
        }
...
int simSetUISlider_internal(int elementHandle, int buttonHandle, int position)
{
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        {
            CApiErrors::setLastError(__func__, SIM_ERROR_BUTTON_NOT_SLIDER);
            return (-1);
        }
...
{ // DEPRECATED
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        {
            CDetachedScript* it = App::currentWorld->sceneObjects->embeddedScriptContainer->getDetachedScriptFromHandle(scriptHandle);
...
{ // deprecated on 29.09.2020
    CApiErrors::setLastError(__func__, "Not supported anymore. Use CoppeliaSim V4.2.0 or earlier.");
    return (-1);
}
...
{ // deprecated on 29.09.2020
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        if (!err)
        {
            ...
            {
                err = true;
                CApiErrors::setLastError(__func__, SIM_ERROR_IK_ELEMENT_INEXISTANT);
            }
        }
        if (!err)
        {
...
{ // deprecated on 01.10.2020
    ...
    if (VThread::isSimThread())
    {
        ...
        CApiErrors::setLastError(__func__, SIM_ERROR_CANNOT_BE_CALLED_FROM_MAIN_THREAD);
        return (-1);
    }
    return (0);
}
...
{ // deprecated on 01.10.2020
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        {
            CDetachedScript* it = App::worldContainer->getDetachedScriptFromHandle(targetID);
            if (it == nullptr)
            {
        ...
        if (dataHeader < 0)
        {
            CApiErrors::setLastError(__func__, SIM_ERROR_INVALID_DATA_HEADER);
            return (-1);
        }
        if (strlen(dataName) < 1)
        {
            CApiErrors::setLastError(__func__, SIM_ERROR_INVALID_DATA_NAME);
            return (-1);
        }
        if (dataLength < 1)
        {
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        if (dataType == 0)
        {
            ...
            { // regular use of the function
                ...
                {
                    CApiErrors::setLastError(__func__, SIM_ERROR_PATH_EMPTY);
                    return (-1);
                }
            }
            else
            { // We are working with indices pointing on ctrl points. index=-(relativeDistance+1.0)
                ...
                {
                    CApiErrors::setLastError(__func__, SIM_ERROR_INVALID_CTRL_PT);
                    return (-1);
                }
            }
        }
...
{ // deprecated on 17.11.2020
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        int retVal = int(it->getObjectHandle());
        return (retVal);
    }
    CApiErrors::setLastError(__func__, SIM_ERROR_COULD_NOT_LOCK_RESOURCES_FOR_READ);
    return (-1);
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_WRITE_DATA
    {
        ...
        if (collectionHandle == sim_handle_all)
        {
            ...
            for (size_t i = 0; i < App::currentWorld->collections->getObjectCount(); i++)
                App::currentWorld->collections->addCollectionToSelection(int(App::currentWorld->collections->getObjectFromIndex(i)->getObjectHandle()));
            std::vector<int> sel;
            for (size_t i = 0; i < App::currentWorld->sceneObjects->getSelectionCount(); i++)
        ...
        else
        {
            ...
            App::currentWorld->sceneObjects->deselectObjects();
            App::currentWorld->collections->addCollectionToSelection(int(it->getObjectHandle()));
            std::vector<int> sel;
            for (size_t i = 0; i < App::currentWorld->sceneObjects->getSelectionCount(); i++)
                sel.push_back(App::currentWorld->sceneObjects->getObjectHandleFromSelectionIndex(i));
            App::currentWorld->sceneObjects->deselectObjects();
...
{ // deprecated on 20.11.2020
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        {
            if (silentErrorPos == std::string::npos)
                CApiErrors::setLastError(__func__, SIM_ERROR_COLLISION_INEXISTANT);
            return (-1);
        }
        int retVal = it->getObjectHandle();
        return (retVal);
...
{ // deprecated on 20.11.2020
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        {
            if (silentErrorPos == std::string::npos)
                CApiErrors::setLastError(__func__, SIM_ERROR_DISTANCE_INEXISTANT);
            return (-1);
        }
        int retVal = it->getObjectHandle();
        return (retVal);
...
{ // deprecated on 23.11.2020
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        if (stream == nullptr)
        {
            CApiErrors::setLastError(__func__, SIM_ERROR_INVALID_DATA_STREAM);
            return (-1);
        }
        if (stream->getDataType() != GRAPH_NOOBJECT_USER_DEFINED)
        {
            CApiErrors::setLastError(__func__, SIM_ERROR_DATA_STREAM_NOT_USER_DEFINED);
            return (-1);
        }
...
{ // deprecated on 04.05.2021
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        CApiErrors::setLastError(__func__, SIM_ERROR_INVALID_ITEM_HANDLE);
        return (-1);
    }
    CApiErrors::setLastError(__func__, SIM_ERROR_COULD_NOT_LOCK_RESOURCES_FOR_READ);
    return (-1);
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_WRITE_DATA
    {
        ...
        if (originalText.length() > 127)
        {
            if ((handleFlags & sim_handleflag_silenterror) == 0)
                CApiErrors::setLastError(__func__, SIM_ERROR_ILLEGAL_OBJECT_NAME);
            return (-1);
        }
...
{ // deprecated on 18.06.2021
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        if ((index < 0) || (index >= int(App::currentWorld->sceneObjects->embeddedScriptContainer->allScripts.size())))
            return (-1);
        CDetachedScript* it = App::currentWorld->sceneObjects->embeddedScriptContainer->allScripts[index];
        int retVal = it->getScriptHandle();
        return (retVal);
    }
    CApiErrors::setLastError(__func__, SIM_ERROR_COULD_NOT_LOCK_RESOURCES_FOR_READ);
...
{ // deprecated on 18.06.2021
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        if (!doesObjectExist(__func__, objectHandle))
            return (-1);
        CDetachedScript* it = App::currentWorld->sceneObjects->embeddedScriptContainer->getScriptFromObjectAttachedTo(sim_scripttype_simulation, objectHandle);
        int retVal = -1;
        if (it == nullptr)
        {
...
{ // deprecated on 18.06.2021
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        if (!doesObjectExist(__func__, objectHandle))
            return (-1);
        CDetachedScript* it = App::currentWorld->sceneObjects->embeddedScriptContainer->getScriptFromObjectAttachedTo(sim_scripttype_customization, objectHandle);
        int retVal = -1;
        if (it == nullptr)
        {
            if (App::currentWorld->sceneObjects->getScriptFromHandle(objectHandle) != nullptr)
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        if (it == nullptr)
        {
            if ((silentErrorPos == std::string::npos) && ((options & 1) == 0))
            {
                ...
                CApiErrors::setLastError(__func__, additionalMessage_backCompatibility.c_str());
            }
            return (-1);
        }
        int retVal = it->getObjectHandle();
...
{ // deprecated on 27.04.2022
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_WRITE_DATA
    {
        ...
        if (pType == 0)
        {
            CApiErrors::setLastError(__func__, SIM_ERROR_INVALID_TYPE);
            return (-1);
        }
        int op = 0;
        if ((options & 1) != 0)
...
{ // deprecated on 13.10.2022
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        CDetachedScript* it = App::worldContainer->getDetachedScriptFromHandle(_currentScriptHandle);
        if (it != nullptr)
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_WRITE_DATA
    {
        ...
        if (it != nullptr)
        {
                ...
                (it->getScriptType() == sim_scripttype_customization))
            {
                    ...
                    { // object does exist
                        ...
                        { // script not yet associated
                            ...
                            if (currentSimilarObj == nullptr)
                            {
                                ...
                                CApiErrors::setLastError(__func__,
                ...
                CApiErrors::setLastError(__func__, SIM_ERROR_SCRIPT_NOT_CHILD_OR_CUSTOMIZATION_SCRIPT);
        }
        else
            CApiErrors::setLastError(__func__, SIM_ERROR_SCRIPT_INEXISTANT);
        return (retVal);
...
char* simGetStringSignal_internal(const char* signalName, int* stringLength)
{
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        pName += signalName;
        char* retVal = nullptr;
        int res = simGetBufferProperty_internal(sim_handle_scene, pName.c_str(), &retVal, stringLength);
        if (res < 1)
            retVal = nullptr;
        CApiErrors::getAndClearLastError();
        return retVal;
...
int simSetObjectInt32Param_internal(int objectHandle, int parameterID, int parameter)
{
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        if (retVal == 0)
            CApiErrors::setLastError(__func__, SIM_ERROR_COULD_NOT_SET_PARAMETER);
        return (retVal);
    }
    CApiErrors::setLastError(__func__, SIM_ERROR_COULD_NOT_LOCK_RESOURCES_FOR_READ);
    return (-1);
