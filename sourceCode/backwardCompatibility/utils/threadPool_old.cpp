...
VTHREAD_ID_TYPE CThreadPool_old::createNewThread(VTHREAD_START_ADDRESS threadStartAddress)
{
    ...
    VThread::launchThread(_intermediateThreadStartPoint);
    ...
    if (fqs > 1)
    {
        ...
        for (size_t i = 0; i < _allThreadData.size(); i++)
        {
            if (VThread::areThreadIdsSame(_allThreadData[i]->threadID, fID))
            {
                ...
                if (order > 0)
                    order = 1;
                if (int(_allThreadData[i]->threadResumeLocationAndOrder / 3) !=
                    location) // these 2 lines new since 4/6/2015 (forward-relocated scripts would only execute in next
                              // simulation pass)
...
void CThreadPool_old::switchToThread(VTHREAD_ID_TYPE threadID)
{
    ...
    {
        _unlock(3);
        return; // We have a free-running thread here (cannot be stopped if it doesn't itself disable the free-running
                // mode)
    }
    for (size_t i = 0; i < _allThreadData.size(); i++)
    {
        if (VThread::areThreadIdsSame(_allThreadData[i]->threadID, threadID))
        {
            ...
            { // we switch back to a previous (calling) fiber
                ...
                { // Regular situation
                    ...
                    _threadStartTime[fql - 2] +=
                        totalTimeInMs; // We have to "remove" the time spent in the called fiber!
                ...
                if (it != nullptr)
                {
                    ...
                    // If we arrived here, it is because CThreadPool_old::switchToFiberOrThread was called for this
            ...
            { // we switch forward to an auxiliary thread
                ...
                // If we arrived here, it is because CThreadPool_old::switchToFiberOrThread was called for this thread
                // from another thread
                it->threadWantsResumeFromYield = false; // We reset it
                // Now this thread resumes!
...
bool CThreadPool_old::switchBackToPreviousThread()
{
    ...
    {                                // Switch back only if not main thread, and not external thread
        ...
        {
            if (VThread::areThreadIdsSame(
                    VThread::getCurrentThreadId(),
                    _threadQueue[fql - 1])) // new since 10/11/2014. Now thread switch can also be called from the C API
            {
                int totalTimeInMs = VDateTime::getTimeDiffInMs(_threadStartTime[fql - 1]);
                for (size_t i = 0; i < _allThreadData.size(); i++)
                {
...
VTHREAD_RETURN_TYPE CThreadPool_old::_intermediateThreadStartPoint(VTHREAD_ARGUMENT_TYPE lpData)
{
    ...
    // If we arrived here, it is because CThreadPool_old::switchToFiberOrThread was called for this thread from another
    // thread
    it->threadWantsResumeFromYield = false; // We reset it
...
int CThreadPool_old::handleThread_ifHasResumeLocation(VTHREAD_ID_TYPE theThread, bool allThreadsWithResumeLocation,
                                                      int location)
{
    int retVal = 0;
    _lock(8);
        ...
        for (size_t i = 1; i < _allThreadData.size(); i++)
        {
            if (allThreadsWithResumeLocation || (_allThreadData[i]->threadID == theThread))
            {
                ...
                { // We first execute those with 0, then 1, then 2! (then 3 in the sensing phase!)
                    if ((_allThreadData[i]->threadExecutionTime == -1) ||
                        (_allThreadData[i]->allowToExecuteAgainInThisSimulationStep && (!doAll)))
                    {
                        _allThreadData[i]->allowToExecuteAgainInThisSimulationStep = false;
                        // Following is a special condition to support free-running mode:
                        if ((!_allThreadData[i]->threadShouldRunFreely) &&
                            (!_allThreadData[i]->threadSwitchShouldTriggerNoOtherThread))
                        {
                            std::string tmp("==. In fiber/thread handling routine (fiberID/threadID: ");
                            tmp +=
                                boost::lexical_cast<std::string>((unsigned long)_threadQueue[_threadQueue.size() - 1]);
                            tmp += ")";
                            App::logMsg(sim_verbosity_tracelua, tmp.c_str());
...
bool CThreadPool_old::getSimulationStopRequestedAndActivated()
{
    ...
    bool retVal = false;
    if (_simulationStopRequest)
        retVal = VDateTime::getTimeDiffInMs(_simulationStopRequestTime) >=
                 App::userSettings->threadedScriptsStoppingGraceTime;
    _unlock(15);
    return (retVal);
}
