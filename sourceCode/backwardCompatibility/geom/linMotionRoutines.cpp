...
double CLinMotionRoutines::getDistanceToDeceleration(double currentVelocity, double absAccel, double targetVelocity,
                                                     double distanceToLimit)
{ // absAccel and currentVelocity have to be different from zero!
    // 1. Until when can we go on at this speed before we have to decelerate? (t1==same speed duration, t0==deceleration
    // duration) (d1=same speed distance, d0=deceleration distance)
...
{ // accelAbs cannot be zero! position and velocity will be updated to the next position and velocity values
    ...
    // getNextValues again with different limits) Handle trivial cases:
    ...
    {   // 1.0.0 We first try to go to zero. The only thing that limits us here is the deltaTime that might not be enough
        ...
        { // 1.B.0 We will reach zero velocity before we reach a limit!
            ...
            { // 1.B.B We can reach zero velocity and even go further. We do this in 2 steps:
                ...
                return (getNextValues(position, velocity, targetVelocity, accelAbs, lowerLimitPosition,
                                      upperLimitPosition, lowerMaxAbsVel, upperMaxAbsVel, deltaTime));
            }
        }
    }
    else
    { // 2.0.0 Here velocity always stays with the same sign (not going from pos to neg or opposite):
        ...
        { // 2.B.0  signedAccel means speeding down!
            ...
            { // 2.B.A Yes we can:
                ...
                double t = fabs((nextLimitPos - position) /
                                velocity); // constant speed! no divide by zero problem here (velocity cannot be zero!)
                if (t >= deltaTime)
        ...
        { // 2.C.0   signedAccel means speeding down!
            ...
            if (fabs(velocity) >= nextLimitMaxSpeed)
            {
                ...
                { // 2.C.A We cannot speed-up! we have to decelerate
                    ...
                    return (getNextValues(
                        position, velocity, nextLimitMaxSpeed * (targetVelocity / fabs(targetVelocity)), accelAbs,
                        lowerLimitPosition, upperLimitPosition, lowerMaxAbsVel, upperMaxAbsVel, deltaTime));
                }
            }
            ...
            {                                                                                  // 2.C.B.2
                ...
                else
                {
                    ...
                    if (halfDist > 0.0)
                    {
                        ...
                        maxSpeed = getNextVelocityForTimeStep_lvp(nextLimitMaxSpeed, accelAbs, t2) *
            ...
            return (getNextValues(position, velocity, velocity, accelAbs, lowerLimitPosition, upperLimitPosition,
                                  lowerMaxAbsVel, upperMaxAbsVel, deltaTime));
        }
        return (true); // should never happen!
    }
}

double CLinMotionRoutines::getMaxVelocityAtPosition(double position, double accelAbs, double lowerLimitPosition,
                                                    double upperLimitPosition, double lowerMaxAbsVel,
                                                    double upperMaxAbsVel)
...
double CLinMotionRoutines::getCurrentVelocityOnPath(double position, double nominalVelocity, double lowerLimitPosition,
                                                    double upperLimitPosition, double lowerMaxRelVel,
                                                    double upperMaxRelVel)
{
...
{ // position and velocity will be updated to the next position and velocity values. accelAbs cannot be zero.
    ...
    double velocity = getCurrentVelocityOnPath(position, nominalVelocity, lowerLimitPosition, upperLimitPosition,
                                               lowerMaxRelVel, upperMaxRelVel);
    ...
    if ((lowerMaxRelVel * fabs(nominalVelocity) < LIN_MOTION_VELOCITY_TOLERANCE) &&
        (upperMaxRelVel * fabs(nominalVelocity) < LIN_MOTION_VELOCITY_TOLERANCE))
    {
        deltaTime = 0.0;
        velocity = 0.0;
        return (false);
    ...
    { // Regular cases (acceleration or deceleration (constant vel case was already handled previously)):
        ...
        { // We have to accelerate
            ...
            { // We accelerate and then reach the target velocity:
                nominalVelocity = targetNominalVelocity;
                deltaTime -= t;
                adjustNominalVelocity(nominalVelocity, targetNominalVelocity, acceleration,
                                      deltaTime); // Here we continue at constant velocity
            }
            else
                                              ...
                                              double lowerMaxRelVel, double upperMaxRelVel, double& deltaTime)
{
    ...
    bool retVal = getNextValuesForPath(position, nominalVelocity, lowerLimitPosition, upperLimitPosition,
                                       lowerMaxRelVel, upperMaxRelVel, deltaTime);
