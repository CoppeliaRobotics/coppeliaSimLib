...
class CPathCont_old
{
  public:
    ...
    void handlePath_keepObjectUnchanged(
        double deltaTime, double &pos_, double &nomVel_, double &targetNomVel_, double maxAccel, bool invertVelocity,
        bool infiniteAccel); // so that the path can be handled by simFollowPath functions or such!
  ...
  protected:
    ...
    void _applyDistinctConsecutivePoints(const std::vector<CPathPoint_old *> &ptCont, std::vector<C4X4Matrix> &tr,
                                         std::vector<std::vector<int>> &ptIndices);
  ...
  protected:
    void _draw(std::vector<CPathPoint_old *> &ptCont, bool pathEditMode, bool isPath, bool showLine, bool stripplePts,
               bool showSquare, bool showOrientation, int lineSize, double squareSize, bool pathIsOnlySelectedObject,
               int objectID);
#endif
};
