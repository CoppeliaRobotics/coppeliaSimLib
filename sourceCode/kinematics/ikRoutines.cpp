...
CMatrix* CIkRoutine::getJacobian(CIkElement* ikElement,C4X4Matrix& tooltipTransf,std::vector<int>* rowJointIDs,std::vector<int>* rowJointStages)
{   // rowJointIDs is nullptr by default. If not null, it will contain the ids of the joints
    ...
    CDummy* tooltip=App::currentWorld->sceneObjects->getDummyFromHandle(ikElement->getTipHandle());
    if (tooltip==nullptr)
    { // Should normally never happen!
        // 28/4/2016 ikElement->setActive(false);
        return(nullptr);
    }
    CSceneObject* base=App::currentWorld->sceneObjects->getObjectFromHandle(ikElement->getBase());
    ...
    while (iterat!=base)
    {
        CSceneObject* nextIterat=iterat->getParent();
        C7Vector local;
        if (iterat->getObjectType()==sim_object_joint_type)
        {
            if ( (((CJoint*)iterat)->getJointMode()!=sim_jointmode_ik)&&(((CJoint*)iterat)->getJointMode()!=sim_jointmode_reserved_previously_ikdependent)&&(((CJoint*)iterat)->getJointMode()!=sim_jointmode_dependent) )
                local=iterat->getFullLocalTransformation_ikOld();
            else
            {
                ...
                if (it->getJointType()==sim_joint_spherical_subtype)
                {
                    ...
                    local=iterat->getLocalTransformation();
            }
        }
        else
            local=iterat->getFullLocalTransformation_ikOld();
        ...
        {   // If base is nullptr then the second part is not evaluated (iterat->getObjectType())
            ...
            {   // Here we have a joint:
                if (lastJoint->getJointType()==sim_joint_revolute_subtype)
                {
                    ...
                    paramPart.buildZRotation(lastJoint->getPosition_useTempValues());
                }
                else if (lastJoint->getJointType()==sim_joint_prismatic_subtype)
                {
                    buildDeltaZTranslation(d0,dp);
                    multiply(d0,dp,positionCounter,jMatrices);
                    paramPart.buildTranslation(0.0f,0.0f,lastJoint->getPosition_useTempValues());
                }
                else 
    ...
    {
        CDummy* alb=App::currentWorld->sceneObjects->getDummyFromHandle(alternativeBaseForConstraints);
        if (alb!=nullptr)
        { // We want everything relative to the alternativeBaseForConstraints dummy orientation!
            C7Vector alternativeBase(alb->getCumulativeTransformation_ikOld());
            C7Vector currentBase;
            currentBase.setIdentity();
            if (base!=nullptr)
                currentBase=base->getFullCumulativeTransformation_ikOld(); // could be a joint, we want also the joint intrinsic transformation part!
...
CMatrix* CIkRoutine::getAvoidanceJacobian(CSceneObject* base,CSceneObject* avoidingObject,const C4X4Matrix& relPos,std::vector<int>* rowJointIDs,std::vector<int>* rowJointStages)
{   // rowJointIDs is nullptr by default. If not null, it will contain the ids of the joints
    // corresponding to the rows of the jacobian.
    ...
    // We check the number of degrees of freedom and prepare the rowJointIDs vector:
    CSceneObject* iterat=avoidingObject;
    int doF=0;
    while (iterat!=base)
    {
        iterat=iterat->getParent();
    ...
    while (iterat!=base)
    {
        ...
        if (iterat->getObjectType()==sim_object_joint_type)
        {
            ...
            local=iterat->getFullLocalTransformation_ikOld().getMatrix();
        buff=C4X4FullMatrix(local.getMatrix())*buff;
        iterat=nextIterat;
        bool activeJoint=false;
        if (iterat!=nullptr)
...
//  for (int i=0;i<doF;i++)
//  {
//      CJoint* act=App::currentWorld->objCont->getJoint(rowJointIDs->at(i));
