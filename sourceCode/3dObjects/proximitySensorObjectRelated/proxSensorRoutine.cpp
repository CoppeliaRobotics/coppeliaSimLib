...
{ // -2: sensor triggered in the forbidden zone, -1: sensor didn't trigger. Otherwise the object handle that triggered the sensor
    ...
    C7Vector shapeITr(inv*shape->getCumulativeTransformation());
...
{ // -2: sensor triggered in the forbidden zone, -1: sensor didn't trigger. Otherwise the object handle that triggered the sensor
    ...
    C7Vector octreeITr(sensTrInv*octree->getCumulativeTransformation());
...
{ // -2: sensor triggered in the forbidden zone, -1: sensor didn't trigger. Otherwise the object handle that triggered the sensor
    ...
    C7Vector sensTr(sensor->getCumulativeTransformation());
    C7Vector sensTrInv(sensTr.getInverse());
    C7Vector pointCloudITr(sensTrInv*pointCloud->getCumulativeTransformation());
...
bool CProxSensorRoutine::_doesSensorVolumeOverlapWithObjectBoundingBox(CProxSensor* sensor,C3DObject* obj)
{
    ...
    if (obj->getObjectType()==sim_object_shape_type)
    {
        ...
        objectTr=obj->getCumulativeTransformation();
    }
    if (obj->getObjectType()==sim_object_dummy_type)
    {
        objectIsPoint=true;
...
int CProxSensorRoutine::_detectObject(CProxSensor* sensor,C3DObject* object,C3Vector& detectedPt,float& dist,C3Vector& triNormalNotNormalized,bool closestFeatureMode,bool angleLimitation,float maxAngle,bool frontFace,bool backFace,float minThreshold)
{
    int retVal=-1;
