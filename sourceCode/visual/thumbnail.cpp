    ...
    if (verticalFlip)
    {
        for (int i = 0; i < 128; i++)
        {
            for (int j = 0; j < 128; j++)
            {
                ...
                {
                    for (int k = 0; k < 4; k++)
                        _thumbnailRGBAImage[4 * (i * 128 + j) + k] =
                            uncompressedRGBAImage[4 * ((127 - i) * 128 + j) + k];
                }
                else
                {
                    for (int k = 0; k < 3; k++)
                        _thumbnailRGBAImage[4 * (i * 128 + j) + k] =
                            uncompressedRGBAImage[3 * ((127 - i) * 128 + j) + k];
                    ...
                    { // for backward compatibility (7/3/2012)
                        ...
                        if (imagePresent != 0)
                        {
                            ...
                            for (int j = 0; j < 128 * 128; j++)
                                _thumbnailRGBAImage[4 * j + 3] =
                                    char(double((unsigned char)(_thumbnailRGBAImage[4 * j + 3])) / 0.22508);
                        }
                    }
