...
{ // namespaceAndVersion: e.g. simAssimp, simAssimp-2-78, etc.
    ...
    if (plug == nullptr)
    {
        ...
        CDetachedScript* scr = App::worldContainer->getDetachedScriptFromUid(loadOrigin);
        if (scr != nullptr)
...
{ // not for legacy plugins
    ...
    if ((it != nullptr) && (!it->isLegacyPlugin()))
    {
        ...
        if (force || (!it->hasAnyDependency()))
        {
            ...
            CDetachedScript* scr = App::worldContainer->getDetachedScriptFromUid(unloadOrigin);
            if (scr != nullptr)
                App::logScriptMsg(scr, sim_verbosity_loadinfos | sim_verbosity_onlyterminal, msg.c_str());
            else
                App::logMsg(sim_verbosity_loadinfos | sim_verbosity_onlyterminal, msg.c_str());
                                                                ...
                                                                void* data /*=nullptr*/, int retVals[4] /*=nullptr*/)
{
    ...
    for (size_t i = 0; i < _allPlugins.size(); i++)
    {
        ...
        { // only old plugins
            ...
            if (retData || ((retVals != nullptr) &&
                            ((retVals[0] != -1) || (retVals[1] != -1) || (retVals[2] != -1) || (retVals[3] != -1))))
            {
                if (msg != sim_message_eventcallback_mainscriptabouttobecalled) // this message is handled in a special
                                                                                // fashion, because the remoteApi and
                                                                                // ROS might interfere otherwise!
                                            ...
                                            const float* diffuse, const float* specular, const float* emission)
{
    ...
    {
        currentDynEngine->pushCurrentPlugin();
        retVal = currentDynEngine->dynPlugin_addParticleObject(objectType, size, massOverVolume, params, lifeTime,
                                                               maxItemCount, ambient, diffuse, specular, emission);
        currentDynEngine->popCurrentPlugin();
    }
    return (retVal);
}
...
{ // returns the mass-less diagonal inertia. Returned mass is for a density of 1000
    ...
    {
        mujocoEngine->pushCurrentPlugin();
        mass = mujocoEngine->mujocoPlugin_computePMI(vertices.data(), int(vertices.size()), indices.data(),
                                                     int(indices.size()), tr.X.data, tr.Q.data, diagI.data);
        mujocoEngine->popCurrentPlugin();
    }
    return (mass);
}
                                              ...
                                              int maxTrianglesInBoundingBox /*=8*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_createMesh(vertices, verticesSize, indices, indicesSize, _tr,
                                                          triangleEdgeMaxLength, maxTrianglesInBoundingBox);
        currentGeomPlugin->popCurrentPlugin();
    }
    return (retVal);
}
                                                             ...
                                                             double proximityTol /*=0.001*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        currentGeomPlugin->pushCurrentPlugin();
        retVal =
            currentGeomPlugin->geomPlugin_intersectPointsWithPtcloud(pcStruct, _tr, points, pointCnt, proximityTol);
        currentGeomPlugin->popCurrentPlugin();
    }
    return (retVal);
}
                                                       ...
                                                       int* mesh1Caching /*=nullptr*/, int* mesh2Caching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        if (intersections != nullptr)
            _int = &_intersections;
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getMeshMeshCollision(mesh1ObbStruct, _tr1, mesh2ObbStruct, _tr2, _int,
                                                                    &_intersectionsSize, mesh1Caching, mesh2Caching);
                                                         ...
                                                         unsigned long long int* ocCaching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        octreeTransformation.getData(_tr2);
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getMeshOctreeCollision(meshObbStruct, _tr1, ocStruct, _tr2, meshCaching,
                                                                      ocCaching);
        currentGeomPlugin->popCurrentPlugin();
    }
    return (retVal);
                                                           ...
                                                           int* caching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        if (intersections != nullptr)
            _int = &_intersections;
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getMeshTriangleCollision(meshObbStruct, _tr, p.data, v.data, w.data,
                                                                        _int, &_intersectionsSize, caching);
        if (retVal && (intersections != nullptr))
        {
            intersections->assign(_intersections, _intersections + _intersectionsSize);
            currentGeomPlugin->geomPlugin_releaseBuffer(_intersections);
                                                          ...
                                                          int* caching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        if (intersections != nullptr)
            _int = &_intersections;
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getMeshSegmentCollision(
            meshObbStruct, _tr, segmentExtremity.data, segmentVector.data, _int, &_intersectionsSize, caching);
        if (retVal && (intersections != nullptr))
        {
            intersections->assign(_intersections, _intersections + _intersectionsSize);
            currentGeomPlugin->geomPlugin_releaseBuffer(_intersections);
                                                           ...
                                                           unsigned long long int* oc2Caching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        octree2Transformation.getData(_tr2);
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getOctreeOctreeCollision(oc1Struct, _tr1, oc2Struct, _tr2, oc1Caching,
                                                                        oc2Caching);
        currentGeomPlugin->popCurrentPlugin();
    }
    return (retVal);
                                                            ...
                                                            unsigned long long int* pcCaching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getOctreePtcloudCollision(ocStruct, _tr1, pcStruct, _tr2, ocCaching,
                                                                         pcCaching);
        currentGeomPlugin->popCurrentPlugin();
    }
    return (retVal);
                                                             ...
                                                             unsigned long long int* caching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        currentGeomPlugin->pushCurrentPlugin();
        retVal =
            currentGeomPlugin->geomPlugin_getOctreeTriangleCollision(ocStruct, _tr1, p.data, v.data, w.data, caching);
        currentGeomPlugin->popCurrentPlugin();
    }
    return (retVal);
}
                                                            ...
                                                            unsigned long long int* caching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        octreeTransformation.getData(_tr1);
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getOctreeSegmentCollision(ocStruct, _tr1, segmentExtremity.data,
                                                                         segmentVector.data, caching);
        currentGeomPlugin->popCurrentPlugin();
    }
    return (retVal);
}
                                                     ...
                                                     bool boxesAreSolid)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getBoxBoxCollision(_tr1, box1HalfSize.data, _tr2, box2HalfSize.data,
                                                                  boxesAreSolid);
        currentGeomPlugin->popCurrentPlugin();
    }
    return (retVal);
                                                          ...
                                                          const C3Vector& p, const C3Vector& v, const C3Vector& w)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getBoxTriangleCollision(_tr1, boxHalfSize.data, boxIsSolid, p.data,
                                                                       v.data, w.data);
        currentGeomPlugin->popCurrentPlugin();
    }
    return (retVal);
                                                         ...
                                                         const C3Vector& segmentVector)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        boxTransformation.getData(_tr1);
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getBoxSegmentCollision(_tr1, boxHalfSize.data, boxIsSolid,
                                                                      segmentEndPoint.data, segmentVector.data);
        currentGeomPlugin->popCurrentPlugin();
    }
    return (retVal);
}
                                                               ...
                                                               std::vector<double>* intersections /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        if (intersections != nullptr)
            _int = &_intersections;
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getTriangleTriangleCollision(p1.data, v1.data, w1.data, p2.data, v2.data,
                                                                            w2.data, _int, &_intersectionsSize);
        if (retVal && (intersections != nullptr))
        {
            intersections->assign(_intersections, _intersections + _intersectionsSize);
            currentGeomPlugin->geomPlugin_releaseBuffer(_intersections);
                                                              ...
                                                              std::vector<double>* intersections /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        if (intersections != nullptr)
            _int = &_intersections;
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getTriangleSegmentCollision(
            p.data, v.data, w.data, segmentEndPoint.data, segmentVector.data, _int, &_intersectionsSize);
        if (retVal && (intersections != nullptr))
        {
            intersections->assign(_intersections, _intersections + _intersectionsSize);
            currentGeomPlugin->geomPlugin_releaseBuffer(_intersections);
    ...
    C3Vector* minDistSegPt2 /*=nullptr*/, int* mesh1Caching /*=nullptr*/, int* mesh2Caching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getMeshMeshDistanceIfSmaller(mesh1ObbStruct, _tr1, mesh2ObbStruct, _tr2,
                                                                            &dist, _minDistSegPt1, _minDistSegPt2,
                                                                            mesh1Caching, mesh2Caching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
    ...
    C3Vector* ocMinDistPt /*=nullptr*/, int* meshCaching /*=nullptr*/, unsigned long long int* ocCaching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getMeshOctreeDistanceIfSmaller(
            meshObbStruct, _tr1, ocStruct, _tr2, &dist, _minDistPt1, _minDistPt2, meshCaching, ocCaching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
    ...
    C3Vector* pcMinDistPt /*=nullptr*/, int* meshCaching /*=nullptr*/, unsigned long long int* pcCaching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getMeshPtcloudDistanceIfSmaller(
            meshObbStruct, _tr1, pcStruct, _tr2, &dist, _minDistPt1, _minDistPt2, meshCaching, pcCaching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (meshMinDistPt != nullptr)
    ...
    int* caching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getMeshTriangleDistanceIfSmaller(
            meshObbStruct, _tr, p.data, v.data, w.data, &dist, _minDistPt1, _minDistPt2, caching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (minDistSegPt1 != nullptr)
    ...
    C3Vector* minDistSegPt2 /*=nullptr*/, int* caching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getMeshSegmentDistanceIfSmaller(
            meshObbStruct, _tr, segmentEndPoint.data, segmentVector.data, &dist, _minDistPt1, _minDistPt2, caching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (minDistSegPt1 != nullptr)
                                                                ...
                                                                int* caching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt1[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getMeshPointDistanceIfSmaller(meshObbStruct, _tr, point.data, &dist,
                                                                             _minDistPt1, caching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
    ...
    unsigned long long int* oc2Caching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getOctreeOctreeDistanceIfSmaller(
            oc1Struct, _tr1, oc2Struct, _tr2, &dist, _minDistPt1, _minDistPt2, oc1Caching, oc2Caching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
    ...
    unsigned long long int* ocCaching /*=nullptr*/, unsigned long long int* pcCaching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getOctreePtcloudDistanceIfSmaller(
            ocStruct, _tr1, pcStruct, _tr2, &dist, _minDistPt1, _minDistPt2, ocCaching, pcCaching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (ocMinDistPt != nullptr)
    ...
    unsigned long long int* ocCaching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getOctreeTriangleDistanceIfSmaller(
            ocStruct, _tr, p.data, v.data, w.data, &dist, _minDistPt1, _minDistPt2, ocCaching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (ocMinDistPt != nullptr)
    ...
    C3Vector* segMinDistPt /*=nullptr*/, unsigned long long int* ocCaching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getOctreeSegmentDistanceIfSmaller(
            ocStruct, _tr, segmentEndPoint.data, segmentVector.data, &dist, _minDistPt1, _minDistPt2, ocCaching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (ocMinDistPt != nullptr)
                                                                  ...
                                                                  unsigned long long int* ocCaching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt1[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getOctreePointDistanceIfSmaller(ocStruct, _tr, point.data, &dist,
                                                                               _minDistPt1, ocCaching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (ocMinDistPt != nullptr)
    ...
    unsigned long long int* pc1Caching /*=nullptr*/, unsigned long long int* pc2Caching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getPtcloudPtcloudDistanceIfSmaller(
            pc1Struct, _tr1, pc2Struct, _tr2, &dist, _minDistPt1, _minDistPt2, pc1Caching, pc2Caching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
    ...
    unsigned long long int* pcCaching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getPtcloudTriangleDistanceIfSmaller(
            pcStruct, _tr, p.data, v.data, w.data, &dist, _minDistPt1, _minDistPt2, pcCaching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (pcMinDistPt != nullptr)
    ...
    C3Vector* segMinDistPt /*=nullptr*/, unsigned long long int* pcCaching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getPtcloudSegmentDistanceIfSmaller(
            pcStruct, _tr, segmentEndPoint.data, segmentVector.data, &dist, _minDistPt1, _minDistPt2, pcCaching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (pcMinDistPt != nullptr)
                                                                   ...
                                                                   unsigned long long int* pcCaching /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt1[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getPtcloudPointDistanceIfSmaller(pcStruct, _tr, point.data, &dist,
                                                                                _minDistPt1, pcCaching);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (pcMinDistPt != nullptr)
                                                            ...
                                                            const C3Vector& box2HalfSize)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        currentGeomPlugin->pushCurrentPlugin();
        retVal =
            currentGeomPlugin->geomPlugin_getApproxBoxBoxDistance(_tr1, box1HalfSize.data, _tr2, box2HalfSize.data);
        currentGeomPlugin->popCurrentPlugin();
    }
    return (retVal);
}
    ...
    C3Vector* distSegPt2 /*=nullptr*/, bool altRoutine /*=false*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getBoxBoxDistanceIfSmaller(
            _tr1, box1HalfSize.data, _tr2, box2HalfSize.data, boxesAreSolid, &dist, _minDistPt1, _minDistPt2);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
    ...
    C3Vector* distSegPt2 /*=nullptr*/, bool altRoutine /*=false*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getBoxTriangleDistanceIfSmaller(
            _tr, boxHalfSize.data, boxIsSolid, p.data, v.data, w.data, &dist, _minDistPt1, _minDistPt2);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (distSegPt1 != nullptr)
    ...
    bool altRoutine /*=false*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getBoxSegmentDistanceIfSmaller(_tr, boxHalfSize.data, boxIsSolid,
                                                                              segmentEndPoint.data, segmentVector.data,
                                                                              &dist, _minDistPt1, _minDistPt2);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (distSegPt1 != nullptr)
                                                               ...
                                                               C3Vector* distSegPt1 /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt1[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getBoxPointDistanceIfSmaller(_tr, boxHalfSize.data, boxIsSolid,
                                                                            point.data, &dist, _minDistPt1);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (distSegPt1 != nullptr)
    ...
    const C3Vector& w2, double& dist, C3Vector* minDistSegPt1 /*=nullptr*/, C3Vector* minDistSegPt2 /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getTriangleTriangleDistanceIfSmaller(
            p1.data, v1.data, w1.data, p2.data, v2.data, w2.data, &dist, _minDistPt1, _minDistPt2);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (minDistSegPt1 != nullptr)
                                                                      ...
                                                                      C3Vector* minDistSegPt2 /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getTriangleSegmentDistanceIfSmaller(
            p.data, v.data, w.data, segmentEndPoint.data, segmentVector.data, &dist, _minDistPt1, _minDistPt2);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (minDistSegPt1 != nullptr)
                                                                    ...
                                                                    double& dist, C3Vector* minDistSegPt /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        double _minDistPt1[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getTrianglePointDistanceIfSmaller(p.data, v.data, w.data, point.data,
                                                                                 &dist, _minDistPt1);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
                                                                     ...
                                                                     C3Vector* minDistSegPt2 /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _minDistPt2[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getSegmentSegmentDistanceIfSmaller(
            segment1EndPoint.data, segment1Vector.data, segment2EndPoint.data, segment2Vector.data, &dist, _minDistPt1,
            _minDistPt2);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
                                                                   ...
                                                                   double& dist, C3Vector* minDistSegPt /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        double _minDistPt1[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_getSegmentPointDistanceIfSmaller(
            segmentEndPoint.data, segmentVector.data, point.data, &dist, _minDistPt1);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (minDistSegPt != nullptr)
    ...
    C3Vector* triN /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _triN[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_volumeSensorDetectMeshIfSmaller(
            _planesIn, int(planesIn.size()), _planesOut, int(planesOut.size()), obbStruct, _tr, &dist, fast,
            frontDetection, backDetection, maxAngle, _detectPt, _triN);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (detectPt != nullptr)
    ...
    C3Vector* triN /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _triN[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_volumeSensorDetectOctreeIfSmaller(
            _planesIn, int(planesIn.size()), _planesOut, int(planesOut.size()), ocStruct, _tr, &dist, fast,
            frontDetection, backDetection, maxAngle, _detectPt, _triN);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
    ...
    const C7Vector& ptcloudTransformation, double& dist, bool fast /*=false*/, C3Vector* detectPt /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _detectPt[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_volumeSensorDetectPtcloudIfSmaller(
            _planesIn, int(planesIn.size()), _planesOut, int(planesOut.size()), pcStruct, _tr, &dist, fast, _detectPt);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (detectPt != nullptr)
    ...
    double maxAngle /*=0.0*/, C3Vector* detectPt /*=nullptr*/, C3Vector* triN /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _triN[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_volumeSensorDetectTriangleIfSmaller(
            _planesIn, int(planesIn.size()), _planesOut, int(planesOut.size()), p.data, v.data, w.data, &dist,
            frontDetection, backDetection, maxAngle, _detectPt, _triN);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
    ...
    const C3Vector& segmentVector, double& dist, double maxAngle /*=0.0*/, C3Vector* detectPt /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        double _detectPt[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_volumeSensorDetectSegmentIfSmaller(
            _planesIn, int(planesIn.size()), _planesOut, int(planesOut.size()), segmentEndPoint.data,
            segmentVector.data, &dist, maxAngle, _detectPt);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (detectPt != nullptr)
    ...
    C3Vector* triN /*=nullptr*/, bool* forbiddenDistTouched /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _triN[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_raySensorDetectMeshIfSmaller(
            rayStart.data, rayVect.data, obbStruct, _tr, &dist, forbiddenDist, fast, frontDetection, backDetection,
            maxAngle, _detectPt, _triN, forbiddenDistTouched);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
            if (detectPt != nullptr)
    ...
    C3Vector* triN /*=nullptr*/, bool* forbiddenDistTouched /*=nullptr*/)
{
    ...
    if (currentGeomPlugin != nullptr)
    {
        ...
        double _triN[3];
        currentGeomPlugin->pushCurrentPlugin();
        retVal = currentGeomPlugin->geomPlugin_raySensorDetectOctreeIfSmaller(
            rayStart.data, rayVect.data, ocStruct, _tr, &dist, forbiddenDist, fast, frontDetection, backDetection,
            maxAngle, _detectPt, _triN, forbiddenDistTouched);
        currentGeomPlugin->popCurrentPlugin();
        if (retVal)
        {
...
void CPluginContainer::oldIkPlugin_setJointDependency(int jointHandle, int dependencyJointHandle, double offset,
                                                      double mult)
{
    if (currentIKPlugin == nullptr)
        currentIKPlugin = _tryToLoadPluginOnce(SIMIK0_DEFAULT);
    ...
    {
        currentIKPlugin->pushCurrentPlugin();
        currentIKPlugin->oldIkPlugin_setJointDependency(ikEnvironment, jointHandle, dependencyJointHandle, offset,
                                                        mult);
        currentIKPlugin->popCurrentPlugin();
    }
}
...
void CPluginContainer::oldIkPlugin_setIkGroupCalculation(int ikGroupHandle, int method, double damping,
                                                         int maxIterations)
{
    if (currentIKPlugin == nullptr)
        currentIKPlugin = _tryToLoadPluginOnce(SIMIK0_DEFAULT);
    ...
    {
        currentIKPlugin->pushCurrentPlugin();
        currentIKPlugin->oldIkPlugin_setIkGroupCalculation(ikEnvironment, ikGroupHandle, method, damping,
                                                           maxIterations);
        currentIKPlugin->popCurrentPlugin();
    }
}
...
void CPluginContainer::oldIkPlugin_setIkElementBase(int ikGroupHandle, int ikElementIndex, int baseHandle,
                                                    int constraintsBaseHandle)
{
    if (currentIKPlugin == nullptr)
        currentIKPlugin = _tryToLoadPluginOnce(SIMIK0_DEFAULT);
    ...
    {
        currentIKPlugin->pushCurrentPlugin();
        currentIKPlugin->oldIkPlugin_setIkElementBase(ikEnvironment, ikGroupHandle, ikElementIndex, baseHandle,
                                                      constraintsBaseHandle);
        currentIKPlugin->popCurrentPlugin();
    }
}
...
void CPluginContainer::oldIkPlugin_setIkElementPrecision(int ikGroupHandle, int ikElementIndex, double linearPrecision,
                                                         double angularPrecision)
{
    ...
    {
        currentIKPlugin->pushCurrentPlugin();
        currentIKPlugin->oldIkPlugin_setIkElementPrecision(ikEnvironment, ikGroupHandle, ikElementIndex,
                                                           linearPrecision, angularPrecision);
        currentIKPlugin->popCurrentPlugin();
    }
}
void CPluginContainer::oldIkPlugin_setIkElementWeights(int ikGroupHandle, int ikElementIndex, double linearWeight,
                                                       double angularWeight)
{
    ...
    {
        currentIKPlugin->pushCurrentPlugin();
        currentIKPlugin->oldIkPlugin_setIkElementWeights(ikEnvironment, ikGroupHandle, ikElementIndex, linearWeight,
                                                         angularWeight);
        currentIKPlugin->popCurrentPlugin();
    }
}
                                                      ...
                                                      std::string& errString)
{
    ...
    if (currentIKPlugin != nullptr)
    {
        ...
        {
            currentIKPlugin->pushCurrentPlugin();
            retVal = oldIkPlugin_getConfigForTipPose(ikGroupHandle, jointCnt, jointHandles, thresholdDist,
                                                     maxIterationsOrTimeInMs, retConfig, metric, _validationCB,
                                                     jointOptions, lowLimits, ranges, errString);
            currentIKPlugin->popCurrentPlugin();
        }
    }
    else
                                       ...
                                       const bool* selection, const double* targetPos, const double* targetVel)
{
    ...
    if (currentRuckigPlugin != nullptr)
    {
        ...
        retVal = currentRuckigPlugin->ruckigPlugin_pos(scriptHandle, dofs, smallestTimeStep, flags, currentPos,
                                                       currentVel, currentAccel, maxVel, maxAccel, maxJerk, selection,
                                                       targetPos, targetVel);
                                       ...
                                       const double* targetVel)
{
    ...
    if (currentRuckigPlugin != nullptr)
    {
        currentRuckigPlugin->pushCurrentPlugin();
        retVal =
            currentRuckigPlugin->ruckigPlugin_vel(scriptHandle, dofs, smallestTimeStep, flags, currentPos, currentVel,
                                                  currentAccel, maxAccel, maxJerk, selection, targetVel);
        currentRuckigPlugin->popCurrentPlugin();
    }
    return (retVal);
}
                                           ...
                                           int options, int* shapeCount)
{
    ...
    if (currentAssimpPlugin != nullptr)
    {
        ...
        retVal =
            currentAssimpPlugin->_assimp_importShapes(fileNames, maxTextures, scaling, upVector, options, shapeCount);
                                           ...
                                           const char* format, double scaling, int upVector, int options)
{
    ...
    {
        currentAssimpPlugin->pushCurrentPlugin();
        currentAssimpPlugin->_assimp_exportShapes(shapeHandles, shapeCount, filename, format, scaling, upVector,
                                                  options);
        currentAssimpPlugin->popCurrentPlugin();
    }
    else
                                          ...
                                          int** indicesSizes)
{
    ...
    {
        currentAssimpPlugin->pushCurrentPlugin();
        retVal = currentAssimpPlugin->_assimp_importMeshes(fileNames, scaling, upVector, options, allVertices,
                                                           verticesSizes, allIndices, indicesSizes);
        currentAssimpPlugin->popCurrentPlugin();
    }
    else
        App::logMsg(sim_verbosity_errors, "simAssimp plugin was not found.");
                                           ...
                                           const char* format, double scaling, int upVector, int options)
{
    ...
    {
        currentAssimpPlugin->pushCurrentPlugin();
        currentAssimpPlugin->_assimp_exportMeshes(meshCnt, allVertices, verticesSizes, allIndices, indicesSizes,
                                                  filename, format, scaling, upVector, options);
        currentAssimpPlugin->popCurrentPlugin();
    }
    else
        App::logMsg(sim_verbosity_errors, "simAssimp plugin was not found.");
