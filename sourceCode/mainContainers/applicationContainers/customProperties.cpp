...
int CCustomProperties::getMatrix3x3Property(const char* pName, CMatrix& pState) const
{
    int propType, propInfo;
    std::string infoTxt;
    const char* dataPtr;
    ...
    if (propType != sim_propertytype_matrix3x3)
        return sim_propertyret_unavailable;
    if (dataLen >= 9 * sizeof(double))
    {
        pState.resize(3, 3, 0.0);
        std::memcpy(pState.data.data(), dataPtr, 9 * sizeof(double));
    }
    return sim_propertyret_ok;
}
...
int CCustomProperties::getMatrix4x4Property(const char* pName, CMatrix& pState) const
{
    int propType, propInfo;
    std::string infoTxt;
    const char* dataPtr;
    ...
    if (propType != sim_propertytype_matrix4x4)
        return sim_propertyret_unavailable;
    if (dataLen >= 16 * sizeof(double))
    {
        pState.resize(4, 4, 0.0);
        std::memcpy(pState.data.data(), dataPtr, 16 * sizeof(double));
    }
    return sim_propertyret_ok;
}
