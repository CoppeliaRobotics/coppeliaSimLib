...
int CSerialPortWin::Open(const char* portString, int nBaud)
{
    if (m_bOpened)
        return (TRUE);
    DCB dcb;
    m_hIDComDev = CreateFileA(portString, GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING,
    ...
    if (!SetCommState(m_hIDComDev, &dcb) || !SetupComm(m_hIDComDev, 10000, 10000) ||
        m_OverlappedRead.hEvent == nullptr || m_OverlappedWrite.hEvent == nullptr)
    {
        // DWORD dwError = GetLastError();
        if (m_OverlappedRead.hEvent != nullptr)
            CloseHandle(m_OverlappedRead.hEvent);
        if (m_OverlappedWrite.hEvent != nullptr)
