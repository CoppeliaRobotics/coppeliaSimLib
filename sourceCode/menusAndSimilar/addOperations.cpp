...
CShape *CAddOperations::addConvexHull(const std::vector<CSceneObject *> &inputObjects, double growDist,
        ...
        if (CMeshRoutines::getConvexHull(allHullVertices, vert, ind))
        {
            if (growDist > 0.0)
            {
                ...
                for (size_t j = 0; j < ind.size() / 3; j++)
                {
                    ...
                    C3Vector w[3] = {C3Vector(&vert2[0] + 3 * indd[0]), C3Vector(&vert2[0] + 3 * indd[1]),
                                     C3Vector(&vert2[0] + 3 * indd[2])};
            ...
            if ((vert.size() > 0) && (ind.size() > 0))
            {
                ...
                retVal->setObjectAltName_direct_old(
                    tt::getObjectAltNameFromObjectName(retVal->getObjectName_old().c_str()).c_str());

                if ((oneShape != nullptr) && (inputObjects.size() == 1))
                    oneShape->getMesh()->copyAttributesTo(
                        retVal->getMesh()); // we extracted the hull from one single shape
                else
                    retVal->alignBB("mesh");
...
void CAddOperations::addMenu(VMenu *menu, CSView *subView, bool onlyCamera)
{
    ...
    {
        canAddChildScript = (App::currentWorld->embeddedScriptContainer->getScriptFromObjectAttachedTo(
        ...
        ((associatedViewable != nullptr) && (associatedViewable->getObjectType() == sim_object_camera_type)))
    {
        ...
        if (CSimFlavor::getBoolVal(7) && (!onlyCamera))
        {
            ...
            menu->appendMenuItem(convexHok > 0, false, ADD_COMMANDS_ADD_GROWN_CONVEX_HULL_ACCMD,
...
{ // Return value is true if the command belonged to Add menu and was executed
        ...
        (commandID <= ADD_COMMANDS_ADD_THREADED_CHILD_SCRIPT_PYTHON_ACCMD))
    {
        ...
        { // we are NOT in the UI thread. We execute the command now:
            if (App::currentWorld->sceneObjects->getSelectionCount() == 1)
            {
                ...
                bool isThreaded = (commandID == ADD_COMMANDS_ADD_THREADED_CHILD_SCRIPT_LUA_ACCMD) ||
                                  (commandID == ADD_COMMANDS_ADD_THREADED_CHILD_SCRIPT_PYTHON_ACCMD);
                int scriptID = App::currentWorld->embeddedScriptContainer->insertDefaultScript(
        ...
        (commandID <= ADD_COMMANDS_ADD_THREADED_CUSTOMIZATION_SCRIPT_PYTHON_ACCMD))
    {
        ...
        { // we are NOT in the UI thread. We execute the command now:
            if (App::currentWorld->sceneObjects->getSelectionCount() == 1)
            {
                ...
                bool isThreaded = (commandID == ADD_COMMANDS_ADD_THREADED_CUSTOMIZATION_SCRIPT_LUA_ACCMD) ||
                                  (commandID == ADD_COMMANDS_ADD_THREADED_CUSTOMIZATION_SCRIPT_PYTHON_ACCMD);
                int scriptID = App::currentWorld->embeddedScriptContainer->insertDefaultScript(
    ...
    if (commandID == ADD_COMMANDS_ADD_GROWN_CONVEX_HULL_ACCMD)
    {
        ...
        { // we are NOT in the UI thread. We execute the command now:
            ...
            double grow = 0.03;
            bool doIt = true;
            doIt = GuiApp::uiThread->dialogInputGetFloat(GuiApp::mainWindow, "Convex hull", "Grow parameter", 0.05,
                                                         0.001, 10.0, 3, &grow);
            App::currentWorld->sceneObjects->deselectObjects();
