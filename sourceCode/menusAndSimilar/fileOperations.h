...
{
  public:
    static bool loadScene(const char* pathAndFilename, bool setCurrentDir, std::vector<char>* loadBuffer = nullptr,
                          std::string* infoStr = nullptr, std::string* errorStr = nullptr);
    static bool loadModel(const char* pathAndFilename, bool setCurrentDir, bool doUndoThingInHere,
    ...
    static int createHeightfield(int xSize, double pointSpacing, const std::vector<std::vector<double>*>& readData,
                                 double shadingAngle, int options);
