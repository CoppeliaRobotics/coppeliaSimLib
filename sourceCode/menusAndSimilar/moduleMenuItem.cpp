...
CModuleMenuItem::CModuleMenuItem(const char* path, int nakedScriptHandle)
{
    std::string s(path);
    _label = path;
    size_t pos = 0;
...
int CModuleMenuItem::getnakedScriptHandle() const
{
...
void CModuleMenuItem::addMenu(std::vector<VMenu*>& menus, std::vector<std::string>& labels)
{
    ...
    if (_label.size() > 0)
        menus[menus.size() - 1]->appendMenuItem((_state & 1) != 0, (_state & 2) != 0, _handle, _label.c_str(),
                                                (_state & 6) != 0);
}
#endif
