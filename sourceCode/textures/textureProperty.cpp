...
{
    CTextureObject* it = App::currentScene->textureContainer->getObject(_textureOrVisionSensorObjectID);
    if (it != nullptr)
        it->addDependentObject(objID, objSubID);
}

void CTextureProperty::scaleObject(double scalingFactor)
{
    _objectStateId = -1; // Force a new calculation of texture coordinates
...
// we need to compute the texture coordinates!
...
#ifdef SIM_WITH_GUI
        CVisionSensor* rend =
            App::currentScene->sceneObjects->getVisionSensorFromHandle(_textureOrVisionSensorObjectID);
        if (rend != nullptr)
    ...
    if (it != nullptr)
    {
        ...
        if (sizeX >= sizeY)
            ys = double(sizeY) / double(sizeX);
        else
            xs = double(sizeX) / double(sizeY);
    }
    ...
    for (size_t i = 0; i < triangles.size() / 3; i++)
    {
        ...
        C3Vector v[3];
        v[0].setData(&vertices[3 * ind3[0]]);
        v[1].setData(&vertices[3 * ind3[1]]);
        v[2].setData(&vertices[3 * ind3[2]]);
        v[0] *= tr;
        v[1] *= tr;
        v[2] *= tr;
        double x = 0.0;
        ...
        if ((_textureCoordinateMode == sim_texturemap_cylinder) || (_textureCoordinateMode == sim_texturemap_sphere))
        {
            ...
            for (int ot3 = 0; ot3 < 3; ot3++)
            {
                ...
                if (_textureCoordinateMode == sim_texturemap_cylinder)
                {
                    x = ((atan2(v[ot3](1), v[ot3](0)) / piValue) + 1.0) /
                        2.0; // _textureScalingX doesn't make sense here!  (2.0*_textureScalingX);
                ...
                if (_textureCoordinateMode == sim_texturemap_sphere)
                {
                    x = ((atan2(v[ot3](1), v[ot3](0)) / piValue) + 1.0) /
                        2.0; // _textureScalingX doesn't make sense here!   (2.0*_textureScalingX);
                    double a2 = C3Vector::unitZVector.getAngle(v[ot3]);
            ...
            else
            {
                ...
                {
                    double am = tc[verticesNotInCenterIndex[0]][0];
                    double bm = tc[verticesNotInCenterIndex[1]][0];
                    if (am >= bm)
                    {
                        if (am - bm > 0.5)
...
{
    if (_textureOrVisionSensorObjectID > sim_object_sceneobjectend)
        return (App::currentScene->textureContainer->getObject(_textureOrVisionSensorObjectID));
#ifdef SIM_WITH_GUI
    if ((_textureOrVisionSensorObjectID >= sim_object_sceneobjectstart) &&
        (_textureOrVisionSensorObjectID <= sim_object_sceneobjectend))
    {
        CVisionSensor* rs = App::currentScene->sceneObjects->getVisionSensorFromHandle(_textureOrVisionSensorObjectID);
        if (rs != nullptr)
...
void CTextureProperty::setTextureScaling(double x, double y)
{
    _textureScalingX = x;
    _textureScalingY = y;
    _objectStateId = -1; // Force a new calculation of texture coordinates
...
void CTextureProperty::serialize(CSer& ar)
{
    if (ar.isBinary())
    {
        ...
        { // Storing
            ar.storeDataName(
                "Bst"); // Old, replaced by "Bs2". But keep (in that position) for backward compatibility (24/9/2014)
            ...
            ar << _textureRelativeConfig.Q(0) << _textureRelativeConfig.Q(1) << _textureRelativeConfig.Q(2)
               << _textureRelativeConfig.Q(3);
            ...
            while (theName.compare(SER_END_OF_OBJECT) != 0)
            {
                ...
                if (theName.compare(SER_END_OF_OBJECT) != 0)
                {
                    ...
                    if (theName.compare("_rc") == 0)
                    {
                        noHit = false;
                        ar >> byteQuantity;
                        ar >> _textureRelativeConfig.Q(0) >> _textureRelativeConfig.Q(1) >>
                            _textureRelativeConfig.Q(2) >> _textureRelativeConfig.Q(3);
                    ...
                    if (theName.compare("Ftc") == 0)
                    {
                        noHit = false;
                        ar >> byteQuantity;
                        _fixedTextureCoordinates.resize(byteQuantity / sizeof(float), 0.0);
                        for (size_t i = 0; i < _fixedTextureCoordinates.size(); i++)
                            ar >> _fixedTextureCoordinates[i];
                    }
