...
int CSer::SER_SERIALIZATION_VERSION =
    25; // 9 since 2008/09/01,
        // 15 since 2010/04/17 (after removal of CGeomCont and cloning option for geom resources)
        // 16 since 2012/11/10 (after rewritten shape and geometric classes. We do not support serialization version
        // 14 and earlier anymore!) 17 since 2013/08/29 (after correcting for light intensity and object colors) 18
        // since 2014/07/01 (after saving the vertices, indices, normals and edges in a separate section, in order
        // to reduce file size with redundant content) 19 since 2016/10/29 (Materials are now part of the shapes,
        // and are not shared anymore. This version still supports older CoppeliaSim versions (material data is
        // redundant for a while) 20 since 2017/03/08 (small detail) 21 since 2017/05/26 (New API notation) 22 since
        // 2019/04/29 (Striped away some backward compatibility features) 23 since 2021/03/08 (not a big diff. in
        // format, more in content (e.g. Lua5.3, main script using require, etc.)) 24 since 2022/12/02, V4.5.1
        // (float --> double) 25 since 2023/07/09, V4.5 (float no more supported)

int CSer::SER_MIN_SERIALIZATION_VERSION_THAT_CAN_READ_THIS =
    24; // means: files written with this can be read by older CoppeliaSim with serialization THE_NUMBER
int CSer::SER_MIN_SERIALIZATION_VERSION_THAT_THIS_CAN_READ =
    18; // means: this executable can read versions >=THE_NUMBER
...
void CSer::_writeXmlHeader()
{
    ...
    xmlAddNode_comment(" Linear values are specified in meters, angular values in degrees, unless otherwise specified ",
                       exhaustiveXml);
    xmlAddNode_comment("", exhaustiveXml);
    ...
    if (_xmlDocument.LoadFile(_filename.c_str()) == tinyxml2::XML_SUCCESS)
    {
        if (xmlPushChildNode("CoppeliaSim"))
        {
            ...
            if (xmlGetNode_string("filetype", str))
            {
                ...
                if (xmlGetNode_int("xmlSerializationNb", serializationVersion) &&
                    xmlGetNode_int("prgVer", prgVer, tp == 1) && xmlGetNode_int("prgRev", prgRev, tp == 1))
                {
...
{ // return values: -4 file can't be opened, -3=wrong fileformat, -2=format too old, -1=format too new, 0=compressor
  // unknown, 1=alright!
    ...
    _getFileOpenInfoAndError(fileType, retVal, serializationVersion, coppeliaSimVersionThatWroteThis,
                             licenseTypeThatWroteThis, revNumber, infoStr, errorStr);
    ...
    if (retVal == 1)
    {
        ...
        if (!_noHeader)
        {
            ...
            if (retVal == 1)
            {
                ...
                if (!ignoreTooOldSerializationVersion) // we can most of the time ignore a too old serialization number,
                                                       // if we only want to load the thumbnail
                {
                                    ...
                                    std::string *errorStr /*=nullptr*/)
{
    ...
    if (infoStr != nullptr)
    {
        ...
        if (serializationVersion > SER_SERIALIZATION_VERSION)
            infoStr[0] += "\n*** The file was created with a newer version of CoppeliaSim. Some functionality might "
                          "not be available or not working properly ***";
    }
}
...
{
    return ((_filetype != filetype_csim_xml_xscene_file) && (_filetype != filetype_csim_xml_xmodel_file) &&
            (_filetype != filetype_csim_xml_simplescene_file) && (_filetype != filetype_csim_xml_simplemodel_file));
}
...
{ // keep this as single precision float
    ...
    if (_xmlUseImageAndMeshFileformats && App::worldContainer->pluginContainer->isAssimpPluginAvailable() &&
        (!exhaustiveXml))
    {
...
bool CSer::xmlGetNode_nameAttribute(std::string &val, bool required /*=true*/)
{
    ...
    {
        if (required)
            App::logMsg(sim_verbosity_warnings, "XML read: missing attribute 'name' (stack: %s).",
                        xmlGetStackString().c_str());
    }
    return (nm != nullptr);
}
