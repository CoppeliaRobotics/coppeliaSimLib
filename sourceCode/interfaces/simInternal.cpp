...
const int  ERRCODE_INVALIDPROPERTYVALUE = -9;
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_WRITE_DATA
    {
        ...
        if (isPropertyNameValid(__func__, ppName)) // only when writing data, we still want to read legacy data
        {
            ...
            if ((pState == -1) || App::isTargetValid(pState))
            {
                ...
                {
                    int res = App::setHandleProperty(target, pName.c_str(), pState);
                    if (res == 1)
                        retVal = 1;
                    else if (res == -2)
                    {
            ...
            else
            {
                ...
                retVal = ERRCODE_INVALIDPROPERTYVALUE;
            }
        }
        else
            retVal = ERRCODE_INVALIDPROPERTYNAME;
...
int simGetHandleProperty_internal(long long int target, const char* ppName, long long int* pState)
{
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        // should always pass when reading, (for legacy data names) if (isPropertyNameValid(__func__, ppName))
        {
            ...
            {
                int res = App::getHandleProperty(target, pName.c_str(), pState[0]);
                if (res == 1)
                    retVal = 1;
                else if (res == -2)
                {
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_WRITE_DATA
    {
        ...
        if (isPropertyNameValid(__func__, ppName)) // only when writing data, we still want to read legacy data
        {
            ...
            else
            {
                ...
                int res = App::setMatrixProperty(target, pName.c_str(), m);
                if (res == 1)
                    retVal = 1;
                else if (res == -2)
                {
...
int simGetMatrixProperty_internal(long long int target, const char* ppName, double** pState, int* r, int* c)
{
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        // should always pass when reading, (for legacy data names) if (isPropertyNameValid(__func__, ppName))
        {
            ...
            {
                CMatrix m;
                int res = App::getMatrixProperty(target, pName.c_str(), m);
                if (res == 1)
                {
...
int simGetMatrix4x4Property_internal(long long int target, const char* ppName, double* pState)
{
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        // should always pass when reading, (for legacy data names) if (isPropertyNameValid(__func__, ppName))
        {
            ...
            {
                CMatrix m;
                int res = App::getMatrix4x4Property(target, pName.c_str(), m);
                if (res == 1)
                {
        ...
        if (isPropertyNameValid(__func__, ppName)) // only when writing data, we still want to read legacy data
        {
            if (vL >= 0)
            {
                ...
                if (valid)
                {
                    ...
                    {
                        int res = App::setHandleArrayProperty(target, pName.c_str(), v, vL);
                        if (res == 1)
                            retVal = 1;
                        else if (res == -2)
                        {
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        // should always pass when reading, (for legacy data names) if (isPropertyNameValid(__func__, ppName))
        {
            ...
            else
            {
                ...
                int res = App::getHandleArrayProperty(target, pName.c_str(), vv);
                if (res == 1)
                {
