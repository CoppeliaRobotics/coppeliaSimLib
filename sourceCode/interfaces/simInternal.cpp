...
void simulatorInit()
{
    ...
    {
        if ( (theNames[i].compare("Geometric")==0)||(theNames[i].compare("Dynamics")==0) )
        {
...
simInt simCheckOctreePointOccupancy_internal(simInt octreeHandle,simInt options,const simFloat* points,simInt ptCnt,simUInt* tag,simUInt64* location,simVoid* reserved)
{
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        if (ptCnt==1)
        {
            if (CPluginContainer::geomPlugin_getOctreePointCollision(it->getOctreeInfo(),it->getCumulativeTransformation(),C3Vector(_pts),tag,location))
                return(1);
        }
        else
        {
            if (CPluginContainer::geomPlugin_getOctreePointsCollision(it->getOctreeInfo(),it->getCumulativeTransformation(),_pts,ptCnt))
                return(1);
        }
        return(0);
    }
...
{ // DEPRECATED
    ...
    IF_C_API_SIM_OR_UI_THREAD_CAN_READ_DATA
    {
        ...
        */
        CLuaScriptObject* it=App::ct->luaScriptContainer->getScriptFromID_noAddOnsNorSandbox(scriptHandle);
        C3DObject* obj=App::ct->objCont->getObjectFromHandle(scriptHandle);
        if ( (it!=nullptr)||(obj!=nullptr) )
        {
            if (obj==nullptr)
                obj=App::ct->objCont->getObjectFromHandle(it->getObjectIDThatScriptIsAttachedTo());
            if (obj!=nullptr)
            {
                CUserParameters* uso=obj->getUserScriptParameterObject();
                if (uso!=nullptr)
