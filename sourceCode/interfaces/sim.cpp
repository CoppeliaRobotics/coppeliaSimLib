...
SIM_DLLEXPORT int simSetVectorProperty(long long int target, const char* pName, const double* v, int vL)
{
    return simSetVectorProperty_internal(target, pName, v, vL);
}
SIM_DLLEXPORT double* simGetVectorProperty(long long int target, const char* pName, int* vL)
{
    return simGetVectorProperty_internal(target, pName, vL);
}
SIM_DLLEXPORT int simSetIntVectorProperty(long long int target, const char* pName, const int* v, int vL)
{
    return simSetIntVectorProperty_internal(target, pName, v, vL);
}
SIM_DLLEXPORT int* simGetIntVectorProperty(long long int target, const char* pName, int* vL)
{
    return simGetIntVectorProperty_internal(target, pName, vL);
}
SIM_DLLEXPORT int simRemoveProperty(long long int target, const char* pName)
{
    return simRemoveProperty_internal(target, pName);
}
SIM_DLLEXPORT char* simGetPropertyName(long long int target, int index, SOptions* options)
{
    return simGetPropertyName_internal(target, index, options);
}
SIM_DLLEXPORT int simGetPropertyInfo(long long int target, const char* pName, SPropertyInfo* infos, SOptions* options)
{
...
SIM_DLLEXPORT int simAdjustView(int viewHandleOrIndex, int associatedViewableObjectHandle, int options,
...
SIM_DLLEXPORT int simRegisterScriptCallbackFunction(const char *func, const char *reserved_setToNull,
...
SIM_DLLEXPORT int simRegisterScriptFuncHook(int scriptHandle, const char *funcToHook, const char *userFunction,
...
{
    return (simReceiveData_internal(dataHeader, dataName, antennaHandle, index, dataLength, senderID, dataHeaderR,
                                    dataNameR));
}
...
{
    return (simRuckigPos_internal(dofs, baseCycleTime, flags, currentPos, currentVel, currentAccel, maxVel, maxAccel,
                                  maxJerk, selection, targetPos, targetVel, reserved1, reserved2));
}
SIM_DLLEXPORT int simRuckigVel(int dofs, double baseCycleTime, int flags, const double *currentPos,
                               ...
                               int *reserved2)
{
    return (simRuckigVel_internal(dofs, baseCycleTime, flags, currentPos, currentVel, currentAccel, maxAccel, maxJerk,
                                  selection, targetVel, reserved1, reserved2));
}
...
{
    return (_simDoEntitiesCollide_internal(entity1ID, entity2ID, cacheBuffer, overrideCollidableFlagIfShape1,
                                           overrideCollidableFlagIfShape2, pathOrMotionPlanningRoutineCalling));
}
...
{
    return (simCheckProximitySensorEx_internal(sensorHandle, entityHandle, detectionMode, detectionThreshold, maxAngle,
                                               detectedPoint, detectedObjectHandle, normalVector));
}
...
{
    return (simCheckProximitySensorEx2_internal(sensorHandle, vertexPointer, itemType, itemCount, detectionMode,
...
SIM_DLLEXPORT int simAddGraphCurve_D(int graphHandle, const char *curveName, int dim, const int *streamIds,
                                     ...
                                     int curveWidth)
{
    return (simAddGraphCurve_internal(graphHandle, curveName, dim, streamIds, defaultValues, unitStr, options, color,
                                      curveWidth));
}
...
{
    return (simAddDrawingObject_internal(objectType, size, duplicateTolerance, parentObjectHandle, maxItemCount, color,
                                         setToNULL, setToNULL2, setToNULL3));
}
...
SIM_DLLEXPORT int simImportMesh_D(int fileformat, const char *pathAndFilename, int options,
                                  ...
                                  char ***names)
{
    return (simImportMesh_internal(fileformat, pathAndFilename, options, identicalVerticeTolerance, scalingFactor,
                                   vertices, verticesSizes, indices, indicesSizes, reserved, names));
}
...
{
    return (simExportMesh_internal(fileformat, pathAndFilename, options, scalingFactor, elementCount, vertices,
                                   verticesSizes, indices, indicesSizes, reserved, names));
}
...
{
    return (simCreateTexture_internal(fileName, options, planeSizes, scalingUV, xy_g, fixedResolution, textureId,
                                      resolution, reserved));
}
...
{
    return (
        simSetShapeTexture_internal(shapeHandle, textureId, mappingMode, options, uvScaling, position, orientation));
}
...
{
    return (simGetQHull_internal(inVertices, inVerticesL, verticesOut, verticesOutL, indicesOut, indicesOutL, reserved1,
                                 reserved2));
}
...
{
    return (simGetDecimatedMesh_internal(inVertices, inVerticesL, inIndices, inIndicesL, verticesOut, verticesOutL,
                                         indicesOut, indicesOutL, decimationPercent, reserved1, reserved2));
}
...
{
    return (simSetPointCloudOptions_internal(pointCloudHandle, maxVoxelSize, maxPtCntPerVoxel, options, pointSize,
                                             reserved));
}
SIM_DLLEXPORT int simGetPointCloudOptions_D(int pointCloudHandle, double *maxVoxelSize, int *maxPtCntPerVoxel,
                                            int *options, double *pointSize, void *reserved)
{
    return (simGetPointCloudOptions_internal(pointCloudHandle, maxVoxelSize, maxPtCntPerVoxel, options, pointSize,
                                             reserved));
}
...
SIM_DLLEXPORT int simInsertObjectIntoPointCloud_D(int pointCloudHandle, int objectHandle, int options, double gridSize,
                                                  const unsigned char *color, void *optionalValues)
{
    return (simInsertObjectIntoPointCloud_internal(pointCloudHandle, objectHandle, options, gridSize, color,
                                                   optionalValues));
}
...
{
    return (
        simApplyTexture_internal(shapeHandle, textureCoordinates, textCoordSize, texture, textureResolution, options));
}
...
{
    return (
        _simAddForceSensorCumulativeForcesAndTorques_internal(forceSensor, force, torque, totalPassesCount, simTime));
}
...
{
    return (_simGetDistanceBetweenEntitiesIfSmaller_internal(
