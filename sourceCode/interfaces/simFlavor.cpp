...
std::string CSimFlavor::getStringVal(int w)
{
    ...
    {
#ifdef MAC_SIM
        return ("It seems that CoppeliaSim crashed in last session. If CoppeliaSim didn't crash, you might be running "
                "several instances of CoppeliaSim in parallel, or you might have quit CoppeliaSim not correctly (<File "
                "--> Quit> is correct). Would you like to try to recover auto-saved scenes? (see also CoppeliaSim's "
                "installation folder for backuped scenes)");
#else
        return ("It seems that CoppeliaSim crashed in last session (or you might be running several instances of "
                "CoppeliaSim in parallel). Would you like to try to recover auto-saved scenes? (see also CoppeliaSim's "
                "installation folder for backuped scenes)");
#endif
    }
    if (w == 15)
