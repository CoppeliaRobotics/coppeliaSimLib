        ...
        { // we are NOT in the UI thread. We execute the command now:
            ...
            if (!cancel)
            {
                ...
                for (size_t i = 0; i < sel.size(); i++)
                {
                    ...
                    if (!it->getMesh()->isConvex())
                    {
                        CShape *morphedShape = _morphToConvexDecomposed(
                            it, nClusters, maxConcavity, addExtraDistPoints, addFacesPoints, maxConnectDist,
                            maxTrianglesInDecimatedMesh, maxHullVertices, smallClusterThreshold, useHACD, resolution,
                            depth, concavity, planeDownsampling, convexHullDownsampling, alpha, beta, gamma, pca,
                            voxelBased, maxNumVerticesPerCH, minVolumePerCH);
...
{                         // returned shape is the last shape in the selection (it is modified, others are destroyed)
    ...
    for (size_t i = 0; i < shapesToGroup.size(); i++)
    {
        CShape *it = shapesToGroup[i];
        if (allToNonPure)
            it->getMesh()->setPurePrimitiveType(sim_primitiveshape_none, 1.0, 1.0,
                                                1.0); // this will be propagated to all geometrics!
    ...
    for (size_t i = 0; i < allMeshes.size(); i++)
    {
        ...
        mesh->setIFrame(lastSel->getCumulativeTransformation().getInverse() *
                        shapesToGroup[i]->getCumulativeTransformation() * mesh->getIFrame());
...
void CSceneObjectOperations::addMenu(VMenu *menu)
{
    ...
    if (GuiApp::getEditModeType() == NO_EDIT_MODE)
    {
        ...
        {
            menu->appendMenuItem((shapeCnt > 0) && noSim, false, SCENE_OBJECT_OPERATION_MORPH_INTO_CONVEX_SHAPES_SOOCMD,
                                 "Morph into convex shape(s)");
            menu->appendMenuItem((shapeCnt > 0) && noSim, false,
                                 SCENE_OBJECT_OPERATION_MORPH_INTO_CONVEX_DECOMPOSITION_SOOCMD,
                                 "Morph into convex decomposed shape(s)...");
            menu->appendMenuItem((shapeCnt > 0) && noSim, false, SCENE_OBJECT_OPERATION_DECIMATE_SHAPE_SOOCMD,
                                 "Decimate shape(s)...");
        }
        menu->appendMenuSeparator();
        ...
        if (CSimFlavor::getBoolVal(12))
        {
            ...
            menu->appendMenuAndDetach(removing, (hasChildScriptAttached || hasCustomizationScriptAttached) && noSim,
                                      IDSN_REMOVE_MENU_ITEM);
            menu->appendMenuSeparator();
